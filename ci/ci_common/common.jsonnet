    ...
    if (std.length(std.find('gate', build.targets)) > 0 || std.length(std.find('deploy', build.targets)) == 0) then {
      guard+: {
        excludes+: ["*.md",
          ...
          "<graal>/tools/**.md",
          "<graal>/truffle/**{[!s],[!n]?,[!o]??,[!i]???,[!t]????,[!p]?????,[!O]??????,????????,[!s]/???????,[!c]?/???????,[!o]??/???????,[!d]???/???????,?????/???????,?/????/???????}.md", # all md files in <graal>/truffle except for <graal>/truffle/docs/Options.md
          "<graal>/visualizer/**.md",
          ...
          "<graal>/vm/docs/**",
          "<graal>/wasm/**.md",
          "<graal>/docs/**{[!d],[!m]?,[!.]??,[!g]???,[!n]????,[!i]?????,[!l]??????,[!i]???????,[!f]????????,[!o]?????????,[!r]??????????,[!p]???????????,?????????????,[!s]/????????????,[!l]?/????????????,[!o]??/????????????,[!o]??/????????????,[!o]???/????????????,[!t]????/????????????,??????/????????????,,?/?????/????????????", # everything in <graal>/docs except for <graal>/docs/tools/profiling.md
          "<graal>/.devcontainer/**",
          "<graal>/.github/**",
