...
public final class Thermometer implements AutoCloseable {
    ...
    private long lastObservedCompilationActivity;
    private long lastObservedCompilationTime;
    private int interruptAfterStable;
    ...
    public Thermometer(Env env) {
        this.env = env;
        lastObservedCompilationActivity = 0;
        maxHeapSize = 0;
    }
    ...
    public synchronized void start(ThermometerConfig config) {
        ...
        lastObservedCompilationTime = startTime;
        interruptAfterStable = config.getInterruptAfterStable();

        // Install a node at the root of each compilation unit to set the compilation flag

        final SourceSectionFilter rootNodeFilter = SourceSectionFilter.newBuilder().tagIs(StandardTags.RootTag.class).build(); // FIXME this is supposed to be CompilationRootTag!
    ...
    private void report() {
        ...
        final int activity = compilationState.getQueued() + compilationState.getFinished() + compilationState.getDequeued() + compilationState.getFinished();

        if (activity > lastObservedCompilationActivity) {
            lastObservedCompilationTime = time;
            lastObservedCompilationActivity = activity;
        }

        final long timeSinceCompilation = time - lastObservedCompilationTime;

        final long heapSize = getUsedMemory();
        maxHeapSize = Long.max(maxHeapSize, heapSize);
        ...
        if (interruptAfterStable > 0 && compilationState.getQueued() == 0 && timeSinceCompilation / 1e9 > interruptAfterStable) {
            interruptAfterStable = 0;
            env.getLogger("").severe("interrupting because stable");
