...
public abstract class StateTransitionCanonicalizer<TS extends TransitionSet, TB extends TransitionBuilder<TS>> {
     ...
     * intersections.</li>
     * <li>Merge {@link TransitionBuilder}s generated by the first phase if their
     * <em>target state</em> is equal and
     * {@link #isSameTargetMergeAllowed(TransitionBuilder, TransitionBuilder)} returns
     * <code>true</code>.</li>
     * </ol>
     *
     ...
     * given an input list <code>[1, 2, 3]</code> where all elements intersect with each other,
     * element <code>2</code> will be merged with <code>1</code> before <code>3</code> is merged
     * with <code>1</code>. This property is crucial for generating priority-sensitive DFAs with
     ...
     * {@link #isSameTargetMergeAllowed(TransitionBuilder, TransitionBuilder)} returns
     * <code>true</code>. Equality of target state sets is checked by
     * {@link TransitionSet#hashCode()} and {@link TransitionSet#equals(Object)}.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns <code>true</code> if two DFA transitions leading to the same target state are allowed
