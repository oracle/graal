...
public class RubyCaseFolding {
    ...
    public static String caseFoldUnfoldString(int[] codepoints, CodePointSet encodingRange) {
        return caseFoldUnfoldString(codepoints, encodingRange, false);
    }
     ...
     *            the variants
     * @return a parenthesized ECMAScript regex that can be spliced into the regex generated by
     *         {@link RubyRegexParser}
     */
    public static String caseFoldUnfoldString(int[] codepoints, CodePointSet encodingRange, boolean dropAsciiOnStart) {
        ...
        for (int i = 0; i < unfoldings.size(); i++) {
            ...
            if (unfolding.getStart() >= end) {
                unfoldSegment(out, caseFolded, unfoldings.subList(unfoldingsStartIndex, unfoldingsEndIndex), start, end, 0, dropAsciiOnStart);
                if (unfolding.getStart() > end) {
                    // If the following mandatory string that we would emit would be at the
                    // beginning of the matcher and it would match an ASCII character, then we
                    // return an empty (impossible) matcher (if dropAsciiOnStart is set).
    ...
    private static void emitChar(StringBuilder out, int codepoint, boolean inCharClass) {
        TBitSet syntaxChars = inCharClass ? RubyRegexParser.CHAR_CLASS_SYNTAX_CHARACTERS : RubyRegexParser.SYNTAX_CHARACTERS;
    ...
    private static void emitString(StringBuilder out, List<Integer> codepoints) {
        for (int codepoint : codepoints) {
            if (RubyRegexParser.SYNTAX_CHARACTERS.get(codepoint)) {
                out.append('\\');
            }
            out.appendCodePoint(codepoint);
        }
    ...
    private static void unfoldSegment(StringBuilder out, List<Integer> caseFolded, List<Unfolding> unfoldings, int start, int end, int backtrackingDepth, boolean dropAsciiOnStart) {
        ...
        if (unfolding.getStart() > start) {
            ...
            unfoldSegment(out, caseFolded, unfoldings, unfolding.getStart(), end, backtrackingDepth, dropAsciiOnStart);
            return;
        }
        ...
        if (unfolding.getLength() > 1) {
            ...
            unfoldSegment(out, caseFolded, unfoldings.subList(unfoldingsNextIndex, unfoldings.size()), unfolding.getEnd(), end, backtrackingDepth + 1, dropAsciiOnStart);
            out.append('|');
            // In the second alternative, where we decide not to pursue the unfolding, we just drop
            // it from the `unfoldings` list.
            unfoldSegment(out, caseFolded, unfoldings.subList(1, unfoldings.size()), start, end, backtrackingDepth + 1, dropAsciiOnStart);
            out.append(')');
            return;
        }
        ...
        while (unfoldingsNextIndex < unfoldings.size() && unfoldings.get(unfoldingsNextIndex).getStart() == start) {
            ...
            if (!dropAsciiOnStart || start != 0 || !RubyRegexParser.isAscii(codepoint)) {
                emitChar(out, codepoint, true);
            }
            unfoldingsNextIndex++;
        }
        out.append("]");
        unfoldSegment(out, caseFolded, unfoldings.subList(unfoldingsNextIndex, unfoldings.size()), start + 1, end, backtrackingDepth, dropAsciiOnStart);
    }
}
