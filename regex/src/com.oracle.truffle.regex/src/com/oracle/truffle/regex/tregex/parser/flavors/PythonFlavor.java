...
public final class PythonFlavor implements RegexFlavor {
    ...
    public RegexFlavorProcessor forRegex(RegexSource source) {
        return new PythonFlavorProcessor(source);
    }

    private final class PythonFlavorProcessor implements RegexFlavorProcessor {
        ...
        private PythonFlavorProcessor(RegexSource source) {
            this.input = source.getPattern();
            this.flags = source.getGeneralFlags();
            this.position = 0;
            this.sb = new StringBuilder(input.length());
        ...
        public RegexSource toECMAScriptRegex() throws RegexSyntaxException, UnsupportedRegexException {
            silent = false;
            parse();
            return new RegexSource(sb.toString(), "some flags here");
        }
