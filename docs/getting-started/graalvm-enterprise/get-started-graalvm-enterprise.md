...
next: /en/graalvm/enterprise/21/docs/examples/
previous: /en/graalvm/enterprise/21/docs/overview/solutions/
---
...
If you are new to GraalVM Enterprise or have little experience using it, we recommend starting with the [GraalVM Enterprise Overview](/en/graalvm/enterprise/21/docs/overview/technical-architecture/) page.
...
If you have GraalVM Enterprise already installed and have experience using it, you can skip this getting started guide and proceed to the in-depth [Reference Manuals](/en/graalvm/enterprise/21/docs/reference-manual/).
...
* [Linux](/en/graalvm/enterprise/21/docs/getting-started/installation-linux/)
* [Linux ARM64](/en/graalvm/enterprise/21/docs/getting-started/installation-linux-aarch64/)
* [Oracle Linux](/en/graalvm/enterprise/21/docs/getting-started/oci/compute-instances/)
* [macOS](/en/graalvm/enterprise/21/docs/getting-started/installation-macos/)
* [Windows](/en/graalvm/enterprise/21/docs/getting-started/installation-windows/)
...
Java HotSpot(TM) 64-Bit Server VM GraalVM EE 21.1.0 (build 11.0.11+8-LTS-jvmci-21.1-b03, mixed mode, sharing)

js -version
GraalVM JavaScript (GraalVM EE Native 21.1.0)

lli --version
LLVM 10.0.0 (GraalVM EE Native 21.1.0)
```
...
You can find a collection of larger Java examples on the [Examples Applications](/en/graalvm/enterprise/21/docs/docs/examples/) page.
For more information on the GraalVM compiler, go to [Compiler](/en/graalvm/enterprise/21/docs/reference-manual/compiler/).
For more extensive documentation on running Java, proceed to [JVM Languages](/en/graalvm/enterprise/21/docs/reference-manual/java/).
...
For more detailed documentation and information on compatibility with Node.js, proceed to [JavaScript and Node.js](/en/graalvm/enterprise/21/docs/reference-manual/js/).
...
For in-depth documentation and more examples of running LLVM bitcode on GraalVM Enterprise, go to [LLVM Languages](/en/graalvm/enterprise/21/docs/reference-manual/llvm/).

### Python

With GraalVM Enterprise you can run Python applications in the Python 3 runtime environment.
The support is not available by default, but you can quickly add it to GraalVM using the [GraalVM Updater](/en/graalvm/enterprise/21/docs/reference-manual/graalvm-updater/) tool:
```shell
gu install python
```
...
More examples and additional information on Python support in GraalVM can be found in the [Python reference manual](/en/graalvm/enterprise/21/docs/reference-manual/python/).
...
The Ruby runtime is not available by default in GraalVM, but can be easily added using the [GraalVM Updater](/en/graalvm/enterprise/21/docs/reference-manual/graalvm-updater/) tool:
```shell
gu install ruby
```
...
[same options as the standard implementation of Ruby](/en/graalvm/enterprise/21/docs/reference-manual/ruby/Options/),
...
More examples and in-depth documentation can be found in the [Ruby reference manual](/en/graalvm/enterprise/21/docs/reference-manual/ruby/).

### R

GraalVM Enterprise provides a GNU-compatible environment to run R programs directly or in the REPL mode.
Although the R language support is not available by default, you can add it to GraalVM Enterprise using the [GraalVM Updater](/en/graalvm/enterprise/21/docs/reference-manual/graalvm-updater/) tool:
```shell
gu install R
```
...
More examples and in-depth documentation can be found in the [R reference manual](/en/graalvm/enterprise/21/docs/reference-manual/r/).

### WebAssembly

With GraalVM Enterprise you can run programs compiled to WebAssembly.
The support is not available by default, but you can add it to GraalVM using the [GraalVM Updater](/en/graalvm/enterprise/21/docs/reference-manual/graalvm-updater/) tool:
```shell
gu install wasm
```
...
More details can be found in the [WebAssembly reference manual](/en/graalvm/enterprise/21/docs/reference-manual/wasm/).
...
The [Native Image](/en/graalvm/enterprise/21/docs/reference-manual/enterprise-native-image/) functionality is not available by default, but can be easily installed with the [GraalVM Updater](/en/graalvm/enterprise/21/docs/reference-manual/graalvm-updater/) tool:
```shell
...
More detailed documentation on this innovative technology is available in the [Native Image reference manual](/en/graalvm/enterprise/21/docs/reference-manual/enterprise-native-image/).
...
the [Truffle language implementation framework](/en/graalvm/enterprise/21/docs/graalvm-as-a-platform/language-implementation-framework/) included, which is the case here.
...
For more information on running polyglot applications, see [Polyglot Programming](/en/graalvm/enterprise/21/docs/reference-manual/polyglot-programming/).

## What to Read Next

### New Users
Since this guide is intended mainly for users new to GraalVM Enterprise, or users
who are familiar with GraalVM Enterprise but may have little experience using it, consider investigating more complex [Example Applications](/en/graalvm/enterprise/21/docs/examples/).

### Oracle Cloud Users
Oracle Cloud users considering GraalVM Enterprise for their cloud workloads are
invited to read [GraalVM Enterprise on OCI](/en/graalvm/enterprise/21/docs/getting-started/oci/compute-instances/).
This page focuses on using GraalVM Enterprise with the Oracle Cloud Infrastructure Virtual Machine compute instance.

### Advanced Users
If you are mostly interested in GraalVM Enterprise support for a specific language, or want more in-depth details about GraalVM Enterprise's diverse features, proceed to [Reference Manuals](/en/graalvm/enterprise/21/docs/reference-manual/).

If you are looking for the tooling support GraalVM Enterprise offers, proceed to [Debugging and Monitoring Tools](/en/graalvm/enterprise/21/docs/tools/).

If you are considering GraalVM Enterprise as a platform for your future language or tool implementation, go to [GraalVM Enterprise as a Platform](/en/graalvm/enterprise/21/docs/graalvm-enterprise-as-a-platform/).

You can find information on GraalVM Enterprise's security model in the [Security Guide](/en/graalvm/enterprise/21/docs/security/security-guide/), and rich API documentation in [GraalVM SDK Javadoc](https://docs.oracle.com/en/graalvm/enterprise/21/sdk/index.html).
