...
To add debug information to a generated native executable, provide the `-g -O0` option to the `native-image` builder:
```shell
...
The resulting executable will contain debug records in a format the GNU Debugger (GDB) understands.
