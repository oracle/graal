---
layout: docs
toc_group: dynamic-features
link_title: Accessing Resources
permalink: /reference-manual/native-image/dynamic-features/Resources/
redirect_from: /reference-manual/native-image/Resources/
---

# Accessing Resources in Native Image

By default, the `native-image` builder will not integrate any of the resources that are on the classpath into the native executable.
To make calls such as `Class.getResource()` or `Class.getResourceAsStream()` (or their corresponding `ClassLoader` methods) return specific resources (instead of `null`), you must specify the resources that should be accessible at runtime.

## Resource registration

There are few possible ways how you can request (register) certain resource to be included in your `native-image` and therefore accessible at runtime:
1. Resource Configuration file (`resource-config.json`)
2. Command line option
3. Public API

### Resource configuration file

Resource configuration file contains information about resources you want, encoded with patterns used to match actual resources on your system.
This configuration file is always named `resource-config.json`.
You can either generate it (using [Tracing Agent](AutomaticMetadataCollection/#tracing-agent)) and then manually refine it, or create it from scratch.
Created configuration file should have the following structure (described in [resource-config-schema](assets/resource-config-schema-v1.1.0.json)):

```json
{
  "globs": [
      {
       "glob": "<Glob pattern restricted to only support * and ** wildcards besides literals>"
      },
      {
       "glob": "<another glob pattern>"
      },
      ...
  ],
  "resources": [
      {
       "pattern": "<Java regexp that matches resource(s) to be included in the executable>"
      },
      {
       "pattern": "<another regexp>"
      },
      ...
  ]
}
```

You can choose one of the formats to describe requested resources (or combine them if necessary):
1. Globs (**recommended**)
2. Regexp patterns

Once created, configuration file's path can be passed to the `native-image` tool using the option `-H:ResourceConfigurationFiles=/path/to/resource-config.json`.

#### Globs

You can write glob patterns to describe resources (you want to include) in the `globs` section of the configuration file.
Also, if you use the [Tracing Agent](AutomaticMetadataCollection/#tracing-agent) to generate configuration file, it will print all entries in this format.

In our case, globs are restricted to only support star(`*`) and globstar(`**`) wildcards.
Per definition, **star** can match any number of any characters on one level while **globstar** can match any number of any levels.

We recommend using globs because they:
* Have custom handling in the `native-image` that can speed up resource intensive applications
* Are less expressive and therefore less error-prone than regexp
* Give better support for resource related checks at runtime


#### Regexp

Alternatively, you can write standard Java Regexp patterns to describe resources (you want to include) in the `resources` section of the configuration file.
This approach should be used only in extreme cases (as a workaround) if the expressive power of globs is not enough for your use case.


### Command line option

Alternatively, you can specify individual resource paths directly to the `native-image` tool as follows:

```shell
native-image -H:IncludeResources="<Java regexp that matches resources to be included in the executable>" ...
```
Note that with this approach, you can only specify patterns written in the `regexp` format, and therefore all advantages of globs wil not be used in this case.

### Public API

On the other hand, you may also want to register resources programmatically.
To do so, you can use functions from our public API and call them in your [`Feature`](https://www.graalvm.org/sdk/javadoc/org/graalvm/nativeimage/hosted/Feature.html).
With this approach, you cannot specify resource using patterns, but only with its literal name.
Also, be aware that resource registration cannot be performed after [beforeAnalysis](https://www.graalvm.org/sdk/javadoc/org/graalvm/nativeimage/hosted/Feature.html#beforeAnalysis(org.graalvm.nativeimage.hosted.Feature.BeforeAnalysisAccess)) phase.


## Embedded resources information

There are two possible ways to see which resources were included in the native executable:
1. Use the option `-H:+BuildReport` to generate build report of your native executable.
   There you can find all information about embedded resource under `Resoruces` tab.
2. Use the option `-H:+GenerateEmbeddedResourcesFile` to generate `embedded-resources.json`

For each registered resource you will get its:
* **Module** (or `unnamed` if resource doesn't belong to any module)
* **Name** (resource path)
* **Origin** (location of the resource on the system)
* **Type** (whether the resource is file, directory or missing)
* **Size** (actual resource size)

_Note: size of directory resource represents only size of names of all entries in the directory (not sum of the content sizes)_


## Example Usage

Given this project structure:
```
app-root
└── src
    └── main
        └── resources
            ├── Resource0.txt
            └── Resource1.txt
```
Then:

* All resources can be loaded with:
 * `"**/Resource*txt"`, specified as `{"glob":"**/Resource*txt"}` in a configuration file (**recommended**),
 * `".*/Resource.*txt"`, specified as `{"pattern":".*/Resource.*txt"}` in a configuration file, or `-H:IncludeResources=".*/Resource.*txt"` on the command line.
* _Resource0.txt_ can be loaded with:
 * `**/Resource0.txt` - globs format
 * or with `.*/Resource0.txt` - regexp format.
* _Resource0.txt_ and _Resource1.txt_ can be loaded with:
 * `**/Resource0.txt` and `**/Resource1.txt`- globs format
 * `.*/Resource0.txt` and `.*/Resource1.txt` (or alternatively with a single `.*/(Resource0|Resource1).txt$`) - regexp format

Check [this guide](guides/include-resources.md) which illustrates how to include a resource into a native executable.

## Resource Bundles

Java localization support (`java.util.ResourceBundle`) enables Java code to load L10N resources and show the user messages suitable for runtime settings such as time, locale, and format.

Native Image needs knowledge ahead-of-time of the resource bundles your application needs so that it can load and store the appropriate bundles for usage in the generated executable.
The bundles can be specified in the resource configuration file (see above), in the `bundles` section:

```json
{
  "bundles": [
    {"name":"your.pkg.Bundle"},
    {"name":"another.pkg.Resource"},
    {"name":"etc.Bundle"}
  ],
  "resources": <see above>
}
```

Alternatively, bundles can be specified directly as options to the `native-image` tool as follows:
```shell
native-image -H:IncludeResourceBundles=your.pgk.Bundle,another.pkg.Resource,etc.Bundle ...
```
By default, requested bundles are included for all requested locales.
To optimize this, it is possible to use `IncludeResourceBundles` with a locale-specific substring, for example `-H:+IncludeResourceBundles=com.company.bundles.MyBundle_fr-FR` will only include the bundle in French.

## Locales

It is also possible to specify which locales should be included in the executable and which should be the default.
For example, to switch the default locale to Swiss German and also include French and English, use the following  options:
```shell
native-image -Duser.country=CH -Duser.language=de -H:IncludeLocales=fr,en
```
The locales are specified using [language tags](https://docs.oracle.com/javase/tutorial/i18n/locale/matching.html). You can include all
locales via ``-H:+IncludeAllLocales``, but note that it increases the size of the resulting
executable.


## Resources in Java Modules

For every resource (either specified with globs or regexp) or resource bundles, it is possible to specify the exact modules from which these resources or bundles should be taken.

* For glob based resource patterns, you can specify module name in the separate `module` field in each entry
* For regex based resource patterns, or bundles, you can specify module name before the resource/bundle name with `:` as the separator

For example:

```json
{
   "globs": [
      {
        "module:": "library-module",
        "glob": "resource-file.txt" 
      }
   ],
   "resources": [
      {
        "pattern": "library-module:^resource-file.txt$"
      }
   ],
   "bundles": [
      {
        "name":"main-module:your.pkg.Bundle"
      }
   ]
}
```

This will cause the `native-image` tool to only include `resource-file.txt` from the Java module `library-module`.
If other modules or the classpath contains resources that match the pattern `resource-file.txt` only the one in module `library-module` is registered for inclusion in the executable.
Similarly, if other bundles are accessible with the same bundle name `your.pkg.Bundle` only the one from `main-module` is included.
Native image will also ensure that the modules are guaranteed to be accessible at runtime.

The following code pattern:
```java
InputStream resource = ModuleLayer.boot().findModule(moduleName).getResourceAsStream(resourcePath);
```
will always work as expected for resources registered as described above (even if the module does not contain any code that is considered reachable by static analysis).


## Java VM Mode of Localization

Resource Bundle lookup is a complex and dynamic mechanism which utilizes a lot of Java VM infrastructure.
As a result, it causes the size of the executable to increase for smaller applications such as `HelloWorld`.
Therefore, an optimized mode is set by default in which this lookup is simplified utilizing the fact that all bundles are known ahead of build time.
For the original Java VM lookup, use the `-H:-LocalizationOptimizedMode` option.

### Further Reading

* [Include Resources in a Native Executable](guides/include-resources.md)
* [Native Image Build Configuration](BuildConfiguration.md)