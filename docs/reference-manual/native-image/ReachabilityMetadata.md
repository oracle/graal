...
To provide metadata for a lambda class, add the following entry to the `reflection` array in
...
If `"declaringMethod"` is not provided, all methods of the specified declaring class are searched.
The `"interfaces"` field specifies which interfaces are implemented by the lambda class.
This definition can match multiple lambda classes; in such cases, the registration entry applies to all matching
classes.
