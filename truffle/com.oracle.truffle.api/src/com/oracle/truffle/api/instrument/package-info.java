 ...
 * CoverageTracker.
 * It also supports Truffle's built-in debugging services, as well as utilities
 * that maintain maps of source code locations
 * for other tools such as debugging.
 *
 * <h4>Instrumentation Services</h4>
 * <p>
 * API access to instrumentation services is provided by the Instrumenter that is part of
 ...
 * <ul>
 * <li>Execution events can only be observed at AST locations that have been probed,
 * which results in the creation of a Probe that is permanently associated with a
 ...
 * <li>The relationship between a Probe and
 * a source code location persists across Truffle <em>cloning</em> of ASTs, which is to say, a single
 * Probe corresponds to (and keeps track of)
 * the equivalent location in every clone of the original AST.</li>
 * <li>Probing is specified by registering
 * an instance of ASTProber.
 * A default prober provided by each TruffelLanguage
 ...
 * <li>Clients can also
 * find all existing
 * Probes, possibly filtering the search by
 * tag.</li>
 * </ul></li>
 *
 ...
 * Probe. This creates an
 * Instrument that notifies the listener
 * of every subsequent execution event at the AST location corresponding to the
 * Probe.</li>
 * <li>An
 * Instrument can be
 * disposed, at which time
 ...
 * listener called a
 * StandardBeforeInstrumentListener to every
 * Probe containing a specified
 * tag.</li>
 ...
 * disabled and re-enabled and eventually
 * disposed when no longer needed.</li>
 * <li>A useful example is the
 * LineToProbesMap, which incrementally builds and maintains a map of
 * Probes indexed by
 * source code line. Truffle
 * debugging services depend heavily on this utility.</li>
 * <li>The CoverageTracker maintains counts of execution events where a
 * Probe has been tagged with
