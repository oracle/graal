    ...
    def parse(self, text):
        ...
        with open(self._prepend_working_dir(self.filename), encoding="utf-8") as fp:
            for result in json.load(fp):
                ...
                summary_datapoint.update({"metric.name": self.metric_name, "metric.value": float(metric["score"])})
...
def javadoc(args, vm=None):
    ...
    mx.javadoc(
        ["--unified", "--disallow-all-warnings", "--projects", ",".join(projects), *extraArgs, *args], includeDeps=False
    )
...
def checkLinks(javadocDir):
    ...
    for referencedfile, sections in filesToCheck.items():
        if (
            referencedfile.startswith("javascript:")
            or referencedfile.startswith("http:")
            or referencedfile.startswith("https:")
            or referencedfile.startswith("mailto:")
        ):
            continue
...
mx_unittest.add_unittest_argument(
    ...
    default=False,
    help="Disables the automatic inclusion of Truffle optimized runtime in unittest dependencies.",
    action=_DisableOptimizedRuntimeAction,
)
...
class _TruffleNFIUnittestConfig(mx_unittest.MxUnittestConfig):
    ...
    def processDeps(self, deps):
        deps.update(
            {mx.distribution(runtime_dep) for runtime_dep in _TruffleNFIUnittestConfig.runtimeConfig.runtime_deps}
        )
...
mx_unittest.add_unittest_argument(
    "--nfi-config",
    default=None,
    help="Select test engine configuration for the nfi unittests.",
    metavar="<config>",
    action=_SelectNFIConfigAction,
)
...
def _sl_command(jdk, vm_args, sl_args, use_optimized_runtime=True, force_cp=False):
    ...
    return [
        jdk.java,
        *jdk.processArgs(vm_args + mx.get_runtime_jvm_args(names=dist_names, force_cp=force_cp) + main_class + sl_args),
    ]
...
def slnative(args):
    ...
    parser = ArgumentParser(
        prog="mx slnative",
        description="Builds and executes native SL image.",
        usage="mx slnative [--target-folder <folder>|@VM options|--|SL args]",
    )
    ...
    image = _native_image_sl(
        ...
        force_cp=False,
        hosted_assertions=False,
        log_host_inlining=mx.env_var_to_bool("SL_NATIVE_HOST_INLINING"),
    )
...
def _native_image(jdk):
    ...
    if not exists(native_image_path):
        mx.abort(
            f"No native-image installed in GraalVM {jdk.home}. Switch to an environment that has an installed native-image command."
        )
    return native_image_path


def _native_image_sl(
    jdk,
    vm_args,
    target_dir,
    use_optimized_runtime=True,
...
def gate_truffle_jvm(tasks):
    ...
    with Task("Truffle SL JVM", tasks, tags=TruffleGateTags.truffle_jvm) as t:
        if t:
            supports_optimization = mx_sdk.GraalVMJDKConfig.is_graalvm(
                jdk.home
            ) or mx_sdk.GraalVMJDKConfig.is_libgraal_jdk(jdk.home)
...
def gate_truffle_native(tasks):
    ...
    with Task(
        "Truffle API Native Tests with static libc musl", tasks, tags=TruffleGateTags.truffle_native_libcmusl_static
    ) as t:
        if t:
            truffle_native_unit_tests_gate(True, ["--libc=musl", "--static"])
...
def _truffle_jvm_module_path_unit_tests_gate(additional_unittest_options=None):
    ...
    unittest(
        ["--suite", "truffle", "--enable-timing", "--verbose", "--max-class-failures=25", *additional_unittest_options]
    )
...
def native_truffle_unittest(args):
    ...
    parser = ArgumentParser(
        prog="mx native-truffle-unittest", description="Run truffle unittests as native image on graalvm."
    )
    ...
    parser.add_argument(
        supported_args[2], metavar="CLASS_NAME", action="append", help="exclude given test from test execution"
    )
    ...
    parsed_args.exclude_class = (
        [re.compile(fnmatch.translate(c)) for c in parsed_args.exclude_class] if parsed_args.exclude_class else []
    )
    success = False
    ...
    try:
        ...
        mx_unittest._run_tests(
            parsed_args.unittests_args,
            collect_unittest_distributions,
        ...
        vm_args = (
            enable_asserts_args
            + uid_tracking_args
            + mx.get_runtime_jvm_args(names=unittest_distributions + truffle_runtime_distributions)
            + module_args
        )
        # GR-65661: we need to disable the check in GraalVM for 21 as it does not allow polyglot version 25.1.0-dev
        if jdk.version < mx.VersionSpec("25"):
        ...
        if success:
            ...
            mx.warn(
                f"The native Truffle unit test has failed, preserving the working directory at {tmp} for further investigation."
            )
...
def _sl_jvm_gate_tests(jdk, vm_args, force_cp=False, compile_immediately=True):
    ...
    def run_jvm_fallback(test_file):
        return _sl_command(
            jdk,
            vm_args,
            [test_file, "--disable-launcher-output", "--engine.WarnInterpreterOnly=false", *default_args],
...
def truffle_native_context_preinitialization_tests(build_args=None):
    ...
    native_truffle_unittest(
        [
            "com.oracle.truffle.api.test.polyglot.ContextPreInitializationNativeImageTest",
            "--build-args",
            *use_build_args,
        ]
    )
...
def truffle_native_unit_tests_gate(use_optimized_runtime=True, build_args=None):
    ...
    build_args = (
        ...
        + (
            mx_sdk_vm_impl.svm_experimental_options(["-H:+DumpThreadStacksOnSignal"])
...
def _truffle_gate_state_bitwidth_tests():
    ...
    for run_bits in runs:
        ...
        unittest_args = [
            ...
            "--enable-timing",
            "--max-class-failures=25",
            f"-Dtruffle.dsl.StateBitWidth={run_bits}",
...
mx_unittest.add_unittest_argument(
    "--lookup-truffle-tck-providers",
    default=False,
    help="Enables lookup of Truffle TCK providers.",
    action=_EnableTCKUnittestConfigAction,
)
...
def tck(args):
    ...
    parser = ArgumentParser(
        ...
        epilog=_tckHelpSuffix,
    )
    parser.add_argument(
        "--tck-configuration", help="TCK configuration", choices=["compile", "debugger", "default"], default="default"
    )
    parsed_args, args = parser.parse_known_args(args)
...
mx.update_commands(
    _suite,
    {
        "tck": [
            tck,
            "[--tck-configuration {compile|debugger|default}] [unittest options] [--] [VM options] [filters...]",
            _tckHelpSuffix,
        ]
    },
)
...
def check_filename_length(args):
    ...
    if too_long:
        ...
        for x in too_long:
            mx.log_error(x)
        mx.abort(
            f"File names that are too long where found. Ensure all file names are under {max_length} characters long."
        )
...
def create_parser(
    ...
    for filename in generated_files:
        ...
        content = re.compile("^@SuppressWarnings.*$", re.MULTILINE).sub(
            '@SuppressWarnings({"all", "this-escape"})', content
        )
        # remove useless casts
        content = re.compile(r"\(\([a-zA-Z_]*Context\)_localctx\)").sub("_localctx", content)
        content = re.compile(r"\(Token\)_errHandler.recoverInline\(this\)").sub(
            "_errHandler.recoverInline(this)", content
        )
        ...
        if shaded:
            ...
            content = re.compile(r"^import (org\.antlr\.v4\.runtime(?:\.\w+)*(?:\.\*)?);", re.MULTILINE).sub(
                r"import org.graalvm.shadowed.\1;", content
            )
...
def validate_parser(grammar_project, grammar_path, create_command, args=None, out=None):
    ...
    for before, after, path in ((parser_before, parser_after, parser), (lexer_before, lexer_after, lexer)):
        if before != after:
            ...
            nl = os.linesep
            mx.abort(
                f"Content generated from {grammar_path} does not match content of {path}:{nl}"
                + f"{diff}{nl}"
...
def register_polyglot_isolate_distributions(
    language_suite,
    register_project,
    ...
    maven_group_id,
    language_license,
    isolate_build_options=None,
    platforms=None,
    additional_image_path_artifacts=None,
    ...
    if not polyglot_isolates_value or not (
        polyglot_isolates_value is True
        or (isinstance(polyglot_isolates_value, list) and main_language_id in polyglot_isolates_value)
    ):
        return False
    ...
    for platform in platforms:
        ...
        build_internal_resource = _PolyglotIsolateResourceProject(
            language_suite,
            subDir,
            main_language_id,
            all_language_ids,
        ...
        if build_for_current_platform:
            ...
            layout_dist = mx.LayoutDirDistribution(
                ...
                layout={
                    ...
                    f"{resource_base_folder}/external_isolate/": {
                        "source_type": "dependency",
...
mx.add_argument(
    "--polyglot-isolates",
    action="store",
    help="Comma-separated list of languages for which the polyglot isolate library should be built. Setting the value to `true` builds all polyglot isolate libraries.",
)
...
class LibffiBuilderProject(mx_native.MultitargetProject):
    ...
    def _get_or_create_delegate(self, toolchain):
        ...
        if mx.get_os() == "windows":
            delegate = mx_native.DefaultNativeProject(
                self.suite,
                self.name,
                self.subDir,
                [],
            ...
            delegate._source = {
                "tree": ["include", "src", os.path.join("src", "x86")],
                "files": {
                    ".h": [
                        os.path.join("include", "ffi.h"),
            ...
            class LibtoolNativeProject(
                mx.NativeProject, mx_native.NativeDependency  # pylint: disable=too-many-ancestors
            ):
            ...
            class LibtoolNativeBuildTask(mx.NativeBuildTask):
                ...
                def _build_run_args(self):
                    ...
                    def _find_cc_var():
                        ...
                        with mx_native.NinjaManifestGenerator(
                            self.subject, cwd, filename, toolchain=self.toolchain
                        ) as gen:
                        ...
                        mx.run(
                            [mx_native.Ninja.binary, target, "-f", os.path.join(cwd, filename)], cwd=cwd, out=capture
                        )
                ...
                def verify_include_dirs(self):
                    ...
                    if len(self.subject.include_dirs) != len_ref:
                        mx.abort(
                            f"Number of include_dirs between delegates are not matching:\nlen({self.ref_include_dirs})\n!=\nlen({self.subject.include_dirs})"
                        )
            ...
            configure_args = ["--disable-dependency-tracking", "--disable-shared", "--with-pic"]
            ...
            if mx.get_os() == "darwin":
                ...
                assert configure_libc, "translation to configure style libc is not supported yet for" + str(
                    toolchain.spec.target.libc
                )
            ...
            configure_args += [f' CFLAGS="{cflags}"', 'CPPFLAGS="-DNO_JAVA_RAW_API"']
    ...
    def patches(self, toolchain):
        ...
        os_arch_libc_variant_dir = patch_dir(
            f"{mx.get_os()}-{mx.get_arch()}-{toolchain.spec.target.libc}-{toolchain.spec.target.variant}"
        )
...
mx_sdk_vm.register_graalvm_component(
    mx_sdk_vm.GraalVmTruffleLibrary(
        suite=_suite,
        name="Truffle API",
        short_name="tfla",
        dir_name="truffle",
...
mx_polybench.register_polybench_language(
    mx_suite=_suite, language="pmh", distributions=["PMH"], native_distributions=["PMH_BENCHMARK_NATIVE"]
)
...
def nfi_polybench_runner(polybench_run: mx_polybench.PolybenchRunFunction, tags) -> None:
    if "gate" in tags:
        polybench_run(
            ["--jvm", "nfi/*.pmh", "--experimental-options", "--engine.Compilation=false", "-w", "1", "-i", "1"]
        )
        polybench_run(
            ["--jvm", "nfi/panama/*.pmh", "--experimental-options", "--engine.Compilation=false", "-w", "1", "-i", "1"]
        )
        polybench_run(
            ["--native", "nfi/*.pmh", "--experimental-options", "--engine.Compilation=false", "-w", "1", "-i", "1"]
        )
    if "benchmark" in tags:
...
mx_polybench.register_polybench_benchmark_suite(
    ...
    name="nfi",
    languages=["pmh", "js"],
    benchmark_distribution="NFI_POLYBENCH_BENCHMARKS",
    benchmark_file_filter=".*pmh",
    runner=nfi_polybench_runner,
    tags={"gate", "benchmark"},
    suppress_validation_warnings=True,
)
