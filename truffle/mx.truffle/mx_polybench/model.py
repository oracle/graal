...
class PolybenchBenchmarkSuite(
    ...
    def _image_is_cached(self, bm_suite_args: List[str]) -> bool:
        ...
        if mx.get_env(PolybenchBenchmarkSuite.REUSE_DISK_IMAGES) in ["true", "True"]:
            image_path = self.get_image_path(
                self.get_final_image_name(self.get_base_image_name(), self.jvmConfig(bm_suite_args)),
                self.vmArgs(bm_suite_args),
            )
    ...
    def runAndReturnStdOut(self, benchmarks, bm_suite_args):
        ret_code, out, dims = super().runAndReturnStdOut(benchmarks, bm_suite_args)
        guest_vm, guest_vm_config = self._infer_guest_vm_info(benchmarks, bm_suite_args)
        dims.update(
            {
                "guest-vm": guest_vm,
