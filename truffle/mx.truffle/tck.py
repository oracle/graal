...
def execute_tck(graalvm_home, mode=Mode.default(), language_filter=None, values_filter=None, tests_filter=None, cp=None, truffle_cp=None, boot_cp=None, vm_args=None, debug_port=None):
    ...
    for jarPath in cp:
        if 'polyglot-tck.jar' in jarPath:
            additional_vm_arguments.append('--add-exports=org.graalvm.polyglot_tck/org.graalvm.polyglot.tck=ALL-UNNAMED')
            jarsToPatch.append(os.path.abspath(jarPath))
        if 'truffle-tck-common.jar' in jarPath:
            additional_vm_arguments.append('--add-exports=org.graalvm.polyglot_tck/com.oracle.truffle.tck.common.inline=ALL-UNNAMED')
            jarsToPatch.append(os.path.abspath(jarPath))
    if jarsToPatch:
        additional_vm_arguments.extend(['--patch-module', 'org.graalvm.polyglot_tck=' + ':'.join(jarsToPatch)])
