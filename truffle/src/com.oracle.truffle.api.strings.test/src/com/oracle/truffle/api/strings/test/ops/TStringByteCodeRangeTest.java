...
public class TStringByteCodeRangeTest extends TStringTestBase {

    @Parameter public TruffleString.GetByteCodeRangeNode node;

    @Parameters(name = "{0}")
    public static Iterable<TruffleString.GetByteCodeRangeNode> data() {
        return Arrays.asList(TruffleString.GetByteCodeRangeNode.create(), TruffleString.GetByteCodeRangeNode.getUncached());
    }

    @Test
    public void testAll() throws Exception {
        forAllStrings(true, (a, array, codeRange, isValid, encoding, codepoints, byteIndices) -> {
            Assert.assertEquals(byteCodeRange(codeRange, encoding), node.execute(a, encoding));
        });
    }
    ...
    private static TruffleString.CodeRange byteCodeRange(TruffleString.CodeRange codeRange, TruffleString.Encoding encoding) {
        switch (codeRange) {
            case ASCII:
                ...
                return TruffleString.CodeRange.ASCII;
            case LATIN_1:
            case BMP:
            case VALID:
                return TruffleString.CodeRange.VALID;
            case BROKEN:
                return TruffleString.CodeRange.BROKEN;
            default:
