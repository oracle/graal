...
public class InteropAssertionsTest extends InteropLibraryBaseTest {
    ...
    @Test
    public void testPublicMembersMustBeDeclared() {
        setupEnv(Context.create()); // we need no multi threaded context.
        MembersTest membersTest = new MembersTest();
        MembersTest metaTest = new MembersTest();
        InteropLibrary membersLib = createLibrary(InteropLibrary.class, membersTest);

        membersTest.hasMetaObject = true;
        membersTest.metaObject = () -> metaTest;
        metaTest.isMetaObject = true;
        metaTest.isMetaInstance = (o) -> false;
        metaTest.getMetaQualifiedName = () -> "std";
        metaTest.getMetaSimpleName = () -> "std";
        MembersTest memberElement1 = new MembersTest();
        memberElement1.isMember = true;
        memberElement1.memberSimpleName = () -> new TestStringWrapper("test1");
        memberElement1.memberQualifiedName = () -> new TestStringWrapper("test1");
        MembersTest memberElement2 = new MembersTest();
        memberElement2.isMember = true;
        memberElement2.memberSimpleName = () -> new TestStringWrapper("test2");
        memberElement2.memberQualifiedName = () -> new TestStringWrapper("test2");

        metaTest.hasDeclaredMembers = true;
        metaTest.declaredMembers = () -> new InteropDefaultsTest.Array(memberElement1);
        // No public members, but there are declared members:
        assertFails(() -> membersLib.getMemberObjects(membersTest), AssertionError.class);
        membersTest.hasMembers = true;
        membersTest.publicMembers = () -> new InteropDefaultsTest.Array(memberElement2);
        // Public members are not among the declared ones:
        assertFails(() -> membersLib.getMemberObjects(membersTest), AssertionError.class);
        membersTest.publicMembers = () -> new InteropDefaultsTest.Array(memberElement1, memberElement2);
        assertFails(() -> membersLib.getMemberObjects(membersTest), AssertionError.class);
    }
}
