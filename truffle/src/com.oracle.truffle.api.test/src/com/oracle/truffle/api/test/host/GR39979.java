...
public class GR39979 {

    public static class ApplyTest {
    ...
    @Test
    public void testPolyglotFunction() {
        try (Context context = Context.newBuilder().build()) {
            Value applyTest = context.asValue(new ApplyTest());
            Value expectSingleArgument = context.asValue(new ExpectSingleArgument());

            Value result = applyTest.invokeMember("call", expectSingleArgument);
            Assert.assertArrayEquals(new Object[]{null, 0}, result.as(Object[].class));
        }
    }

    @Test
    public void testPolyglotFunctionVarArgs() {
        try (Context context = Context.newBuilder().build()) {
            Value applyTest = context.asValue(new ApplyTest());
            Value expectZeroArguments = context.asValue(new ExpectZeroArguments());

            applyTest.invokeMember("callVarArgs", expectZeroArguments);
        }
    }
    ...
    @ExportLibrary(InteropLibrary.class)
    static final class ExpectZeroArguments implements TruffleObject {
        @SuppressWarnings("static-method")
        @ExportMessage
        boolean isExecutable() {
            return true;
