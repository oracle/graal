...
public class LegacyVarArgsTest extends ProxyLanguageEnvTest {
    ...
    public void testStringEllipsis() throws InteropException {
        TruffleObject mainClass = asTruffleHostSymbol(VarArgsTest.Join.class);
    ...
    public void testOverloadedVarArgsPrimitive() throws InteropException {
        TruffleObject paths = asTruffleHostSymbol(VarArgsTest.Sum.class);
        Object result;
    ...
    public void testGuestArray() throws InteropException {
        ...
        TruffleObject element1 = asTruffleObject("Hello");
        TruffleObject element2 = asTruffleObject("World");
        Object result = INTEROP.execute(ellipsis, new AsCollectionsTest.ListBasedTO(Arrays.asList(element1, element2)));
        Assert.assertEquals("Hello World", result);
    }

    @Test
    public void testGuestArray2() throws InteropException {
        TruffleObject sum = (TruffleObject) INTEROP.readMember(asTruffleHostSymbol(VarArgsTest.Sum.class), "sum");
        Object result = INTEROP.execute(sum, 10, new AsCollectionsTest.ListBasedTO(Arrays.asList(20, 30)));
        Assert.assertEquals("I[I", result);
    }

    @Test
    public void testGenericReturnType() throws InteropException {
        for (VarArgsTest.Container<?> c : new VarArgsTest.Container<?>[]{new VarArgsTest.GenericContainer<>(), new VarArgsTest.GenericContainer2<>()}) {
            ...
            result = INTEROP.invokeMember(container, "withPorts", new AsCollectionsTest.ListBasedTO(Arrays.asList(80)));
            Assert.assertEquals(container, result);
            result = INTEROP.invokeMember(container, "getPorts");
            Assert.assertEquals(Arrays.asList(80), asJavaObject(List.class, (TruffleObject) result));
    ...
    public void testPreferVarargsWithExactSignature() throws InteropException {
        ...
        Object container = asTruffleObject(new VarArgsTest.PreferVarargsWithExactSignature());
