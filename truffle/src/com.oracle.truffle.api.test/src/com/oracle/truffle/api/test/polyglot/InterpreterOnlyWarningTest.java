...
public class InterpreterOnlyWarningTest {
    ...
    public void testInterpreterOnlyWarning() {
        ...
        try {
            ...
            try (Context ctx = Context.newBuilder().logHandler(handler).build()) {
                Assert.assertEquals(isInterpreterOnlyWarningExpected(), hasInterpreterOnlyWarning(handler.getLog()));
            }
            handler.clear();
    ...
    private static boolean isInterpreterOnlyWarningExpected() {
        return !AbstractPolyglotTest.isGraalRuntime() && !TruffleTestAssumptions.isStrongEncapsulation();
    }
    ...
    private static final class TestHandler extends Handler {
        ...
        public void publish(final LogRecord record) {
            ...
            logRecords.add(Map.entry(level, message));
        }
