...
public class GR68545Test {
    ...
    public void testAlwaysDeoptNoInvalidate() {
        ...
        try (Context context = Context.newBuilder().allowExperimentalOptions(true).option("engine.CompilationFailureAction", "Throw").option("engine.BackgroundCompilation", "false").option(
                        "engine.CompileImmediately", "true").option("compiler.DeoptCycleDetectionThreshold", String.valueOf(DEOPT_CYCLE_DETECTION_THRESHOLD)).build()) {
            context.enter();
            CallTarget callTarget = (new AlwaysDeoptNoInvalidate()).getCallTarget();
            for (int i = 0; i < DEOPT_CYCLE_DETECTION_THRESHOLD; i++) {
                callTarget.call(i);
            }
            callTarget.call(DEOPT_CYCLE_DETECTION_THRESHOLD);
            TestAPIAccessor.runtimeAccess().invalidateCallTarget(callTarget, "Test reason");
