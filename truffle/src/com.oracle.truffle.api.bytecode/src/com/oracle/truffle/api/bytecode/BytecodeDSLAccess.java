...
public abstract sealed class BytecodeDSLAccess permits BytecodeDSLAccess.SafeImpl, BytecodeDSLAccess.UnsafeImpl {
    ...
    public static volatile BytecodeDSLAccess singleton;

    /**
     * Acessor methods for generated bytecode DSL interpreters. Do not use directly.
     */
    public static final BytecodeDSLAccess lookup(Object token) {
        if (token != BytecodeNodes.TOKEN) {
            throw new IllegalArgumentException("Invalid usage token. Seriously, you shouldn't use this.");
        }
        BytecodeDSLAccess impl = singleton;
        if (impl == null) {
            if (Boolean.getBoolean("truffle.dsl.DisableUnsafeBytecodeDSLAccess")) {
                impl = createSafe();
            } else {
                impl = createUnsafe();
            }
