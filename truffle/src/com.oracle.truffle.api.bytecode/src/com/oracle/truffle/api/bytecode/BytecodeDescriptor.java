 ...
 * {@code *RootNodeGen.Bytecode} class. It is recommended to not refer to the
 * {@link BytecodeDescriptor} directly and instead use the generated {@code *RootNodeGen.Bytecode}
 * class to avoid the rather verbose generic type. That field is a concrete subclass of
 * {@link BytecodeDescriptor}, and acts as the central entry point for:
 *
 * <ul>
 ...
 * {@link BytecodeRootNode} instances by driving a {@link BytecodeBuilder}. Tests create new roots
 * by calling {@code BYTECODE.create(...).getNode(0)} and then execute them via
 * {@link com.oracle.truffle.api.CallTarget CallTarget}.
 *
 ...
 * }</pre>
 *
 * @param <R> the generated root node type, which is both a {@link RootNode} and a
...
public abstract class BytecodeDescriptor<R extends RootNode & BytecodeRootNode, L extends TruffleLanguage<?>, B extends BytecodeBuilder> {
    ...
    /**
     * Returns the specification class for this descriptor. This is typically the user defined
    ...
    /**
     * Casts a {@link RootNode} to the generated root node type {@code R}. This also handles
     ...
     * previous call to {@link #serialize}.").newLine()
     *
     * @param language the language instance.
     ...
     * @since 25.1
     */
    protected final long updateConfig(TruffleLanguage<?> language, long config) {
        if (isDescriptorLookupEnabled(language)) {
            /*
             * Fast-path optimization. We want to avoid looking up the engine data if not needed.
             * Typically only needed for tracing and other debug features.
