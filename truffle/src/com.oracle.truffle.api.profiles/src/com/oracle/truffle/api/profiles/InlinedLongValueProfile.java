...
public final class InlinedLongValueProfile extends AbstractInlinedValueProfile {
    ...
    static {
        InlinedLongValueProfile profile = new InlinedLongValueProfile();
        DISABLED = profile;
    }

    private final LongField cachedValue;

    private InlinedLongValueProfile() {
        this.cachedValue = null;
    }

    private InlinedLongValueProfile(InlineTarget target) {
        super(target);
        this.cachedValue = target.getPrimitive(1, LongField.class);
    }
    ...
    public String toString(Node node) {
        ...
        return toString(LongValueProfile.class, isUninitialized(node), isGeneric(node), //
                        String.format("value == (long)%s", getCachedValue(node)));
    }
     ...
     * @since 23.0
     */
    public static InlinedLongValueProfile inline(
                    @RequiredField(value = StateField.class, bits = REQUIRED_STATE_BITS) //
                    @RequiredField(LongField.class) InlineTarget target) {
