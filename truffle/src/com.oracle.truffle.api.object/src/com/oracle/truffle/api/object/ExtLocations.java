...
abstract class ExtLocations {
    ...
    static final class ObjectLocation extends InstanceLocation {
        ...
        protected void clear(DynamicObject store) {
            if (!isFieldLocation()) {
                clearObjectArray(store);
            } else {
                clearObjectField(store);
            }
        }

        private void clearObjectArray(DynamicObject store) {
            UnsafeAccess.unsafePutObject(getObjectArray(store, false), getObjectArrayOffset(), null, this);
        }

        private void clearObjectField(DynamicObject store) {
            UnsafeAccess.unsafePutObject(store, getFieldOffset(), null, this);
        }
