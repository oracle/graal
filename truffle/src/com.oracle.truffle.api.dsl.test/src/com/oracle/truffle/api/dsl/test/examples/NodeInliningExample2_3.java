...
import com.oracle.truffle.api.dsl.test.examples.NodeInliningExample2_3Factory.SumArrayNodeGen;
...
public class NodeInliningExample2_3 {
    ...
    enum ArrayKind {
        ...
        RANGE(RangeArray.class),
        MATERIALIZED(MaterializedArray.class);

        final Class<? extends AbstractArray> type;

        ArrayKind(Class<? extends AbstractArray> type) {
            this.type = type;
        }

        static ArrayKind resolve(Object value) {
            for (ArrayKind kind : values()) {
    ...
    public void test() {
        ...
        // 20 bytes down from 120 bytes when everything is inlined and unrolled. success!
        assertEquals(20, ObjectSizeEstimate.forObject(sum, 1).getCompressedTotalBytes());
    }
}
