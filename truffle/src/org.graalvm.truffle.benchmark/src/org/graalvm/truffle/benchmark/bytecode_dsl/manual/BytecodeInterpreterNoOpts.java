        ...
        loop: while (true) {
            ...
            switch (opcode) {
                ...
                case Opcodes.OP_MULT: {
                    Object lhs = FRAMES.uncheckedGetObject(frame, sp - 2);
                    Object rhs = FRAMES.uncheckedGetObject(frame, sp - 1);
                    FRAMES.setObject(frame, sp - 2, ACCESS.uncheckedCast(ACCESS.readObject(localNodes, BYTES.getIntUnaligned(localBc, bci + 2)), MultNode.class).execute(lhs, rhs));
                    FRAMES.clear(frame, sp - 1);
                    sp -= 1;
                    bci += 6;
                    continue loop;
                ...
                case Opcodes.OP_DIV: {
                    Object lhs = FRAMES.uncheckedGetObject(frame, sp - 2);
                    Object rhs = FRAMES.uncheckedGetObject(frame, sp - 1);
                    FRAMES.setObject(frame, sp - 2, ACCESS.uncheckedCast(ACCESS.readObject(localNodes, BYTES.getIntUnaligned(localBc, bci + 2)), DivNode.class).execute(lhs, rhs));
                    FRAMES.clear(frame, sp - 1);
                    sp -= 1;
                    bci += 6;
                    continue loop;
                ...
                case Opcodes.OP_EQ: {
                    Object lhs = FRAMES.uncheckedGetObject(frame, sp - 2);
                    Object rhs = FRAMES.uncheckedGetObject(frame, sp - 1);
                    FRAMES.setObject(frame, sp - 2, ACCESS.uncheckedCast(ACCESS.readObject(localNodes, BYTES.getIntUnaligned(localBc, bci + 2)), EqNode.class).execute(lhs, rhs));
                    FRAMES.clear(frame, sp - 1);
                    sp -= 1;
                    bci += 6;
                    continue loop;
                ...
                case Opcodes.OP_ARRAY_LEN: {
                    Object o = FRAMES.uncheckedGetObject(frame, sp - 1);
                    FRAMES.setObject(frame, sp - 1, ACCESS.uncheckedCast(ACCESS.readObject(localNodes, BYTES.getIntUnaligned(localBc, bci + 2)), ArrayLengthNode.class).execute(o));
                    bci += 6;
                    continue loop;
                }
                // (o i -- i)
                case Opcodes.OP_ARRAY_INDEX: {
                    Object o = FRAMES.uncheckedGetObject(frame, sp - 2);
                    Object i = FRAMES.uncheckedGetObject(frame, sp - 1);
                    FRAMES.setObject(frame, sp - 2, ACCESS.uncheckedCast(ACCESS.readObject(localNodes, BYTES.getIntUnaligned(localBc, bci + 2)), ArrayIndexNode.class).execute(o, i));
                    FRAMES.clear(frame, sp - 1);
                    sp -= 1;
                    bci += 6;
                    continue loop;
