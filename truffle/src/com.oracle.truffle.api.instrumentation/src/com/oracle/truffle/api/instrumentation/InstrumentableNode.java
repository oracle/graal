 ...
 * interface are only allowed to be invoked if {@link #isInstrumentable()} returns
 * <code>true</code>.
 * <p>
 ...
 * automatically using an annotation processor by annotating the class
 * with @{@link GenerateWrapper}. If an instrumentable node subclass has additional declared execute
 * methods then a new wrapper must be generated or implemented. Otherwise the
 * {@link Node#replace(Node) replacement} of the instrumentable node with the wrapper will fail if
 * the subtype is used in a node {@link Child child}.
 * <p>
...
public interface InstrumentableNode extends NodeInterface {
     ...
     * {@link TruffleRuntime#createCallTarget(RootNode)}. The result is stable if the result of calling
     * this method remains always the same.
     *
     * @return
     */
    ...
    /**
     * Returns a new, never adopted, unshared {@link WrapperNode wrapper} node implementation for this
     * {@link InstrumentableNode instrumentable} node. The returned wrapper implementation must extend
     * the same type that implements {@link InstrumentableNode}.
     * <p>
     * The instrumentation framework will, when needed during execution, {@link Node#replace(Node)
     * replace} the instrumentable node with a {@link WrapperNode wrapper} and delegate to the original
     * node. After the replacement of an instrumentable node with a wrapper we refer to the original
     * node as an instrumented node. Wrappers can be generated automatically using an annotation
     * processor by annotating the class with @{@link GenerateWrapper}. Please note that if an
     * instrumetnable node subclass has additional execute methods then a new wrapper must be generated
     * or implemented. Otherwise the {@link Node#replace(Node) replacement} of the instrumentable node
     * with the wrapper will fail if the subtype is used as static type in nodes {@link Child children}.
     * <p>
     ...
     * onReturnExceptional(Frame,Throwable)}: an <em>execute</em> method on the delegate has just thrown
     * an exception.</li>
     * </ul>
     * </p>
     *
     ...
     * <p>
     * The node implementor may decide how to implement tagging for nodes. The simplest way to implement
     * tagging using Java types is by overriding the {@link #hasTag(Class)} method. This example shows
     * how to tag a node subclass and all its subclasses as statement:
     *
     * {@link com.oracle.truffle.api.instrumentation.InstrumentableNodeSnippets.StatementNode}
     *
     * <p>
     * Often it is impossible to just rely on the node's Java type to implement tagging. This example
     * shows how to use local state to implement tagging for a node.
     *
     ...
     * {@link RootNode root node} was wrapped in a {@link CallTarget} using
     * {@link TruffleRuntime#createCallTarget(RootNode)}. The result is stable if the result of calling
     * this method for a particular tag remains always the same.
     *
     * @param tag the class {@link com.oracle.truffle.api.instrumentation.ProvidedTags provided} by the
     *            {@link TruffleLanguage language}
    ...
    public interface WrapperNode extends NodeInterface, InstrumentableFactory.WrapperNode {
        ...
        /**
         * The {@link InstrumentableNode instrumentable} guest language node, adopted as a child, whose
         * execution events the wrapper reports to the instrumentation framework.
         *
         * @since 0.32
         */
