 ...
 * . Please note that if an instrumetnable node subclass has additional execute methods then a new
 * wrapper must be generated or implemented. Otherwise the {@link Node#replace(Node) replacement} of
 * the instrumentable node with the wrapper will fail if the subtype is used in a node {@link Child
 * child}.
 * <p>
