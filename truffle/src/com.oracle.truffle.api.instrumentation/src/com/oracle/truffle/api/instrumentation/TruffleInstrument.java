...
public abstract class TruffleInstrument {
    ...
    public static final class Env {
         ...
         * {@link ThreadLocalAction#ThreadLocalAction(boolean, boolean, boolean) continuous} then
         * the action will automatically be rescheduled in the same configuration until it is
         * {@link Future#cancel(boolean) cancelled}. For continuous actions, an invocation of
         * {@link Future#get()} will only wait for a single action to occur. {@link Future#isDone()}
         * will return <code>true</code> only if the action was canceled. Canceling a continuous
         * action will result in the current event being canceled and no further events being
         * submitted. Using continuous events should be preferred over submitting the event again
         * for the current thread while performing the thread-local action as continuous events are
         * also resubmitted in case all threads leave and later reenter.
         * <p>
