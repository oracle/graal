...
public abstract class InteropLibrary extends Library {
     ...
     * <p>
     * <b>Sample interpretations:</b> In Java static fields and methods would be provided by static
     * receiver, in Python class variables would be provided.
     * <p>
     ...
     * long as no side-effecting operations were performed.</li>
     * </ul>
     * All members that are returned from {@link #getMemberObjects(Object)} on an instance of this
     * meta-object need to be contained in the declared members of this meta-object or of some of
     * its {@link #getMetaParents(Object) parents}.
     * <p>
     * Static members are not returned unless called on a {@link #getStaticReceiver(Object) static
     * receiver}.
     *
    ...
    static class Asserts extends InteropLibrary {
        ...
        public Object readMember(Object receiver, Object member) throws UnsupportedMessageException, UnknownMemberException {
            ...
            assert isMultiThreaded(receiver) || assertMemberInReceiver(receiver, member);
        ...
        public Object getMemberObjects(Object receiver) throws UnsupportedMessageException {
            ...
            try {
                ...
                assert validInteropReturn(receiver, result);
                assert isMultiThreaded(receiver) || assertMembers(receiver, result, true);
        ...
        public Object getDeclaredMembers(Object receiver) throws UnsupportedMessageException {
            ...
            try {
                ...
                assert validInteropReturn(receiver, result);
                assert isMultiThreaded(receiver) || assertMembers(receiver, result, false);
                assert validInteropReturn(receiver, result);
                return result;
            } catch (InteropException e) {
                assert e instanceof UnsupportedMessageException : violationPost(receiver, e);
        ...
        private static boolean assertMembers(Object receiver, Object result, boolean compareWithDeclared) {
            assert result != null : violationPost(receiver, result);
            InteropLibrary uncached = InteropLibrary.getFactory().getUncached(result);
            assert uncached.hasArrayElements(result) : violationPost(receiver, result);
            long arraySize;
            ...
            for (long i = 0; i < arraySize; i++) {
                ...
                try {
                    memberNames.add(UNCACHED.asString(UNCACHED.getMemberQualifiedName(member)));
                } catch (UnsupportedMessageException e) {
                    assert false : violationPost(receiver, e);
                }
            }
            if (compareWithDeclared && UNCACHED.hasMetaObject(receiver)) {
                try {
                    Object meta = UNCACHED.getMetaObject(receiver);
                    if (UNCACHED.hasDeclaredMembers(meta)) {
                        Set<String> declaredMemberNames = new HashSet<>();
                        Deque<Object> metas = new ArrayDeque<>();
                        metas.add(meta);
                        while (!metas.isEmpty()) {
                            meta = metas.removeFirst();
                            Object declaredMembers = UNCACHED.getDeclaredMembers(meta);
                            assert uncached.hasArrayElements(declaredMembers) : violationPost(receiver, declaredMembers);
                            long numDeclared = UNCACHED.getArraySize(declaredMembers);
                            for (long i = 0; i < numDeclared; i++) {
                                Object member = UNCACHED.readArrayElement(declaredMembers, i);
                                declaredMemberNames.add(UNCACHED.asString(UNCACHED.getMemberQualifiedName(member)));
                            }
                            if (UNCACHED.hasMetaParents(meta)) {
                                Object metaParents = UNCACHED.getMetaParents(meta);
                                long numMetas = UNCACHED.getArraySize(metaParents);
                                for (long i = 0; i < numMetas; i++) {
                                    Object m = UNCACHED.readArrayElement(metaParents, i);
                                    metas.addLast(m);
                                }
                            }
                        }
                        memberNames.removeAll(declaredMemberNames);
                        assert memberNames.isEmpty() : violationPost(receiver, result) + " Members " + memberNames + " are not declared on meta-object.";
                    }
                } catch (UnsupportedMessageException | InvalidArrayIndexException e) {
                    assert false : violationPost(receiver, e);
                }
        ...
        private static boolean assertMemberInReceiver(Object receiver, Object member) {
            if (InteropLibrary.UNCACHED.isString(member)) {
                return true;
            }
            // assert that the member is among receiver's members
            try {
                String qualifiedName = UNCACHED.asString(UNCACHED.getMemberQualifiedName(member));
                Object meta;
                if (UNCACHED.hasMetaObject(receiver) && (meta = UNCACHED.getMetaObject(receiver)) != null && UNCACHED.hasDeclaredMembers(meta)) {
                    Deque<Object> metas = new ArrayDeque<>();
                    metas.add(meta);
                    while (!metas.isEmpty()) {
                        meta = metas.removeFirst();
                        ...
                        long numDeclared = UNCACHED.getArraySize(declaredMembers);
                        for (long i = 0; i < numDeclared; i++) {
                            Object m = UNCACHED.readArrayElement(declaredMembers, i);
                            String qn = UNCACHED.asString(UNCACHED.getMemberQualifiedName(m));
                ...
                if (UNCACHED.hasMembers(receiver)) {
                    Object members = UNCACHED.getMemberObjects(receiver);
                    long num = UNCACHED.getArraySize(members);
                    for (long i = 0; i < num; i++) {
                        Object m = UNCACHED.readArrayElement(members, i);
                        String qn = UNCACHED.asString(UNCACHED.getMemberQualifiedName(m));
                        if (qualifiedName.equals(qn)) {
                            return true;
                        }
                ...
                assert false : violationPost(member, e);
            }
            return false;
        }
