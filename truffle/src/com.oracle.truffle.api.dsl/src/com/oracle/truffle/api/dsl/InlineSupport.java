...
public final class InlineSupport {
    ...
    abstract static class UnsafeField {
        ...
        /**
         * Validates that we are allow to do a parent lookup for the inline context. Parent lookups
         * are only allowed for all instances of {@link SpecializationDataNode} generated by the
         * DSL. Any further partent lookups are invalid.
         *
         * An error here can only happen if an invalid inlining context was provided or due to a DSL
         * bug. Since this is quite an expensive check we only perform it if assertions are enabled.
         * If assertions are not enabled we would freely reso
         */
        @TruffleBoundary
