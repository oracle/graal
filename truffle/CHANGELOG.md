...
* GR-44217 (Breaking change) If your language or instrument was specified as Java module, any usages of `@ExportLibrary(useForAOT = true)` or `@GenerateLibrary(defaultExportLookupEnabled = true)`  need to register the generated implementation of the `com.oracle.truffle.api.provider.EagerExportProvider` or `com.oracle.truffle.api.provider.DefaultExportProvider` using the provides directive in the module descriptor. If your language did not use any of these features, no changes are necessary.
