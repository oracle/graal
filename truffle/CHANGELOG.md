...
* GR-73325: Extended `HostCompilerDirectives.BytecodeInterpreterHandlerConfig.Argument.Field.templateVariable()` to support per-field template variant counts for bytecode handler threading, and added `HostCompilerDirectives.BytecodeInterpreterThreadingExit` for cleanup when threaded dispatch returns to the switch loop.
