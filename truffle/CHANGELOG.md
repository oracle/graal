...
* GR-35093 Deprecated `AlwaysValidAssumption` and `UnionAssumption` without replacement. Deprecated `NeverValidAssumption`, use `Assumption.NEVER_VALID` instead. Language implementations should avoid custom `Assumption` subclasses, they lead to performance degradation when compiled with AOT compilation.
