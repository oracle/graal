...
To better illustrate how splitting works, let us first consider a small example
written in Simple Language:

```
function splitableFunction(arg1, arg2) {
    return arg1 + arg2;
}

function callsSplittable() {
    splitableFunction(1, 2);
    splitableFunction("foo", "bar");
}
...
As we can see in this example, the `splitableFunction` is called from
`callsSplittable` once with integer arguments and once with string arguments.
Once `splitableFunction` if executed enough times to be compiled its execution
profile will show that the `+` operator has been executed with both integers and
string and thus handlers (i.e. type checks and execution) for both types will be
compiled which has a performance impact. This can be avoided by rewriting the
example as follows:

```
function splitableFunctionInt(arg1, arg2) {
    return arg1 + arg2;
}

function splitableFunctionString(arg1, arg2) {
    return arg1 + arg2;

}
function callsSplittable() {
    splitableFunctionInt(1, 2);
    splitableFunctionString("foo", "bar");
}
...
In this example the `splitableFunction` has been duplicated in such a way that
each type profile is contained in a separate copy of the function
(`splitableFunctionInt` and `splitableFunctionString`). The result is that, come
compilation time, only a single type profile is available for each function
avoiding potentially costly type checks in the compiled code.

Automating the detection suitable candidates as well as their duplication and
performing both tasks at run time is what we call splitting. It is, in other
words, automated run-time monomorphization of polymorphic nodes through AST
duplication.
