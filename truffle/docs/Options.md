...
- `--engine.PropagateLoopCountsToLexicalSingleCaller=[0, inf)` : How high to propagate call and loop count (hotness proxy) up a single caller chain to lexical scope parent. Allows certain lambda callers to get compiled sooner.
