...
*tl;dr* Truffle Libraries allow Truffle languages to use polymorphic dispatch for receiver types with support for implementation specific caching/profiling and customization of the dispatch. Truffle Libraries enable modularity and encapsulation for representation types in Truffle language implementations. Use with care and read the documentation first.
...
@NodeChild @NodeChild
abstract class ArrayReadNode extends LanguageNode {
...
```java
@NodeChild @NodeChild
class ReadArrayNode extends LanguageNode {
    /*...*/
...
@NodeChild @NodeChild
class ArrayReadNode extends LanguageNode {
    @Specialization 
...
_When not to use it?_

* For basic trivial types that only have one representation.
...
class ArrayAssertions extends ArrayLibrary {
