...
public final class InstrumentationRegistrationProcessor extends AbstractProcessor {
    ...
    private void generateFile(List<TypeElement> instrumentations) {
        String filename = "META-INF/truffle/instrumentation";
        Properties p = new Properties();

        int cnt = 0;
        for (TypeElement l : instrumentations) {
            ...
            p.setProperty(prefix + "autostart", Boolean.valueOf(annotation.autostart()).toString());
        ...
        if (cnt > 0) {
            try {
                ...
                try (OutputStream os = file.openOutputStream()) {
                    p.store(os, "Generated by " + InstrumentationRegistrationProcessor.class.getName());
                }
            } catch (IOException e) {
                processingEnv.getMessager().printMessage(Kind.ERROR, e.getMessage(), instrumentations.get(0));
            }
        }
    }

    static void loadExistingTypes(ProcessingEnvironment env, List<TypeElement> instrumentations, String filename, String pre) {
        Set<String> typeNames = new HashSet<>();
        for (TypeElement type : instrumentations) {
        ...
        for (int cnt = 1;; cnt++) {
            ...
            if (foundType != null && !typeNames.contains(ElementUtils.getQualifiedName(foundType))) {
                instrumentations.add(foundType);
            }
        }
    }
        ...
        for (Element e : roundEnv.getElementsAnnotatedWith(Registration.class)) {
            ...
            if (annotation != null && e.getKind() == ElementKind.CLASS) {
                if (!e.getModifiers().contains(Modifier.PUBLIC)) {
                    emitError("Registered instrumentation class must be public", e);
                    continue;
                }
                if (e.getEnclosingElement().getKind() != ElementKind.PACKAGE && !e.getModifiers().contains(Modifier.STATIC)) {
                    emitError("Registered instrumentation inner-class must be static", e);
                    continue;
                }
                ...
                if (!processingEnv.getTypeUtils().isAssignable(e.asType(), truffleLang)) {
                    emitError("Registered instrumentation class must subclass Instrumentation", e);
                    continue;
                }
