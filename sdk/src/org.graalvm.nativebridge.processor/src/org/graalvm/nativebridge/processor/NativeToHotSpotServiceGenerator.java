...
public class NativeToHotSpotServiceGenerator extends AbstractNativeServiceGenerator {

    private static final String END_POINT_SIMPLE_NAME = "NativeToHSEndPoint";
    ...
    NativeToHotSpotServiceGenerator(NativeToHotSpotServiceParser parser, TypeCache typeCache, ServiceDefinitionData definitionData) {
    ...
    void generateCommonCustomDispatchFactoryReturn(CodeBuilder builder) {
        CharSequence isolateThreadVar = "isolateThread";
        CodeBuilder hsIsolate = new CodeBuilder(builder).invokeStatic(typeCache.hsIsolate, "get");
        builder.lineStart().write(typeCache.hsIsolateThread).space().write(isolateThreadVar).write(" = ").invoke(hsIsolate.build(), "enter").lineEnd(";");
        builder.line("try {");
        builder.indent();
        CodeBuilder jniEnv = new CodeBuilder(builder).invoke(isolateThreadVar, "getJNIEnv");
        builder.lineStart("return ").newInstance(factoryMethod.startPointSimpleName, jniEnv.build()).lineEnd(";");
        builder.dedent();
    ...
    private void generateNativeToHSStartPoint(CodeBuilder builder, FactoryMethodInfo factoryMethodInfo) {
        ...
        if (!getDefinition().toGenerate.isEmpty()) {
            ...
            Parameter foundJNIEnv = findParameterOfType(typeCache.jniEnv, factoryMethodInfo.constructorParameters);
    ...
    private void generateJNIFieldsInit(CodeBuilder builder, CodeBuilder.Parameter jniEnv) {
        ...
        for (MethodData methodData : getDefinition().toGenerate) {
            ...
            List<? extends TypeMirror> signature = getParser().endPointMethodProvider.getEndPointSignature(methodData, getDefinition().serviceType, getDefinition().hasCustomDispatch());
            ...
            TypeMirror returnType = hasOutMarshalledParameter ? types.getArrayType(types.getPrimitiveType(TypeKind.BYTE))
                            : getParser().endPointMethodProvider.getEndPointMethodParameterType(methodData.getReturnTypeMarshaller(), methodData.type.getReturnType());
            builder.lineStart("this." + fieldName).write(" = ").invokeStatic(typeCache.jNIMethod, "findMethod", jniEnv.name, END_POINT_CLASS_FIELD, "true",
                            ...
                            "\"" + Utilities.encodeMethodSignature(getParser().elements, types, returnType, signature.toArray(new TypeMirror[0])) + "\"").lineEnd(";");
        }
    }
    ...
    private void generateNativeToHSStartMethod(CodeBuilder builder, CacheSnippet cacheSnippets, MethodData methodData) {
        ...
        AbstractServiceParser.CacheData cacheData = methodData.cachedData;
        CharSequence args;
        CharSequence env;
        ...
        if (cacheData != null) {
            ...
            builder.write(resultMarshallerSnippets.unmarshallResult(builder, returnType, preUnmarshallResult.result, isolateVar, binaryUnmarshallVars != null ? binaryUnmarshallVars.getValue() : null,
                            env));
            builder.lineEnd(";");
            if (hasPostMarshall) {
                binaryUnmarshallVars = generatePostMarshall(builder, methodData, nonReceiverParameterStart, binaryMarshallVars, binaryUnmarshallVars, env, resultVariable,
                                staticMarshallBufferVar, staticBufferSize, hasOutParameters, isolateVar);
            }
            ...
            if (firstOutMarshalledParameter != null) {
                ...
                if (!voidMethod) {
                    ...
                    resultMarshallerSnippets.read(builder, returnType, resultVariable, isolateVar, binaryUnmarshallVars.getValue(), env);
                }
            } else {
                CharSequence jniCall = callHotSpot(builder, methodData, env, args, resultMarshallerData);
                PreUnmarshallResult preUnmarshallResult = generatePreUnmarshallResult(builder, resultMarshallerSnippets, binaryMarshallVars, returnType, jniCall, env,
                ...
                builder.write(resultMarshallerSnippets.unmarshallResult(builder, returnType, preUnmarshallResult.result, isolateVar,
            ...
            if (hasPostMarshall) {
                binaryUnmarshallVars = generatePostMarshall(builder, methodData, nonReceiverParameterStart, binaryMarshallVars, binaryUnmarshallVars,
                                env, resultVariable, staticMarshallBufferVar, staticBufferSize, hasOutParameters, isolateVar);
            }
            if (resultVariable != null) {
    ...
    private PreUnmarshallResult generatePreUnmarshallResult(CodeBuilder builder, MarshallerSnippet snippets, BinaryMarshallVariables binaryMarshallVars, TypeMirror returnType,
        ...
        CharSequence resultVariable = snippets.preUnmarshallResult(builder, returnType, result, isolateVar, input, jniEnvFieldName);
    ...
    private Map.Entry<CharSequence, CharSequence> generatePostMarshall(CodeBuilder builder, MethodData methodData, int nonReceiverParameterStart,
                    ...
                    CharSequence resultVariable, CharSequence staticMarshallBufferVar, int staticBufferSize, boolean hasOutParameters, CharSequence isolateVar) {
        ...
        for (int i = nonReceiverParameterStart; i < formalParameters.size(); i++) {
            ...
            marshallerSnippets(marshaller).postMarshallParameter(builder, formalParameterTypes.get(i),
                            formalParameters.get(i).getSimpleName(), isolateVar, res != null ? res.getValue() : null, jniEnv, resultVariable);
        }
        return res;
    }
    ...
    private CharSequence generatePushArgs(CodeBuilder builder, MethodData methodData, int nonReceiverParameterStart,
        ...
        boolean hasExplicitReceiver = getDefinition().hasCustomDispatch();
        ...
        for (int i = nonReceiverParameterStart; i < formalParameters.size(); i++) {
            ...
            builder.lineStart().invoke(address, jValueSetterName(getParser().endPointMethodProvider.getEndPointMethodParameterType(marshaller, formalParameterTypes.get(i))), value).lineEnd(";");
        }
    ...
    private CharSequence callHotSpot(CodeBuilder builder, MethodData methodData, CharSequence jniEnv, CharSequence args,
        ...
        return new CodeBuilder(builder).invoke(hsCallsInstance, callHotSpotName(getParser().endPointMethodProvider.getEndPointMethodParameterType(marshaller, retType)),
    ...
    private void generateNativeToHSEndMethod(CodeBuilder builder, MethodData methodData) {
        ...
        if (!getDefinition().hasCustomDispatch()) {
            params.add(CodeBuilder.newParameter(getDefinition().serviceType, "receiverObject"));
        }
        boolean needsIsolate = needsExplicitIsolateParameter(methodData);
        if (needsIsolate) {
        ...
        for (int i = 0; i < methodParameters.size(); i++) {
            ...
            if (isBinaryMarshallable(marshallerData, methodParameterTypes.get(i), false)) {
                ...
                params.add(CodeBuilder.newParameter(getParser().endPointMethodProvider.getEndPointMethodParameterType(marshallerData, parameterType), parameterName));
            }
            if (isOutParameter(marshallerData, parameterType, false)) {
        ...
        TypeMirror endMethodReturnType = firstOutMarshalledParameter != null
                        ...
                        : getParser().endPointMethodProvider.getEndPointMethodParameterType(returnTypeMarshaller, receiverMethodReturnType);
        ...
        if (needsIsolate) {
            builder.lineStart().write(typeCache.nativeIsolate).space().write(nativeIsolateVar).write(" = ").invokeStatic(typeCache.nativeIsolate, "get", REFERENCE_ISOLATE_ADDRESS_NAME).lineEnd(";");
        }
        ...
        for (int i = nonReceiverParameterStart; i < methodParameters.size(); i++) {
            marshallerSnippets(methodData.getParameterMarshaller(i)).preUnmarshallParameter(builder, methodParameterTypes.get(i), methodParameters.get(i).getSimpleName(),
                            nativeIsolateVar, marshalledParametersInputVar, params.get(0).name, parameterValueOverrides);
        }
        ...
        for (int i = nonReceiverParameterStart; i < methodParameters.size(); i++) {
            ...
            if (parameterValueOverride != null) {
                ...
                actualParameters[i] = marshallerSnippets(marshallerData).unmarshallParameter(builder, parameterType, parameterName,
                                nativeIsolateVar, marshalledParametersInputVar, null);
            }
        }
        ...
        builder.indent();
        builder.lineStart("throw ").invokeStatic(typeCache.foreignException, "forThrowable", e, getDefinition().getCustomMarshaller(typeCache.throwable, null, types).name).lineEnd(";");
        builder.dedent();
        builder.line("}");
        builder.dedent();
        builder.line("}");
    ...
    private abstract static class NativeToHotSpotMarshallerSnippet extends MarshallerSnippet {
        ...
        NativeToHotSpotMarshallerSnippet(NativeToHotSpotServiceGenerator generator, MarshallerData marshallerData) {
            super(generator, marshallerData);
            this.cache = generator.typeCache;
        }

        static NativeToHotSpotMarshallerSnippet forData(NativeToHotSpotServiceGenerator generator, MarshallerData marshallerData) {
        ...
        private static final class DirectSnippet extends NativeToHotSpotMarshallerSnippet {
            ...
            DirectSnippet(NativeToHotSpotServiceGenerator generator, MarshallerData marshallerData) {
                super(generator, marshallerData);
            }
            ...
            @Override
            void postMarshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence parameterName,
                            CharSequence isolateVar, CharSequence marshalledParametersInput, CharSequence jniEnvFieldName, CharSequence resultVariableName) {
        ...
        private static final class ReferenceSnippet extends NativeToHotSpotMarshallerSnippet {
            ...
            ReferenceSnippet(NativeToHotSpotServiceGenerator generator, MarshallerData marshallerData) {
                super(generator, marshallerData);
            ...
            boolean preMarshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence parameterName, CharSequence marshalledParametersOutput, CharSequence jniEnvFieldName,
                ...
                if (parameterType.getKind() == TypeKind.ARRAY) {
                    ...
                    WriteArrayPolicy needsCopy = marshallerData.out == null || marshallerData.in != null ? WriteArrayPolicy.STORE : WriteArrayPolicy.RESERVE;
                    if (marshallerData.sameDirection) {
                        CharSequence targetArrayVariable = Utilities.javaMemberName("hs", parameterName);
                        if (needsCopy == WriteArrayPolicy.STORE) {
                            generateCopyHSPeerArrayToHotSpot(currentBuilder, targetArrayVariable, componentType, parameterName, arrayOffsetParameter, arrayLength, jniEnvFieldName);
                        } else {
                        ...
                        currentBuilder.indent();
                        generateWriteReferenceArray(currentBuilder, parameterName, needsCopy, true, arrayOffsetParameter, arrayLength, marshalledParametersOutput,
            ...
            void postMarshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence parameterName,
                ...
                if (parameterType.getKind() == TypeKind.ARRAY) {
                    if (marshallerData.out != null) {
                        ...
                        CharSequence arrayOffsetParameter = marshallerData.out.offsetParameter;
                        if (marshallerData.sameDirection) {
                            generateCopyHotSpotToHSPeerArray(currentBuilder, parameterName, arrayOffsetParameter, arrayLength, sourceArrayVariable, null, jniEnvFieldName,
                                            (element) -> unmarshallResult(currentBuilder, componentType, element, isolateVar, null, jniEnvFieldName));
                        } else {
                            generateReadReferenceArray(currentBuilder, parameterName, arrayOffsetParameter, arrayLength, marshalledParametersInput,
                                            (element) -> unmarshallResult(currentBuilder, componentType, element, isolateVar, marshalledParametersInput, jniEnvFieldName));
                        }
                        currentBuilder.dedent();
                        currentBuilder.line("}");
                    }
            ...
            CharSequence preUnmarshallResult(CodeBuilder currentBuilder, TypeMirror resultType, CharSequence invocationSnippet,
                ...
                if (resultType.getKind() == TypeKind.ARRAY) {
                    ...
                    if (marshallerData.sameDirection) {
                        ...
                        generateCopyHotSpotToHSPeerArray(currentBuilder, resultVar, null, lengthSnippet, invocationSnippet, null, jniEnvFieldName,
                                        (element) -> unmarshallResult(currentBuilder, componentType, element, isolateVar, null, jniEnvFieldName));
                    } else {
                        ...
                        currentBuilder.lineStart().arrayElement(resultVar, index).write(" = ").write(
                                        unmarshallResult(currentBuilder, componentType, element, isolateVar, null, jniEnvFieldName)).lineEnd(
                                                        ";");
                        currentBuilder.dedent();
                        currentBuilder.line("}");
                    }
            ...
            boolean preUnmarshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence parameterName, CharSequence isolateVar, CharSequence marshalledParametersInput,
                ...
                if (parameterType.getKind() == TypeKind.ARRAY && !marshallerData.sameDirection) {
                    ...
                    if (marshallerData.out == null || marshallerData.in != null) {
                        ...
                        generateReadReferenceArray(currentBuilder, parameterName, null, arrayLengthVariable, marshalledParametersInput,
                                        (element) -> unmarshallParameter(currentBuilder, componentType, element, isolateVar, marshalledParametersInput, jniEnvFieldName));
                    }
                    currentBuilder.dedent();
                    currentBuilder.line("} else {");
                    currentBuilder.indent();
            ...
            CharSequence unmarshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence parameterName, CharSequence isolateVar, CharSequence marshalledParametersInput,
                ...
                if (marshallerData.sameDirection) {
                    ...
                    return unmarshallHotSpotToNativeProxyInHotSpot(currentBuilder, parameterName, isolateVar);
                }
            }
            ...
            void postUnmarshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence parameterName, CharSequence marshalledResultOutput, CharSequence jniEnvFieldName,
                ...
                if (!marshallerData.sameDirection && parameterType.getKind() == TypeKind.ARRAY) {
                    if (marshallerData.out != null) {
                        ...
                        currentBuilder.indent();
                        TypeMirror componentType = ((ArrayType) parameterType).getComponentType();
                        generateWriteReferenceArray(currentBuilder, parameterName, WriteArrayPolicy.STORE, false, null, arrayLengthSnippet, marshalledResultOutput,
        ...
        private static final class RawReferenceSnippet extends NativeToHotSpotMarshallerSnippet {
            ...
            CharSequence marshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence formalParameter, CharSequence marshalledParametersOutput, CharSequence jniEnvFieldName) {
                if (marshallerData.sameDirection) {
                    CharSequence value = new CodeBuilder(currentBuilder).cast(cache.hSPeer, formalParameter, true).build();
                    return new CodeBuilder(currentBuilder).invoke(value, "getJObject").build();
                } else {
                    return new CodeBuilder(currentBuilder).invokeStatic(cache.referenceHandles, "create", formalParameter).build();
                }
            }
            ...
            CharSequence unmarshallParameter(CodeBuilder currentBuilder, TypeMirror parameterType, CharSequence parameterName, CharSequence isolateVar, CharSequence marshalledParametersInput,
                ...
                if (marshallerData.sameDirection) {
                    ...
                    return new CodeBuilder(currentBuilder).invokeStatic(cache.peer, "create", isolateVar, parameterName).build();
                }
            }
            ...
            CharSequence marshallResult(CodeBuilder currentBuilder, TypeMirror resultType, CharSequence invocationSnippet, CharSequence marshalledResultOutput, CharSequence jniEnvFieldName) {
                if (marshallerData.sameDirection) {
                    ...
                    CharSequence value = new CodeBuilder(currentBuilder).cast(cache.nativePeer, invocationSnippet, true).build();
            ...
            CharSequence unmarshallResult(CodeBuilder currentBuilder, TypeMirror resultType, CharSequence invocationSnippet,
                            CharSequence isolateVar, CharSequence marshalledResultInput, CharSequence jniEnvFieldName) {
                if (marshallerData.sameDirection) {
                    return new CodeBuilder(currentBuilder).invokeStatic(cache.peer, "create", jniEnvFieldName, invocationSnippet).build();
                } else {
                    CharSequence typeLiteral = new CodeBuilder(currentBuilder).classLiteral(resultType).build();
                    return new CodeBuilder(currentBuilder).invokeStatic(cache.referenceHandles, "resolve", invocationSnippet, typeLiteral).build();
                }
            }
        ...
        private static final class CustomSnippet extends NativeToHotSpotMarshallerSnippet {
            ...
            CustomSnippet(NativeToHotSpotServiceGenerator generator, MarshallerData marshallerData) {
                super(generator, marshallerData);
            }
