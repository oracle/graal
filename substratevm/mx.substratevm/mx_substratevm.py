...
def _layeredhelloworld(native_image, javac_command, output_path, build_only, args):
    ...
    # Keep Hello.class in a separate directory so build outputs (libbase.dll,
    # base-layer.nil, etc.) don't change the classpath directory hash between
    # base and extension layer builds.
    classes_path = join(output_path, 'classes')
    mx_util.ensure_dir_exists(classes_path)

    # Write and compile Hello.java
    hello_file = join(classes_path, 'Hello.java')
    with open(hello_file, 'w') as fp:
    ...
    cp_args = ['-cp', classes_path]

    def run_build(ni_args, output_check_file):
        """Run native-image, tolerating Windows temp dir cleanup failures.

        On Windows, native-image may exit with code 1 due to a temp directory
        cleanup failure (AccessDeniedException on layer-snapshot.lsb) even when
        the build itself succeeds. Tolerate this by checking the output file.
        """
        if mx.is_windows():
            exit_code = native_image(ni_args, nonZeroIsFatal=False)
            if exit_code != 0 and not os.path.exists(output_check_file):
                mx.abort('native-image build failed with exit code ' + str(exit_code))
        else:
            native_image(ni_args)

    # Build both layers into output_path so the extension layer exe can find
    # the base layer shared library (Windows DLL search looks in the exe's
    # directory; on Linux/macOS rpath handles this).
    base_layer_name = 'libbase'
    run_build(args + cp_args + [
        '-o', join(output_path, base_layer_name),
    ] + svm_experimental_options([
        f'-H:LayerCreate={base_layer_name}.nil,module=java.base',
    ]), join(output_path, base_layer_name + '.nil'))
    ...
    ]), join(output_path, app_name + ('.exe' if mx.is_windows() else '')))
    ...
    if not build_only:
        ...
        binary_path = join(output_path, app_name)
        if mx.is_linux():
            # On Linux, set LD_LIBRARY_PATH so the loader finds the base layer .so
            env = os.environ.copy()
            env['LD_LIBRARY_PATH'] = output_path + ':' + env.get('LD_LIBRARY_PATH', '')
            mx.run([binary_path], out=_collector, env=env)
        else:
