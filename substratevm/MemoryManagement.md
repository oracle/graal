...
Java objects that a native image allocates at run-time reside in an area called the Java heap.
...
* The **Serial GC** is the default GC on both GraalVM Community and Enterprise Edition.
...
If the Java heap is full and the GC is unable reclaim sufficient memory for a Java object allocation, the allocation will fail with an `OutOfMemoryError`.
...
In its core, the serial GC is a simple (non-parallel, non-concurrent) stop & copy GC.
...
By default, the GC tries to balance the time that is spent in young collections and full collections.
...
Please note that this is just the maximum value.
...
* `-H:AlignedHeapChunkSize` - the size of a heap chunk in bytes. Can only be specified at image build time.
* `-XX:MaximumHeapSizePercent` - percentage of the physical memory size that is used as the maximum Java heap size if the maximum Java heap size is not specified otherwise.
...
However, please note that there is no absolute certainty for a given pause.
...
However, it can be tuned to the performance needs of the particular application.
Here is a small subset of the options that can be specified when doing performance tuning:

* `-H:G1HeapRegionSize` - the size of a G1 region. Can only be specified at image build time.
* `-XX:MaxRAMPercentage` - percentage of the physical memory size that is used as the maximum heap size if the maximum heap size is not specified otherwise.
...
Further image builder options can be listed using `native-image --expert-options-all`.
...
* `-H:±UseCompressedReferences` - determines if 32-bit instead of 64-bit references to Java objects are used. Can only be specified at image build time.

### Native Memory

A Native Image may also allocate memory that is separate from the Java heap.
