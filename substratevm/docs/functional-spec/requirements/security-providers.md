# REQ-002-security-providers: Requirements for JCA Security Provider Support

[§FS-002-security-providers](../security-providers.md) specifies what Native Image does with Java
...
[§FS-002-security-providers.7](../security-providers.md#7-transition-to-the-future-defaults) are the
...
**Domain.** The successful construction-path shapes admitted by
[§FS-002-security-providers.1.1](../security-providers.md#11-registered-providers-and-services) and
[§FS-002-security-providers.2.2](../security-providers.md#22-provider-construction): a public provider
...
class; a provider method that returns a non-public implementation class; and a provider class that
the configured list names more than once.

A provider whose run-time name differs from the configured name is a negative run-time-list shape,
not a successful construction-parity shape. It is covered by the actionable failure required by
[§FS-002-security-providers.7.1](../security-providers.md#71-run-time-provider-list-initialization).
...
[§FS-002-security-providers.1.1](../security-providers.md#11-registered-providers-and-services) without
...
metadata signals of
[§FS-002-security-providers.2.1](../security-providers.md#21-qualifying-reflection-metadata) and the
platform-owned signal of
[§FS-002-security-providers.2.4](../security-providers.md#24-securerandom-providers).
...
format ([§FS-002-security-providers.4.3](../security-providers.md#43-missing-reflection-registration)
and [§FS-002-security-providers.6.1](../security-providers.md#61-provider-and-service-coverage)).
...
([§FS-002-security-providers.4.3](../security-providers.md#43-missing-reflection-registration)).
...
at build time
([§FS-002-security-providers.4.1](../security-providers.md#41-unregistered-providers) and
...
([§FS-002-security-providers.1.3](../security-providers.md#13-run-time-provider-list)).
...
function of the configured security properties and the registration signals alone.
Two builds of the same application that differ only in code that is reachable but performs no
provider access and introduces no platform-owned registration signal must produce the same run-time
provider list, the same lookup results, and the same service availability.

**Domain.** Pairs of builds that differ by one reachable but unexecuted JCA factory call or
security-service facade, excluding the `SecureRandom` acquisition paths that are explicitly the
platform-owned conditional registration signal of
[§FS-002-security-providers.2.4](../security-providers.md#24-securerandom-providers).
...
The conditional `SecureRandom` signal is not a deviation: it is one of the registration inputs over
which this requirement is defined.

## 7. Transition Compatibility

**Requirement.** Every supported option combination of
[§FS-002-security-providers.7](../security-providers.md#7-transition-to-the-future-defaults) must be a
...
[§FS-002-security-providers.1](../security-providers.md#1-provider-reflection-registration) through
[§FS-002-security-providers.6](../security-providers.md#6-tracing-metadata) that no option combination
...
([§FS-002-security-providers.6](../security-providers.md#6-tracing-metadata)).

**Domain.** Traced runs of the programs that cover
[§FS-002-security-providers.6.1](../security-providers.md#61-provider-and-service-coverage): provider
...
[§FS-002-security-providers.6.2](../security-providers.md#62-observational-transparency).
...
[§AR-002-security-providers](../../architecture/security-providers.md) must discharge them
...
[§FS-002-security-providers.2.1](../security-providers.md#21-qualifying-reflection-metadata) states that
