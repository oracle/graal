...
This document describes the software architecture of the GraalVM Native Image project from the current
`substratevm` suite.

The main focus of this document is build-time architecture: how the `native-image` driver starts the
hosted builder, how the builder derives a closed-world model, and how it produces a native
executable, shared library, static executable, or image layer. It mentions runtime components where
the build pipeline constructs, configures, or embeds them, but it does not try to describe the
generated image as a running VM. Runtime behavior such as VM startup, isolates, threads, safepoints,
the image heap, GC, JNI/JVMTI/JFR, and low-level OS integration belongs in
...
- Runtime behavior of the generated image; see
  [RUNTIME.md](RUNTIME.md)
