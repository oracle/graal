...
public class ForeignFunctionsConfigurationParser extends ConfigurationParser {
    ...
    private void parseAndRegisterForeignCall(Object call, boolean forUpcall) {
        ...
        try {
            ...
            throw UserError.abort(e, "Could not register foreign stub '%s'", JsonFormatter.formatJson(map));
        }
    }
    ...
    private void parseAndRegisterDirectUpcall(Object call) {
        ...
        try {
            ...
            throw UserError.abort(e, "Cannot find class '%s' used to register method(s) '%s' for a direct upcall(s). ", className, methodName);
        }
        ...
        if (returnTypeInput != null || parameterTypesInput != null) {
            ...
            try {
                methodHandles = List.of(getImplLookup().findStatic(aClass, methodName, methodType));
            ...
            try {
                List<Method> methods = findStaticMethods(aClass, methodName);
                descriptors = methods.stream().map(ForeignFunctionsConfigurationParser::deriveFunctionDescriptor).toList();
                methodHandles = methods.stream().map(this::unreflect).toList();
            } catch (NoSuchMethodException e) {
                throw UserError.abort(e, "Method '%s.%s' could not be registered as an upcall target method. " +
                                "Please verify that the method is static and that the parameter types match.",
                                className, methodName);
            }
        }
        VMError.guarantee(descriptors.size() == methodHandles.size());

        for (int i = 0; i < methodHandles.size(); i++) {
            FunctionDescriptor descriptor = descriptors.get(i);
            MethodHandle methodHandle = methodHandles.get(i);
            var optionsMap = asMap(map.get("options", EconomicMap.emptyMap()), "options must be a map");
            try {
                var options = parseUpcallOptions(optionsMap);
    ...
    private MethodHandle unreflect(Method method) {
        try {
            ...
            throw UserError.abort(e, "Method '%s' could not be registered as an upcall target method. " +
                            "Please verify that the method is public, static and that the parameter types match.",
                            method);
        }
    }

    private static FunctionDescriptor deriveFunctionDescriptor(Method method) {
    ...
    private static MemoryLayout deriveMemoryLayout(Method method, String parameterName, Class<?> type) {
