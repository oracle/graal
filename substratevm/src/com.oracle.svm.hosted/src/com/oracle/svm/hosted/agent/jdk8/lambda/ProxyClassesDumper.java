 ...
 * @implNote
 *           <p>
 *           Because this class is called by LambdaMetafactory, make use of lambda lead to recursive
 *           calls cause stack overflow.
 */
final class ProxyClassesDumper {
    ...
    public static ProxyClassesDumper getInstance(String path) {
        ...
        try {
            path2 = path2.trim();
            final Path dir = Paths.get(path2.length() == 0 ? "." : path2);
            ...
            PlatformLogger.getLogger(ProxyClassesDumper.class.getName())
                            .warning("Path " + path2 + " is not valid - dumping disabled", ex);
