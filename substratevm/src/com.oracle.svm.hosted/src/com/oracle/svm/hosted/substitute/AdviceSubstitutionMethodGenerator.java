...
public class AdviceSubstitutionMethodGenerator {
                        ...
                        if (parameters[i].getAnnotation(Advice.Rewrite.class) != null) {
                            guarantee(i < originalParameters.length && originalParameters[i].getType().equals(parameters[i].getType()),
                                    "The %sth parameter of @Before method %s#%s doesn't match with its corresponding original parameter." +
                                            "It is supposed to be of %s type, but is %s", i,
                            ...
                            try {
                                ...
                                mv.visitVarInsn(ALOAD, beforeReturnSlot); // load result returned by @Before method
                                ...
                                // rewrite parameter is null, meaning it didn't rewrite, use the original parameter
    ...
    private int lookUpParameterIndex(Executable method, Class<? extends Annotation> annotation) {
    ...
    private Map<Integer, Integer> fillMethodParameterMap(Executable method) {
        Map<Integer, Integer> paramLocalVarMap = new HashMap<>();
        fillMethodParameterMap(method, paramLocalVarMap);
        return paramLocalVarMap;
    }

    private void fillMethodParameterMap(Executable method, Map<Integer, Integer> paramLocalVarMap) {
        for (int i = 0; i < method.getParameters().length; i++) {
            paramLocalVarMap.putIfAbsent(i, i + 1);
        }
    }

    private void loadParameters(Executable method, MethodVisitor methodVisitor, Map<Integer, Integer> localVarMap, boolean inExceptionHandleBlock) {
        Parameter[] parameters = method.getParameters();
        // Load receiver
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            if (p.getAnnotation(Advice.This.class) != null) {
                ...
                methodVisitor.visitVarInsn(ALOAD, 0);
                methodVisitor.visitTypeInsn(CHECKCAST, Type.getInternalName(p.getType())); // cast this ref parameter
            } else {
