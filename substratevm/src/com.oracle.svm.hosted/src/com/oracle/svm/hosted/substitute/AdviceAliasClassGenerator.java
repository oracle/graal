...
public class AdviceAliasClassGenerator {
    ...
    /**
     * Dynamically generate alias class that following the substitution idiom (See {@link TargetClass}).
     *
     * @param annotatedClass
     * @param originalClass
     * @param isDeclared     set to true if the original method must be declared in its class. I.e. can be retrieved via {@link Class#getDeclaredMethod(String, Class[])}.
     *                       When set to false, will ignore the {@link NoSuchMethodException} when the declared method is not found.
     * @return
     */
    public Class<?> generateAliasClass(Class<?> annotatedClass, Class<?> originalClass, boolean isDeclared) {
        ...
        if (ret == null) {
            // 1. Collect Advice methods, find before and after advice methods for each method in original class
    ...
    private void registerOriginalAdvice(Class<?> originalClass, Map<Executable, Pair<Method, Method>> originalMethodMap,
        ...
        for (String originalMethodName : originalMethodNames) {
            Class<?>[] parameterTypes = Arrays.stream(adviceMethod.getParameters()).
                    filter(parameter -> parameter.getDeclaredAnnotation(Advice.Return.class) == null &&
