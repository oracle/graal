...
public class WindowsUnwindInfoFeature implements InternalFeature {
    ...
    public void beforeImageWrite(BeforeImageWriteAccess access) {
        ...
        image.getCodeCache().getOrderedCompilations().stream().parallel()
                        .forEach(entry -> WindowsUnwindInfoSupport.visitRanges(entry.getRight(), (range, startMark, end) -> {
                            ...
                            int countOfCodes = switch (startMark.id) {
                                case PROLOGUE_START -> {
                                    ...
                                    CodeMark[] prologueMarks = new CodeMark[WindowsUnwindInfoSupport.MAX_PROLOGUE_MARKS];
                                    int markCount = WindowsUnwindInfoSupport.collectPrologueMarks(compilation, prologueMarks);
                                    yield WindowsUnwindInfoSupport.countOfPrologueCodes(compilation, prologueMarks, markCount);
                                }
        ...
        for (Pair<HostedMethod, CompilationResult> entry : image.getCodeCache().getOrderedCompilations()) {
            WindowsUnwindInfoSupport.visitRanges(entry.getRight(), (range, startMark, end) -> {
                ...
                switch (startMark.id) {
                    case PROLOGUE_START -> {
                        ...
                        int countOfCodes = WindowsUnwindInfoSupport.countOfPrologueCodes(compilation, prologueMarks, markCount);
...
class WindowsUnwindInfoSupport {
    ...
    static void visitRanges(CompilationResult compilation, RangeVisitor visitor) {
    ...
    static final int MAX_PROLOGUE_MARKS = 3;

    /** Collects prologue marks into a given array and returns the number of marks collected. */
    static int collectPrologueMarks(CompilationResult compilation, CodeMark[] prologueMarks) {
        /*
     ...
     */
    static int countOfPrologueCodes(SubstrateCompilationResult compilation, CodeMark[] prologueMarks, int markCount) {
...
enum UNWIND_CODE {
    ...
    UWOP_ALLOC_HUGE(1, 3) {
        @Override
        void emit(ByteBuffer buffer, int codeOffset, int info) {
            assert 524280 < info && info % 8 == 0 : info;
    ...
    UWOP_ALLOC_LARGE(1, 2) {
        @Override
        void emit(ByteBuffer buffer, int codeOffset, int info) {
            assert 128 < info && info <= 524280 && info % 8 == 0 : info;
    ...
    UWOP_ALLOC_SMALL(2, 1) {
        @Override
        void emit(ByteBuffer buffer, int codeOffset, int info) {
            assert 0 < info && info <= 128 && info % 8 == 0 : info;
