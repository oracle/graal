    ...
    protected void registerConditionalConfiguration(AccessCondition condition, Consumer<AccessCondition> consumer) {
        ...
        if (reachabilityCondition.isAlwaysTrue()) {
            ...
            if (beforeAnalysisAccess == null) {
                Collection<Runnable> handlers = pendingReachabilityHandlers.computeIfAbsent(reachabilityCondition.getType(), key -> new ConcurrentLinkedQueue<>());
                handlers.add(() -> consumer.accept(runtimeCondition));
            } else {
                beforeAnalysisAccess.registerReachabilityHandler(access -> consumer.accept(runtimeCondition), reachabilityCondition.getType());
            }
