...
public final class StackVerifier {
    ...
    private static boolean verifyFrame(Pointer frameSP, CodePointer frameIP, CodeInfoAccessor accessor, CodeInfoHandle handle, DeoptimizedFrame deoptimizedFrame) {
        final Log trace = getTraceLog();
        ...
        if (!CodeInfoTable.visitObjectReferences(frameSP, frameIP, accessor, handle, deoptimizedFrame, verifyFrameReferencesVisitor)) {
            return false;
        }
    ...
    private static class StackFrameVerifierVisitor implements StackFrameVisitor {
        ...
        public boolean visitFrame(Pointer currentSP, CodePointer currentIP, CodeInfoAccessor accessor, CodeInfoHandle handle, DeoptimizedFrame deoptimizedFrame) {
            final Log trace = getTraceLog();
            long totalFrameSize = accessor.lookupTotalFrameSize(handle, accessor.relativeIP(handle, currentIP));
            ...
            if (!verifyFrame(currentSP, currentIP, accessor, handle, deoptimizedFrame)) {
