...
public class HeapFeature implements GraalFeature {
    ...
    public void registerGraphBuilderPlugins(Providers providers, Plugins plugins, boolean analysis, boolean hosted) {
        Registration r = new Registration(plugins.getInvocationPlugins(), AllocationIntrinsics.class);
        r.register5("formatObject", Pointer.class, Class.class, boolean.class, boolean.class, boolean.class, new InvocationPlugin() {
            @Override
            public boolean apply(GraphBuilderContext b, ResolvedJavaMethod targetMethod, Receiver receiver, ValueNode memory, ValueNode hub, ValueNode rememberedSet, ValueNode fillContents,
                            ValueNode emitMemoryBarrier) {
                b.addPush(JavaKind.Object, new FormatObjectNode(memory, hub, rememberedSet, fillContents, emitMemoryBarrier));
                return true;
            }
        });
        r.register7("formatArray", Pointer.class, Class.class, int.class, boolean.class, boolean.class, boolean.class, boolean.class, new InvocationPlugin() {
            @Override
            public boolean apply(GraphBuilderContext b, ResolvedJavaMethod targetMethod, Receiver receiver, ValueNode memory, ValueNode hub, ValueNode length, ValueNode rememberedSet,
                            ValueNode unaligned, ValueNode fillContents, ValueNode emitMemoryBarrier) {
                b.addPush(JavaKind.Object, new FormatArrayNode(memory, hub, length, rememberedSet, unaligned, fillContents, emitMemoryBarrier));
                return true;
            }
        });
    }
