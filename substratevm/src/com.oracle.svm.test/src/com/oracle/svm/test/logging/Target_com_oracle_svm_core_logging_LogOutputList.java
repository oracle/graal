...
public final class Target_com_oracle_svm_core_logging_LogOutputList {
    ...
    /// Removes all configured outputs.
    @Alias
    public native void clear();

    /// Determines whether `level` is enabled by any output.
    @Alias
    public native boolean isLevel(LogLevel level);

    /// Gets the outputs that enable `level`.
    @Alias
    public native LogOutput[] outputsFor(LogLevel level);

    /// Gets the threshold configured for `output`.
    @Alias
    public native LogLevel levelFor(LogOutput output);
}
