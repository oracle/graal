...
/**
 * This test is based on test/jdk/java/lang/StrictMath/HypotTests.java from JDK 11 tag jdk-11+28.
 */
public class StrictHypotTests {
    ...
    static void testHypotCase(double input1, double input2, double expected) {
        Assert.assertEquals(expected, StrictMath.hypot(input1, input2), 0);
        Assert.assertEquals(expected, StrictMath.hypot(input2, input1), 0);
        Assert.assertEquals(expected, StrictMath.hypot(-input1, input2), 0);
        Assert.assertEquals(expected, StrictMath.hypot(input2, -input1), 0);
        Assert.assertEquals(expected, StrictMath.hypot(input1, -input2), 0);
        Assert.assertEquals(expected, StrictMath.hypot(-input2, input1), 0);
    ...
    public void testAgainstTranslit() {
        ...
        double incrementX = 2.0 * Math.ulp(x);
        double incrementY = 2.0 * Math.ulp(y);

        // Don't worry about x or y overflowing to infinity if their
        // exponent is MAX_EXPONENT.
        for (int i = 0; i < 200; i++, x += incrementX) {
            for (int j = 0; j < 200; j++, y += incrementY) {
