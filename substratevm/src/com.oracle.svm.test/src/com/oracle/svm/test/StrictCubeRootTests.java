...
/**
 * This test is based on test/jdk/java/lang/StrictMath/CubeRootTests.java from JDK 11 tag jdk-11+28.
 */
public class StrictCubeRootTests {
    private static void testCubeRootCase(double input, double expected) {
        double minusInput = -input;
        double minusExpected = -expected;
        double delta = 0;

        Assert.assertEquals(expected, StrictMath.cbrt(input), delta);
        Assert.assertEquals(minusExpected, StrictMath.cbrt(minusInput), delta);
    }
