 ...
 * readable legal Java.
 *
 * This file is a copy of test/jdk/java/lang/StrictMath/FdlibmmTranslit.java from JDK 11 tag
 * jdk-11+28
 */
public final class FdlibmTranslit {
     ...
     * first argument..
     */
    private static double lo(double x, int low) {
     ...
     * first argument..
     */
    private static double hi(double x, int high) {
        long transX = Double.doubleToRawLongBits(x);
    ...
    static class Hypot {
        public static double compute(double x, double y) {
            ...
            ha = hi(x) & 0x7fffffff;        // high word of x
            hb = hi(y) & 0x7fffffff;        // high word of y
            if (hb > ha) {
                a = y;
                b = x;
                j = ha;
            ...
            if (ha > 0x5f300000) {   // a>2**500
                if (ha >= 0x7ff00000) {       // Inf or NaN
                    w = a + b;                // for sNaN
                    if (((ha & 0xfffff) | lo(a)) == 0) {
                        w = a;
                    }
                    if (((hb ^ 0x7ff00000) | lo(b)) == 0) {
                        w = b;
                    }
                    return w;
                }
     ...
     * listed below) and | 5 | -59 | 2.0+P1*z+...+P5*z - R(z) | <= 2 | | The computation of exp(r)
     * thus becomes 2*r exp(r) = 1 + ------- R - r r*R1(r) = 1 + r + ----------- (for better
     * accuracy) 2 - R1(r) where 2 4 10 R1(r) = r - (P1*r + P2*r + ... + P5*r ).
     *
    ...
    static class Exp {
        ...
        public static strictfp double compute(double x) {
            ...
            if (k >= -1021) {
                ...
                y = hi(y, hi(y) + ((k + 1000) << 20)); /* add k to y's exponent */
                return y * twom1000;
            }
        }
    }
