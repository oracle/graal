...
public final class TableSwitch {
    ...
    public static int offsetAt(byte[] code, int bci, int i) {
        assert BytecodeStream.opcode(code, bci) == TABLESWITCH;
        return BytecodeStream.readInt(code, getAlignedBci(bci) + OFFSET_TO_FIRST_JUMP_OFFSET + JUMP_OFFSET_SIZE * i);
    }
    ...
    public static int numberOfCases(byte[] code, int bci) {
        assert BytecodeStream.opcode(code, bci) == TABLESWITCH;
        return highKey(code, bci) - lowKey(code, bci) + 1;
    }
    ...
    public static int size(byte[] code, int bci) {
        assert BytecodeStream.opcode(code, bci) == TABLESWITCH;
        return getAlignedBci(bci) + OFFSET_TO_FIRST_JUMP_OFFSET + JUMP_OFFSET_SIZE * numberOfCases(code, bci) - bci;
    }
    ...
    public static int defaultOffset(byte[] code, int bci) {
        assert BytecodeStream.opcode(code, bci) == TABLESWITCH;
        return BytecodeStream.readInt(code, getAlignedBci(bci));
    }
