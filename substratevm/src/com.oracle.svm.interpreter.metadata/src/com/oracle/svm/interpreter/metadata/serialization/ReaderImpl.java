...
final class ReaderImpl extends SerializationContextImpl implements SerializationContext.Reader {
    ...
    public <T> T indexToReference(int refIndex) {
        if (refIndex == NULL_REFERENCE_INDEX) {
            return null;
        }
        @SuppressWarnings("unchecked")
        T ref = (T) indexToReference.get(refIndex);
        if (ref == null) {
    ...
    public <T> ValueReader<T> readerFor(Class<T> targetClass) {
        ...
        ValueReader<T> reader = readerResolver.resolve(targetClass);
        if (reader != null) {
            return reader;
        }
        throw new NoSuchElementException("Cannot resolve ValueReader for " + targetClass);
    }
}
