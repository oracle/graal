...
/**
 * System lookups are currently unsupported, but this would be possible and might be useful to do so.
 * There would be two issues to solve; other than that, the JDK's implementation can be reused:
 * - The library path(s): there might not be a JDK on the machine running the native image
...
public final class Target_jdk_internal_foreign_SystemLookup {
    ...
    public static Target_jdk_internal_foreign_SystemLookup getInstance() {
        // Given that the documentation doesn't specify exactly what the default
        // lookup should provide, we could also return a dummy lookup which doesn't find any symbol.
