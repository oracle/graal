...
public final class FrameId {
    ...
    private static final int FRAME_DEPTH_MASK = (1 << FRAME_DEPTH_BITS) - 1;

    public static int getFrameDepth(long frameId) {
        return (int) (frameId & FRAME_DEPTH_MASK);
    }

    public static long getFrameGeneration(long frameId) {
        return frameId >>> FRAME_DEPTH_BITS;
    }

    public static long createFrameId(long frameGeneration, int frameDepth) {
        assert frameDepth < (1 << FRAME_DEPTH_BITS);
        assert frameGeneration < (1L << (Long.SIZE - FRAME_DEPTH_BITS));
        return (frameGeneration << FRAME_DEPTH_BITS) | frameDepth;
    }
}
