...
public enum LogTagSet {
    ...
    public void writePrefix(Log log) {
        if (isGC) {
            Heap.getHeap().getGC().writeLogPrefix(this, log, LogThreadLocal.previousCollection());
        }
    }
    ...
    /// Opens a message whose GC prefix identifies the collection that most recently completed.
    /// This is used for work that belongs to a collection but executes after its VM operation.
    public LogMessage previousCollectionMessage() {
        VMError.guarantee(isGC, "Only a GC tag set can identify the previous collection.");
        LogThreadLocal.activate(this);
        LogThreadLocal.setPreviousCollection(true);
        return logMessage;
    }
