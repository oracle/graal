...
/** Platform-independent support for patching layered image heaps at runtime. */
public final class LayeredImageHeapSupport {
    ...
    /**
     * Apply patches to the image heap as specified by each layer. This method is called before the
     * image heap provider populates the current isolate's image heap, so it does not patch the new
     * isolate heap directly. Instead, it patches the image heap mapping that the OS loader created
     * when loading the image, specifically the relocatables partition referenced by the layer
     * section. Later, the image heap provider uses that patched loader mapping as the source when it
     * maps or copies image heap contents for new isolates.
     *
     * This method is thread-safe: the first isolate to call it performs the patching while
     * subsequent callers spin-wait until patching is complete.
     *
