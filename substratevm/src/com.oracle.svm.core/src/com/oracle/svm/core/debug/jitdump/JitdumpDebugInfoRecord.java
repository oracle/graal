...
public record JitdumpDebugInfoRecord(JitdumpRecordHeader header, long address, List<JitdumpDebugEntry> entries) {
    ...
    record JitdumpDebugEntry(long address, int line, int discriminator, String filename) {

        /**
         * The base size of a debug entry. Contains the size of all fields except bytes of the file
         * name string. The fixed size fields are as follows:
         * <ul>
         * <li>uint64_t code_addr => 8 bytes
         * <li>uint32_t line => 4 bytes
         * <li>uint32_t discrim => 4 bytes
         * </ul>
         * BASE_SIZE = 8 + 2*4 = 16
         */
        ...
        private int getTotalSize() {
            // +1 for 0 termination of the filename string.
            return BASE_SIZE + filename.getBytes().length + 1;
        }
    }

    /**
     * The base size of a debug info record. Contains the size of all fields except bytes of the
     * debug entry array. The fixed size fields are as follows:
     * <ul>
     * <li>{@link JitdumpRecordHeader#SIZE sizeof(header)} = 16 bytes
     * <li>unint64_t code_addr => 8 bytes
     * <li>unint64_t nr_entry => 8 bytes
     * </ul>
     * BASE_SIZE = sizeof(header) + 2*8 = sizeof(header) + 16
     */
    private static final int BASE_SIZE = JitdumpRecordHeader.SIZE + 16;

    /**
     * Create a jitdump debug info record based on the {@link CompiledMethodEntry debug info} of a
     * compiled method.
     * <p>
     * The {@link JitdumpDebugEntry debug entries} for the debug info record are generated by
     * iterating over all entries in the call tree of a compiled method.
     *
     * @param compiledMethodEntry the {@code CompiledMethodEntry} for a compiled method
     * @param address the address of the compiled code
     * @return a jitdump debug info record
     */
    public static JitdumpDebugInfoRecord create(CompiledMethodEntry compiledMethodEntry, long address) {
        ...
        int recordSize = BASE_SIZE + entries.stream()
                        .mapToInt(JitdumpDebugEntry::getTotalSize)
                        .sum();
