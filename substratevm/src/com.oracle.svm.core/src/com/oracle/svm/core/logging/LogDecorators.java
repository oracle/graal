...
/// each log message for a given output. Decorators are always prepended in
...
public final class LogDecorators {
    ...
    @BasedOnJDKFile("https://github.com/graalvm/labs-openjdk/blob/jdk-25+36/src/hotspot/share/logging/logDecorators.hpp#L44-L67")
    ...
    public static final LogDecorators DEFAULT = new LogDecorators(Decorator.UPTIME.bit() | Decorator.LEVEL.bit() | Decorator.TAGS.bit());

    /// Enabled decorators represented as an ordinal-indexed bit set.
    private final int decorators;

    LogDecorators(int decorators) {
    ...
    public boolean contains(Decorator decorator) {
        return (decorators & decorator.bit()) != 0;
    }
