...
public class DynamicProxySupport implements DynamicProxyRegistry, DuplicableImageSingleton {
    ...
    private static Object createProxyClass(Class<?>[] interfaces) {
        try {
             ...
             */
            ImageSingletons.lookup(RuntimeReflectionSupport.class).register(AccessCondition.alwaysTrue(), false, ReflectionUtil.lookupConstructor(clazz, InvocationHandler.class));
            ...
            for (Class<?> intf : interfaces) {
                ImageSingletons.lookup(RuntimeReflectionSupport.class).register(AccessCondition.alwaysTrue(), false, intf.getMethods());
            }
            return clazz;
        } catch (Throwable t) {
