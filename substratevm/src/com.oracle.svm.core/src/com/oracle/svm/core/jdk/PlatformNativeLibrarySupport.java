...
public abstract class PlatformNativeLibrarySupport {
    ...
    /**
     * Registers the symbols from the {@code .symbols} manifest adjacent to the named static library
     * as built-in JNI symbols. The caller must ensure that the library is or will be selected as a
     * built-in library and invoke this method before built-in symbols are queried. Missing libraries
     * and manifests are ignored.
     */
    ...
    /// The method checks for an exact match with a symbol exported by a built-in static library.
    ///
