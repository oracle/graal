...
public class GraalAbstractQueuedSynchronizer extends AbstractOwnableSynchronizer {
	...
	final int acquire(GraalAbstractQueuedSynchronizer.Node node, int arg, boolean shared,
	...
	private int cancelAcquire(GraalAbstractQueuedSynchronizer.Node node, boolean interrupted,
	...
	private static void signalNextIfShared(GraalAbstractQueuedSynchronizer.Node h) {
		...
		if (h != null && (s = h.next) != null &&
				(s instanceof GraalAbstractQueuedSynchronizer.SharedNode) && s.status != 0) {
			s.getAndUnsetStatus(WAITING);
			LockSupport.unpark(s.waiter);
		}
	}
	...
	private static final long HEAD
			= U.objectFieldOffset(GraalAbstractQueuedSynchronizer.class, "head");

	private static final long TAIL
			= U.objectFieldOffset(GraalAbstractQueuedSynchronizer.class, "tail");
	...
	abstract static class Node {
		...
		static {
			STATUS = GraalAbstractQueuedSynchronizer.U.objectFieldOffset(GraalAbstractQueuedSynchronizer.Node.class, "status");
			NEXT = GraalAbstractQueuedSynchronizer.U.objectFieldOffset(GraalAbstractQueuedSynchronizer.Node.class, "next");
			PREV = GraalAbstractQueuedSynchronizer.U.objectFieldOffset(GraalAbstractQueuedSynchronizer.Node.class, "prev");
		}
	}
	...
	public class GraalConditionObject implements Condition, java.io.Serializable {
		...
		public GraalAbstractQueuedSynchronizer getOuter() {return GraalAbstractQueuedSynchronizer.this;}
		...
		private void unlinkCancelledWaiters(GraalAbstractQueuedSynchronizer.ConditionNode node) {
			if (node == null || node.nextWaiter != null || node == lastWaiter) {
				GraalAbstractQueuedSynchronizer.ConditionNode w = firstWaiter, trail = null;
				while (w != null) {
		...
		public final boolean awaitUntil(Date deadline) {throw new UnsupportedOperationException();}
