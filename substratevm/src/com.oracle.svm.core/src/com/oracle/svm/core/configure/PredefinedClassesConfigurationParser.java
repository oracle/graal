...
public class PredefinedClassesConfigurationParser extends ConfigurationParser {
    ...
    public void parseAndRegister(Reader reader, Path basePath) throws IOException {
    ...
    private void parseOrigin(Path basePath, Map<String, Object> data) {
        ...
        if (!type.equals("agent-extracted")) {
            warnOrFail("Attribute 'type' must have value 'agent-extracted'");
        }
    ...
    private void parseClass(Path basePath, Map<String, Object> data) {
        checkAttributes(data, "class descriptor object", Collections.singleton("hash"), Arrays.asList("nameInfo", CONDITIONAL_KEY));

        ConfigurationCondition condition = parseCondition(data);
        String hash = asString(data.get("hash"), "hash");
        String nameInfo = asNullableString(data.get("nameInfo"), "nameInfo");
        registry.add(condition, nameInfo, hash, basePath);
    }
}
