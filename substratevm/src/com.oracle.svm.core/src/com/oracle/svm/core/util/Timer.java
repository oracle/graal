...
public class Timer implements AutoCloseable {
    ...
    @Uninterruptible(reason = CALLED_FROM_UNINTERRUPTIBLE_CODE, mayBeInlined = true)
    public Timer start(long nanoTime) {
        assert !wasStarted : "Timer already started";
    ...
    public void stop(long nanoTime) {
        ...
        wasStopped = true;
        assert stoppedNanos >= startedNanos() : "Invalid stop time";
    ...
    @Uninterruptible(reason = CALLED_FROM_UNINTERRUPTIBLE_CODE, mayBeInlined = true)
    public void reset() {
        assert wasStopped : "Attempting to reset a started timer";
        startedNanos = 0L;
        wasStarted = false;
        stoppedNanos = 0L;
        wasStopped = false;
