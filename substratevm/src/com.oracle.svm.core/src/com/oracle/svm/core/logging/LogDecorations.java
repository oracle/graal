...
public final class LogDecorations {
    ...
    static LogDecorations capture(LogDecorators decorators) {
        ...
        data.setThreadId(decorators.contains(TID) ? VMThreads.OSThreadIdTL.get().rawValue() : 0);
        return THREAD_LOCAL;
    }
    ...
    public void value(LogDecorators.Decorator decorator, LogLevel level, NativeMemoryLog target) {
        switch (decorator) {
            ...
            case TAGS -> {
                LogTagSet currentTagSet = getTagSet();
                if (currentTagSet != null) {
                    target.string(currentTagSet.commaSeparatedLabel());
                }
            }
        }
    }
