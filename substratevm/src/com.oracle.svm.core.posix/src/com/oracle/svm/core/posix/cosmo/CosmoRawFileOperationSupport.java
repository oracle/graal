...
public class CosmoRawFileOperationSupport extends AbstractRawFileOperationSupport {
    ...
    private static RawFileDescriptor open0(CCharPointer cPath, int flags) {
        int permissions = CosmoStat.S_IRUSR() | CosmoStat.S_IWUSR();
    ...
    public long size(RawFileDescriptor fd) {
        int posixFd = getPosixFileDescriptor(fd);
        return CosmoStat.getSize(posixFd);
    }
    ...
    public boolean write(RawFileDescriptor fd, Pointer data, UnsignedWord size) {
        int posixFd = getPosixFileDescriptor(fd);
        return CosmoUtils.writeUninterruptibly(posixFd, data, size);
    }
    ...
    public long read(RawFileDescriptor fd, Pointer buffer, UnsignedWord bufferSize) {
        int posixFd = getPosixFileDescriptor(fd);
        return CosmoUtils.readUninterruptibly(posixFd, buffer, bufferSize);
    }
...
class CosmoRawFileOperationFeature implements InternalFeature {
    ...
    public void afterRegistration(AfterRegistrationAccess access) {
        ...
        CosmoRawFileOperationSupport littleEndian = new CosmoRawFileOperationSupport(ByteOrder.LITTLE_ENDIAN == nativeByteOrder);
        CosmoRawFileOperationSupport bigEndian = new CosmoRawFileOperationSupport(ByteOrder.BIG_ENDIAN == nativeByteOrder);
        CosmoRawFileOperationSupport nativeOrder = nativeByteOrder == ByteOrder.LITTLE_ENDIAN ? littleEndian : bigEndian;
