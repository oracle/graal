...
final class PosixLoggingSupport extends LoggingSupport {
    ...
    public int rename(RawFileOperationSupport.RawFilePath source, RawFileOperationSupport.RawFilePath target) {
        int result = PosixFileNames.rename((CCharPointer) source, (CCharPointer) target);
        /* Capture errno immediately before another native call can overwrite it. */
        return result == 0 ? 0 : LibC.errno();
    }

    @Override
    public String hostname() {
        int capacity = PosixFileNames.hostNameMax() + 1;
    ...
    private static final class PosixFileNames {
        ...
        @CConstant("_POSIX_HOST_NAME_MAX")
        public static native int hostNameMax();
