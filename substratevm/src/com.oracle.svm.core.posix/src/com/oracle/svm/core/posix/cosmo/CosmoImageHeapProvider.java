 ...
 * strictly required.
 */
public class CosmoImageHeapProvider extends AbstractImageHeapProvider {
    ...
    private static int initializeImageHeapWithMremap(Pointer imageHeap, UnsignedWord imageHeapSizeInFile, UnsignedWord pageSize, WordPointer cachedImageHeapRelocationsPtr, Word heapBeginSym,
        ...
        int mremapFlags = CosmoLibCHelper.MREMAP_FIXED() | CosmoLibCHelper.MREMAP_MAYMOVE() | CosmoLibCHelper.MREMAP_DONTUNMAP();
        PointerBase res = CosmoLibCHelper.NoTransitions.mremapP(heapBeginSym, imageHeapSizeInFile, imageHeapSizeInFile, mremapFlags, imageHeap);
    ...
    private static Pointer getCachedImageHeapRelocations(Pointer cachedImageHeapRelocationsPtr, UnsignedWord pageSize, Word heapRelocsSym, Word heapWritablePatchedEndSym) {
        ...
        if (imageHeapRelocations.isNull() || imageHeapRelocations.equal(COPY_RELOCATIONS_IN_PROGRESS)) {
            if (!cachedImageHeapRelocationsPtr.logicCompareAndSwapWord(0, Word.nullPointer(), COPY_RELOCATIONS_IN_PROGRESS, LocationIdentity.ANY_LOCATION)) {
                ...
                int mremapFlags = CosmoLibCHelper.MREMAP_MAYMOVE() | CosmoLibCHelper.MREMAP_DONTUNMAP();
                imageHeapRelocations = CosmoLibCHelper.NoTransitions.mremapP(linkedRelocsBoundary, heapRelocsLength, heapRelocsLength, mremapFlags, Word.nullPointer());
    ...
    private static int openImageFile(Word heapBeginSym, Pointer magicAddress, WordPointer cachedImageHeapOffsetInFile) {
        ...
        boolean found = false; // findMapping(mapfd, buffer, bufferSize, heapBeginSym, heapBeginSym.add(1), imageHeapMappingStart, imageHeapMappingFileOffset, true);
        if (!found) {
    ...
    private static boolean checkImageFileMagic(int mapfd, int imagefd, CCharPointer buffer, int bufferSize, Pointer magicAddress) {
        ...
        boolean found = false; // findMapping(mapfd, buffer, bufferSize, magicAddress, magicAddress.add(wordSize), magicMappingStart, magicMappingFileOffset, false);
        if (!found) {
            return false;
        }
        ...
        if (CosmoUtils.readUninterruptibly(imagefd, (Pointer) buffer, wordSize) != wordSize) {
            return false;
        }
