...
import com.oracle.svm.core.posix.cosmo.headers.Pthread.pthread_t;
...
public class CosmoThreadCpuTimeSupport implements ThreadCpuTimeSupport {
    ...
    public long getCurrentThreadCpuTime(boolean includeSystemTime) {
        if (!includeSystemTime) {
            ...
            return CosmoLibCHelper.getThreadUserTimeSlow(tid);
        }
        return fastThreadCpuTime(Time.CLOCK_THREAD_CPUTIME_ID());
    }
    ...
    private static long fastCpuTime(pthread_t pthread) {
        ...
        if (Pthread.pthread_getcpuclockid(pthread, threadsClockId) != 0) {
            return -1;
        }
