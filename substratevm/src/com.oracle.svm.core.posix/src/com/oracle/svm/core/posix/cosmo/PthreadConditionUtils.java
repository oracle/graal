 ...
 * waits should use {@link Time#CLOCK_MONOTONIC}. This is not necessarily possible on all
...
public final class PthreadConditionUtils {
    ...
    /**
     * {@link Pthread#pthread_condattr_setclock} is only available on Linux. On Darwin, there
        ...
        if (status == 0) {
            try {
                if (useMonotonicClockForRelativeWait()) {
                    status = Pthread.pthread_condattr_setclock(attr, Time.CLOCK_MONOTONIC());
                    if (status != 0) {
                        return status;
                    }
                }
    ...
    public static void fillTimespec(timespec result, long time, boolean isAbsolute) {
        ...
        int status = CosmoUtils.clock_gettime(clock, now);
        CosmoUtils.checkStatusIs0(status, "PthreadConditionUtils.fillTimespec: clock_gettime failed.");
        if (!isAbsolute) {
