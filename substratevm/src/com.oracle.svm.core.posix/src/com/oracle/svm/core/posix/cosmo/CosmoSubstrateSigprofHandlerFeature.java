...
import static com.oracle.svm.core.posix.cosmo.CosmoSubstrateSigprofHandler.isSignalHandlerBasedExecutionSamplerEnabled;
...
public class CosmoSubstrateSigprofHandlerFeature implements InternalFeature {
     ...
     * {@link CosmoSubstrateGlobalSigprofHandler}), where only one thread is responsible for
     * handling the signal each time the timer expires, resulting in a single sample. The second
     * method is using a per-thread handler (see {@link CosmoSubstrateSigprofHandler}), where each
    ...
    private static SubstrateSigprofHandler makeNewSigprofHandler() {
        return new CosmoSubstrateGlobalSigprofHandler();
    }
}
