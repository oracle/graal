...
public class InterpreterOptions {
    ...
    public static final RuntimeOptionKey<Boolean> InterpreterTrace = new RuntimeOptionKey<>(false, null, InterpreterOptions::validateTraceFlag);
    ...
    private static void validateTraceFlag(RuntimeOptionKey<Boolean> optionKey) {
        if (InterpreterTraceSupport.getValue()) {
            return;
        }
        if (optionKey.getValue()) {
            throw RuntimeOptionValidation.abort("Option '" + optionKey.getName() + "' requires '" + Holder.TRACE_SUPPORT_OPTION + "' to be enabled at build-time.");
        }
    }
