...
import com.oracle.graal.pointsto.nodes.DirectMethodHandleEnsureInitializedNode;
...
public class PointsToMethodHandlePlugin implements NodePlugin {
    ...
    public PointsToMethodHandlePlugin(HostedProviders providers, SnippetReflectionProvider snippetReflection, AnalysisUniverse aUniverse, Providers parsingProviders,
     ...
     *
     * See the documentation in {com.oracle.svm.core.jdk.VarHandleFeature for more information on
     * the overall VarHandle support.
     */
    private boolean isVarHandleMethod(ResolvedJavaMethod method, ValueNode[] args) {
        ...
        if (isVarHandleGuards(method)) {
            ...
            VarHandleInitializer.eagerlyInitializeVarHandle(snippetReflection.asObject(VarHandle.class, args[0].asJavaConstant()), ex -> AnalysisError.shouldNotReachHere(ex));
            return true;
        } else {
            return false;
        }
    ...
    protected void appendWordTypeRewriting(Plugins graphBuilderPlugins) {
        // In standalone mode, the snippetReflection in aUniverse is the original one.
        WordOperationPlugin wordOperationPlugin = new WordOperationPlugin(aUniverse.getSnippetReflection(),
                        aUniverse.getBigbang().getConstantReflectionProvider(),
    ...
    protected static void appendWordOpPlugins(Plugins graphBuilderPlugins, WordOperationPlugin wordOperationPlugin,
                    AbstractTrustedInterfaceTypePlugin trustedInterfaceTypePlugin) {
