 ...
 * @see IDEReport
 */
public final class AnalysisIDEReporting {
    ...
    public static void reportUnreachableNode(Node unreachableNode) {
        if (!IDEReport.isEnabled() || unreachableNode == null) {
            return;
        }
        runIfEnabled(ideReport -> {
    ...
    public static void reportDevirtualizedInvoke(PointsToAnalysisMethod method, Invoke invoke, AnalysisMethod singleCallee) {
        ...
        runIfEnabled(ideReport -> {
            ...
            var nodeSourcePosition = invoke.asNode().getNodeSourcePosition();
            ...
            ideReport.saveLineReport(IDEReportCategory.DEVIRTUALIZATION, srcFile, className, line,
                            "Invocation of " + singleCallee.getName() + " has been devirtualized to " + singleCallee.getQualifiedName(),
    ...
    public static void reportConstantParamAndReturnTypes(AnalysisMethod m) {
        if (!IDEReport.isEnabled() || !(m instanceof PointsToAnalysisMethod method)) {
            /* We only support reports from points-to analysis at the moment. */
            return;
        }
        var lineNumberTable = method.getLineNumberTable();
        if (lineNumberTable == null || lineNumberTable.getLineNumbers().length == 0) {
            return;
        }
        runIfEnabled(ideReport -> {
            ...
            for (var i = 0; i < paramFlows.length; i++) {
                ...
                if (paramFlow != null) {
                    ...
                    reportParamOrReturnType(paramFlow, IDEReportCategory.PARAMETER_VALUE, paramDescr, method, filename, method.getQualifiedName(), line, ideReport);
                }
            }
            reportParamOrReturnType(method.getTypeFlow().getReturn(), IDEReportCategory.RETURN_VALUE, "Return value", method, filename, method.getQualifiedName(), line, ideReport);
        });
    }
    ...
    public static void reportPossibleReturnTypes(Invoke invoke, Collection<AnalysisMethod> callees) {
        ...
        runIfEnabled(ideReport -> {
            ...
            for (var calleeRaw : callees) {
                if (!(calleeRaw instanceof PointsToAnalysisMethod callee)) {
                    return;
                }
                var returnFlow = callee.getTypeFlow().getReturn();
                if (returnFlow == null || returnFlow.isSaturated()) {
                    return;
                }
                var returnState = returnFlow.getState();
                if (returnState.canBeNull()) {
                    return;
                }
                ...
                if (constValue != null) {
                    ...
                    var exactType = returnState.exactType();
                    if (exactType == null) {
                        return;
                    }
            ...
            if (!possibleReturnValues.isEmpty() && !possibleReturnTypes.isEmpty()) {
                /* Avoid claiming that a partial subset of heterogeneous outcomes is exhaustive. */
                return;
            }
            var staticReturnType = invoke.getTargetMethod().getSignature().getReturnType(invoke.getContextType());
            if (possibleReturnTypes.stream().anyMatch(rt -> rt.toJavaName().equals(staticReturnType.toJavaName()))) {
                /* Reporting the declared return type adds no information. */
                return;
            }
            ...
            var className = srcPos.getMethod().getDeclaringClass().toJavaName();
            ideReport.saveLineReport(IDEReportCategory.RETURN_VALUE, filePath, className, line, msg, getInliningTrace(srcPos));
        });
    }

    private static void reportParamOrReturnType(TypeFlow<?> paramOrReturnFlow, IDEReportCategory category, String flowPositionDescr, PointsToAnalysisMethod mth, String filename, String className,
                    int line, IDEReport ideReport) {
        if (paramOrReturnFlow == null) {
            return;
        }
        var constValue = extractConstant(paramOrReturnFlow);
        if (constValue != null) {
            ideReport.saveLineReport(category, filename, className, line, flowPositionDescr + " of method " + mth.getName() + " is constant: " + constValue, null);
            ...
            if (exactType != null && !exactType.equals(staticType)) {
                ideReport.saveLineReport(category, filename, className, line, flowPositionDescr + " of method " + mth.getName() + " has actual type " + exactType.toJavaName(), null);
            }
        }
    }
    ...
    public static void maybeReportConstantField(AnalysisField field) {
        ...
        runIfEnabled(ideReport -> {
            if (!(field instanceof PointsToAnalysisField pointsToField)) {
                /* Field value facts are only available from points-to analysis. */
                return;
            }
            var fieldSinkFlow = pointsToField.getSinkFlow();
            ...
            if (constant != null) {
                ideReport.saveFieldReport(IDEReportCategory.CONSTANT_FIELD, filePath, field.getDeclaringClass().toJavaName(), field.getName(),
                                "Field " + field.getName() + " has constant value " + constant);
    ...
    private static boolean isImplicitCheck(IfNode ifNode) {
        var trueSourcePosition = ifNode.trueSuccessor().getNodeSourcePosition();
        var falseSourcePosition = ifNode.falseSuccessor().getNodeSourcePosition();
        if (trueSourcePosition == null || falseSourcePosition == null) {
            return false;
        }
        var truePos = getLineNumber(trueSourcePosition);
        var falsePos = getLineNumber(falseSourcePosition);
