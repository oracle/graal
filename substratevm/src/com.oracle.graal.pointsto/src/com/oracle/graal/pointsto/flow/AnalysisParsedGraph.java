...
public final class AnalysisParsedGraph {
     ...
     * {@link #OPTIMIZATIONS_APPLIED}. Hence, {@code Opt0} can request the graph of the previous
    ...
    public enum Stage {
         ...
         */
        OPTIMIZATIONS_APPLIED(BYTECODE_PARSED);
        ...
        public static boolean isRequiredStage(Stage stage, AnalysisMethod method) {
            return switch (stage) {
                ...
                case OPTIMIZATIONS_APPLIED -> true;
            };
        }
    }
    ...
    /**
     * Creates the final stage (i.e. {@link Stage#OPTIMIZATIONS_APPLIED}) graph for the given
     * method.
     *
     ...
     * {@link #optimizeAndEncode} directly for stage {@link Stage#OPTIMIZATIONS_APPLIED}. This means
     * that the {@link HostVM#methodAfterParsingHook after parsing hook} will ONLY be called for
     * {@link Stage#OPTIMIZATIONS_APPLIED}.
     */
    public static AnalysisParsedGraph createFinalStage(BigBang bb, AnalysisMethod method, AnalysisParsedGraph stage1Graph) {
        if (stage1Graph == null) {
            assert !Stage.isRequiredStage(Stage.firstStage(), method);
            return parseBytecodeForStage(bb, method, Stage.OPTIMIZATIONS_APPLIED);
        }
        if (stage1Graph.encodedGraph == null) {
            return EMPTY;
        }
        return optimizeAndEncode(bb, method, decodeParsedGraph(bb, method, stage1Graph), stage1Graph.isIntrinsic, Stage.OPTIMIZATIONS_APPLIED);
    }
    ...
    private static AnalysisParsedGraph optimizeAndEncode(BigBang bb, AnalysisMethod method, StructuredGraph graph, boolean isIntrinsic, Stage stage) {
        try (DebugContext.Scope s = graph.getDebug().scope("ClosedWorldAnalysis", graph, method)) {
            ...
            switch (stage) {
                ...
                case OPTIMIZATIONS_APPLIED -> bb.getHostVM().methodAfterParsingHook(bb, method, graph);
            }
