...
* (GR-69858) Restored folded-method filtering in `SVMHost` so methods annotated with either `@Fold` or `@GuestFold` are excluded from the run-time universe. Speculative provider-candidate probing can silently reject hosted-named elements, while the final universe verifier continues to report reachable naming violations.
