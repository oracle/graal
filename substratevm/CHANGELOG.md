...
* (GR-73222) `--future-defaults=exact-reflection` now selects exact reachability metadata at build time. Executables built with it accept the immutable runtime options `-XX:+ExactReachabilityMetadata`, which enables exact handling globally including `Class.forName` and resource lookups, and `-XX:ExactReachabilityMetadataPackages=<packages>`, which limits it to calls from the given packages; other executables keep the legacy metadata and reject these options, and the build rejects them as `-R:` defaults unless `--future-defaults=exact-reflection` is part of the same build. The build-time options `--exact-reachability-metadata` and `--exact-reachability-metadata-path` are deprecated and remain accepted for compatibility. With `-XX:MissingRegistrationReportingMode=Warn`, missing registrations are only reported and the call keeps its legacy behavior.
...
* (GR-63268) In exact reachability metadata mode, reflection and JNI queries do not require metadata entries to throw the expected JDK exception when querying a class that cannot possibly have a valid class name
