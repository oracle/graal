...
public class WasmJNICallWrapperFeature implements InternalFeature {
    ...
    public void duringSetup(Feature.DuringSetupAccess access) {
        ...
        config.registerNativeSubstitutionProcessor(new SubstitutionProcessor() {
            private final Map<ResolvedJavaMethod, WebImageWasmJNINativeCallWrapperMethod> callWrappers = new ConcurrentHashMap<>();

            @Override
            public ResolvedJavaMethod lookup(ResolvedJavaMethod method) {
                assert method.isNative() : "Must have been registered as a native substitution processor";
                return callWrappers.computeIfAbsent(method, WebImageWasmJNINativeCallWrapperMethod::new);
            }
        });
    }
}
