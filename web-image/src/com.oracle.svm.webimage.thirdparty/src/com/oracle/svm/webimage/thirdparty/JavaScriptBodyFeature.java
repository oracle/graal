...
public class JavaScriptBodyFeature implements Feature {
    ...
    private static String[] getJSResources(ResolvedJavaType type) {
        var requiredJavaScriptResources = com.oracle.svm.hosted.webimage.util.AnnotationUtil.getDeclaredAnnotationsByType(type, JavaScriptResource.class, JavaScriptResource.Group.class,
                        JavaScriptResource.Group::value);
        assert requiredJavaScriptResources.size() != 0 ||
    ...
    static class JavaScriptBodySubstitutitionProcessor extends SubstitutionProcessor {
        ...
        private static boolean isJavaScriptBodyStubMethod(ResolvedJavaMethod method) {
            if (AnnotationUtil.isAnnotationPresent(method, JavaScriptBody.class)) {
                return true;
            }
            return false;
        }
