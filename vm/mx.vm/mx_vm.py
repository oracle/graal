...
class SvmSupport(object):
    ...
    def native_image(self, build_args, output_file, allow_server=False, nonZeroIsFatal=True, out=None, err=None):
        ...
        native_image_command = [native_image_bin, '--no-fallback', '--eager-class-initialization', '-H:+EnforceMaxRuntimeCompileMethods'] + build_args
...
class GraalVmNativeImage(mx.Project):
    ...
    def build_args(self):
        return [mx_subst.string_substitutions.substitute(arg) for arg in self.native_image_config.build_args] + ['--eager-class-initialization']
