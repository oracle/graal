...
class NativeImageVM(GraalVm):
    ...
    def _configure_from_name(self, config_name):
        ...
        rule = r'^(?P<native_architecture>native-architecture-)?(?P<gate>gate-)?(?P<devmode>devmode-)?(?P<gc>g1gc-)?(?P<llvm>llvm-)?(?P<pgo>pgo-|pgo-hotspot-|pgo-ctx-insens-)?(?P<inliner>aot-inline-|iterative-|inline-explored-)?(?P<edition>ce-|ee-)?$'
        ...
        if matching.group("devmode") is not None:
            mx.logv("'devmode' is enabled for {}".format(config_name))
...
def register_graalvm_vms():
    ...
    for java_home_config in ['default', 'pgo', 'g1gc', 'g1gc-pgo', 'devmode', 'devmode-g1gc']:
