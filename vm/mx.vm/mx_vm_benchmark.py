...
class NativeImageBenchmarkConfig:
    def __init__(self, vm: NativeImageVM, bm_suite: BenchmarkSuite | NativeImageBenchmarkMixin, args):
        ...
        if self.bundle_create_path:
            self.image_build_reports_directory = self.bundle_create_path / "default" / "reports"
        ...
        if self.bundle_create_path:
            self.image_path = self.bundle_create_path / "default" / self.final_image_name
    ...
    def get_build_output_json_file(self, stage: Stage) -> Path:
        ...
        if self.bundle_create_path:
            return self.bundle_create_path / "other" / filename
        else:
...
class NativeImageVM(GraalVm):
    ...
    @staticmethod
    def copy_bundle_output(config: NativeImageBenchmarkConfig, image_path: Path):
        """
        ...
        """
        bundle_dir = config.bundle_path.parent
        bundle_name = config.bundle_path.name
        assert bundle_name.endswith(BUNDLE_EXTENSION), bundle_name
        bundle_output = bundle_dir / f"{bundle_name[:-len(BUNDLE_EXTENSION)]}.output" / "default"
    ...
    def run_stage_instrument_image(self):
        ...
        with self.get_stage_runner() as s:
            ...
            if self.config.bundle_path is not None:
                NativeImageVM.copy_bundle_output(self.config, self.config.instrumented_image_path)
    ...
    def run_stage_image(self):
        ...
        with self.get_stage_runner() as s:
            ...
            if self.config.bundle_path is not None:
                NativeImageVM.copy_bundle_output(self.config, self.config.image_path)
