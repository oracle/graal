...
class NativeImageBenchmarkConfig:
    def __init__(self, vm: NativeImageVM, bm_suite: BenchmarkSuite | NativeImageBenchmarkMixin, args):
        ...
        if vm.dynamic_access:
            base_image_build_args += ['-H:+EnableDynamicAccess',
              '--initialize-at-build-time=com.sun.jmx.mbeanserver.DefaultMXBeanMappingFactory$CollectionMapping',
              '--initialize-at-build-time=com.sun.jmx.mbeanserver.DefaultMXBeanMappingFactory$ArrayMapping',
              '--initialize-at-build-time=com.sun.jmx.interceptor.DefaultMBeanServerInterceptor$ListenerWrapper',
              '--initialize-at-build-time=oracle.jdbc.logging.annotations.Blinder',
              '--initialize-at-build-time=oracle.sql.converter.CharacterConverterFactoryJDBC',
              '--initialize-at-build-time=oracle.sql.CharacterSetFactoryDefault',
              '--initialize-at-build-time=oracle.jdbc.logging.runtime.Features',
            ]
...
class NativeImageVM(GraalVm):
    ...
    def config_name(self):
        ...
        if self.dynamic_access is True:
            config += ["dynamic-access"]
    ...
    def _configure_from_name(self, config_name):
        ...
        rule = r'^(?P<native_architecture>native-architecture-)?(?P<string_inlining>string-inlining-)?(?P<otw>otw-)?(?P<compacting_gc>compacting-gc-)?(?P<dynamic_access>dynamic-access-)?' \
        ...
        if matching.group("dynamic_access") is not None:
            mx.logv(f"'dynamic-access' is enabled for {config_name}")
...
def register_graalvm_vms():
    ...
    for short_name, config_suffix in [('niee', 'ee'), ('ni', 'ce')]:
        if any(component.short_name == short_name for component in mx_sdk_vm_impl.registered_graalvm_components(stage1=False)):
            for main_config in ['default', 'gate', 'llvm', 'native-architecture', 'dynamic-access'] + analysis_context_sensitivity:
            ...
            for main_config in ['llvm', 'native-architecture', 'g1gc', 'native-architecture-g1gc', 'dynamic-access', ''] + analysis_context_sensitivity:
