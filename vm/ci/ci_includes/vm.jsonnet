...

{
  ...
  notify_releaser_build: vm_common.vm_base('linux', 'amd64', 'daily', deploy=true) + {
    ...
    runAfter: [
      ...
      'post-merge-deploy-vm-installables-standalones-java-latest-linux-amd64',
      'post-merge-deploy-vm-base-java-latest-linux-aarch64',
      'post-merge-deploy-vm-installables-standalones-java-latest-linux-aarch64',
  ...
  local builds = [
    ...
    vm_common.graalvm_complete_build_deps('ce', 'windows', 'amd64', java_version='latest') + vm_common.deploy_build + vm_common.vm_base('windows', 'amd64', 'post-merge', deploy=true, jdk_hint='21') + vm_common.maven_deploy_base_functions.base_object('windows', 'amd64', dry_run=false, remote_mvn_repo=$.maven_deploy_repository, remote_non_mvn_repo=$.binaries_repository, local_repo='local') + {
