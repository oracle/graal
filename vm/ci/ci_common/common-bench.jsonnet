...

{
  ...
  vm_bench_js_linux_amd64(bench_suite=null): vm.vm_java_19 + vm_common.svm_common_linux_amd64 + vm_common.sulong_linux + vm.custom_vm_linux + self.vm_bench_common + {
  ...
  vm_bench_polybenchmarks_linux_common(env='polybench-${VM_ENV}', vm_config='jvm', is_gate=false, suite='default:*'): vm_common.svm_common_linux_amd64 + vm_common.truffleruby_linux_amd64 + vm.custom_vm_linux + self.vm_bench_common + vm.vm_java_19 + self.polybench_hpc_linux_common + {
  ...
  vm_bench_polybench_linux_interpreter: self.vm_bench_polybench_linux_common() + vm.vm_java_19 + {
    run+: [
  ...
  vm_bench_polybench_linux_compiler: self.vm_bench_polybench_linux_common() + vm.vm_java_19 + {
  ...
  vm_bench_polybench_linux_context_init: self.vm_bench_polybench_linux_common() + vm.vm_java_19 + {
  ...
  vm_bench_polybench_linux_warmup: self.vm_bench_polybench_linux_common() + vm.vm_java_19 + {
    run+: [
  ...
  vm_bench_polybench_linux_memory: self.vm_bench_polybench_linux_common() + vm.vm_java_19 + {
    run+: [
  ...
  vm_gate_polybench_linux: self.vm_bench_polybench_linux_common(is_gate=true) + vm.vm_java_19 + {
  ...
  local builds = [
    ...
    vm_common.bench_daily_vm_linux_amd64 + self.vm_bench_polybench_nfi_linux_amd64 + vm.vm_java_19 + {name: 'daily-bench-vm-' + vm.vm_setup.short_name + '-polybench-nfi-java19-linux-amd64', notify_groups:: ['polybench']},

    vm_common.bench_daily_vm_linux_amd64 + self.x52_js_bench_compilation_throughput(true) + vm.vm_java_19 + { name: 'daily-bench-vm-' + vm.vm_setup.short_name + '-libgraal-pgo-throughput-js-typescript-java19-linux-amd64' },
    vm_common.bench_daily_vm_linux_amd64 + self.x52_js_bench_compilation_throughput(false) + vm.vm_java_19 + { name: 'daily-bench-vm-' + vm.vm_setup.short_name + '-libgraal-no-pgo-throughput-js-typescript-java19-linux-amd64' },

    vm_common.bench_daily_vm_linux_amd64 + self.vm_bench_js_linux_amd64() + {
      # Override `self.vm_bench_js_linux_amd64.run`
      run: [
        vm_common.mx_vm_common + ['benchmark', '--results-file', self.result_file, 'agentscript-graal-js:*', '--', '--jvm=graalvm-${VM_ENV}-java19', '--jvm-config=jvm', '--js=graal-js', '--js-config=default'],
        $.vm_bench_common.upload,
        vm_common.mx_vm_common + ['benchmark', '--results-file', self.result_file, 'agentscript-graal-js:*', '--', '--jvm=graalvm-${VM_ENV}-java19', '--jvm-config=native', '--js=graal-js', '--js-config=default'],
        $.vm_bench_common.upload,
      ],
      timelimit: '45:00',
      name: 'daily-bench-vm-' + vm.vm_setup.short_name + '-agentscript-js-java19-linux-amd64',
