...
public class UniquePathOptionValue extends OptionValue<String> {
     ...
     */
    private static final ThreadLocal<Integer> threadDumpId = new ThreadLocal<Integer>() {
        private final AtomicInteger dumpId = new AtomicInteger();

        @Override
        protected Integer initialValue() {
            return dumpId.incrementAndGet();
        }
    };
     ...
     * {@link #getPath} and will include a {@link #globalTimeStamp timestamp} and a
     * {@link #threadDumpId sequence number} as part of the name. The same timestamp is used by all
     * instances to indicate the correspondence between different dump files from the same VM and
     * the sequence number is per thread.
     *
     ...
     * filename}, a {@link #globalTimeStamp global timestamp}, {@link #threadDumpId a per thread
     * unique id} and an optional {@link #formatExtension extension}.
     *
    ...
    public Path getPath() {
        ...
        String name = String.format("%s-%d_%d%s", getValue(), globalTimeStamp, threadDumpId.get(), formatExtension());
