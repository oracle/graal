...
public class UniquePathOptionValue extends OptionValue<String> {
     ...
     * {@link #getThreadDumpId sequence number} as part of the name. The same timestamp is used by
     * all instances to indicate the correspondence between different dump files from the same VM
     * and the sequence number is per thread.
     *
     ...
     * filename}, a {@link #globalTimeStamp global timestamp}, {@link #getThreadDumpId a per thread
    ...
    public Path getPath() {
        ...
        String name = String.format("%s-%d_%d%s", getValue(), globalTimeStamp, getThreadDumpId(), formatExtension());
