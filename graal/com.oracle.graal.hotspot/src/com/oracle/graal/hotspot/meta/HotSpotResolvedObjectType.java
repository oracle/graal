...
public final class HotSpotResolvedObjectType extends HotSpotResolvedJavaType {
    ...
    public ResolvedJavaMethod[] getDeclaredConstructors() {
        ...
        for (int i = 0; i < constructors.length; i++) {
            result[i] = graalRuntime().getProviders().getMetaAccess().lookupJavaConstructor(constructors[i]);
    ...
    public ResolvedJavaMethod[] getDeclaredMethods() {
        ...
        for (int i = 0; i < methods.length; i++) {
            result[i] = graalRuntime().getProviders().getMetaAccess().lookupJavaMethod(methods[i]);
            assert !result[i].isConstructor();
        }
        return result;
    }
