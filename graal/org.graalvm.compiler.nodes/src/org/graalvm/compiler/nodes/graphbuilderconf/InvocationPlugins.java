...
public class InvocationPlugins {
    ...
    private final EconomicMap<String, ClassPlugins> registrations = CollectionFactory.newMap(CompareStrategy.EQUALS);
        ...
        void initializeMap() {
            if (!isClosed()) {
                if (registrations.isEmpty()) {
                    entries = CollectionFactory.newMap(CompareStrategy.EQUALS);
                } else {
                    ...
                    if (declaringClass == null) {
                        ...
                        EconomicMap<ResolvedJavaMethodKey, InvocationPlugin> newEntries = CollectionFactory.newMap(CompareStrategy.EQUALS);
    ...
    public InvocationPlugins(Map<ResolvedJavaMethod, InvocationPlugin> plugins, InvocationPlugins parent, MetaAccessProvider metaAccess) {
        ...
        for (Map.Entry<ResolvedJavaMethod, InvocationPlugin> entry : plugins.entrySet()) {
            ...
            if (classPlugins == null) {
                ...
                registrations.put(internalName, classPlugins);
                classPlugins.entries = CollectionFactory.newMap(CompareStrategy.EQUALS);
            }
    ...
    public EconomicSet<ResolvedJavaMethod> getMethods() {
        EconomicSet<ResolvedJavaMethod> res = CollectionFactory.newSet(CompareStrategy.EQUALS);
        if (parent != null) {
