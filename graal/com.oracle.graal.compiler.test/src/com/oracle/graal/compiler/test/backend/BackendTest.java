...
public abstract class BackendTest extends GraalCompilerTest {
    ...
    protected LIRGenerationResult getLIRGenerationResult(final StructuredGraph graph) {
        try (Scope s = Debug.scope("FrontEnd")) {
            GraalCompiler.emitFrontEnd(getProviders(), getBackend(), graph, getDefaultGraphBuilderSuite(), OptimisticOptimizations.NONE, graph.getProfilingInfo(), createSuites());
        } catch (Throwable e) {
            throw Debug.handle(e);
        }

        LIRGenerationResult lirGen = GraalCompiler.emitLIR(getBackend(), graph, null, null, createLIRSuites(), null);
        return lirGen;
    }
