...
prepareData <- function(combineNameWithIdentity=TRUE){
  ...
  names(RAW_DATA) <- c("methodName","methodIdentity","compilationIndex","metricName","metricValue")
  # method metric data comes in long data format, we want it wide
  WIDE_DATA <<- reshape2::dcast(RAW_DATA, methodName + methodIdentity + compilationIndex ~ metricName,fill = 0)
