...
#define LIB_MOKAPOT "/home/mukel/Desktop/graal/espresso/src/com.oracle.truffle.espresso.mokapot/src/libmokapot.so"
...
int main() {
    ...
    if (dlclose(mokapot)) {
        fprintf(stderr, "Error dlclos'ing mokapot : %s\n", dlerror());
        exit(EXIT_FAILURE);
    }

    // Reload surrogate libjvm (MokaPot), using the same (empty?) namespace
    mokapot = dlmopen(mokapot_nmspc, LIB_MOKAPOT, RTLD_LAZY);
    if (!mokapot) {
        fprintf(stderr, "mokapot reload error %s\n", dlerror());
        exit(EXIT_FAILURE);
    }
