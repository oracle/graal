...
public abstract class LongArrayStoreNode extends QuickNode {
    ...
    protected LongArrayStoreNode(int top, int callerBCI) {
        super(top, callerBCI);
    }

    @Override
    public final int execute(VirtualFrame frame, long[] primitives, Object[] refs) {
        StaticObject array = nullCheck(BytecodeNode.popObject(primitives, refs, top - 4));
        int index = BytecodeNode.popInt(primitives, refs, top - 3);
        long value = BytecodeNode.popLong(primitives, refs, top - 1);
        executeStore(array, index, value);
        return Bytecodes.stackEffectOf(Bytecodes.LASTORE);
    }

    abstract void executeStore(StaticObject array, int index, long value);

    @Specialization(guards = "array.isForeignObject()")
    void doForeign(StaticObject array, int index, long value,
    ...
    void doEspresso(StaticObject array, int index, long value) {
        getBytecodesNode().getInterpreterToVM().setArrayLong(value, index, array);
    }
