...
public class VMImplProcessor extends AbstractProcessor {
    // Annotations
 ...
 * published by the Free Software Foundation.
    ...
    // Checkstyle: resume
    // @formatter:on
    private static String generateConstructor(String substitutorName, String targetMethodName, String jniNativeSignature, boolean isJni, int parameterCount, String returnType) {
        StringBuilder str = new StringBuilder();
        ...
        str.append(TAB_3).append(isJni).append("\n");
    ...
    private static AnnotationMirror getAnnotation(ExecutableElement e, TypeElement type) {
        for (AnnotationMirror annotationMirror : e.getAnnotationMirrors()) {
            if (annotationMirror.getAnnotationType().asElement().equals(type)) {
                return annotationMirror;
            }
    ...
    private static String generateInvoke(String className, String targetMethodName, List<String> parameterTypes, List<Boolean> nonPrimitives, String returnType, boolean isStatic, boolean isJni) {
        StringBuilder str = new StringBuilder();
        str.append(TAB_1).append(PUBLIC_FINAL_OBJECT).append(INVOKE);
        int argIndex = 0;
        for (String type : parameterTypes) {
            boolean isNonPrimitive = nonPrimitives.get(argIndex);
            str.append(extractArg(argIndex++, type, isNonPrimitive, isJni ? 1 : 0, TAB_2));
        }
        switch (returnType) {
            case "char":
    ...
    private static String spawnSubstitutor(String className, String targetMethodName, String jniNativeSignature, List<String> parameterTypeName, List<Boolean> nonPrimitives, String returnType,
                    boolean isStatic, boolean isJni) {
        ...
        classFile.append(generateConstructor(substitutorName, targetMethodName, jniNativeSignature, isJni, parameterTypeName.size(), returnType)).append("\n");
        classFile.append(generateGetter(INSTANCE_NAME, SUBSTITUTOR, GETTER)).append("\n");
        classFile.append(TAB_1).append(OVERRIDE).append("\n");

        // Substitution
        classFile.append(generateInvoke(className, targetMethodName, parameterTypeName, nonPrimitives, returnType, isStatic, isJni));
