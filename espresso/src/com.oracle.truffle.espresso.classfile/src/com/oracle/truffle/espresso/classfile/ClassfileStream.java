...
public final class ClassfileStream {
    ...
    public void checkEndOfFile() {
        if (!isAtEndOfFile()) {
            throw classFormatError("Extra bytes");
        }
    }
    ...
    @TruffleBoundary
    public ParserException classFormatError(String message) {
        String extendedMessage = message;
        if (classfile != null) {
            extendedMessage += " [in .class file " + classfile + "]";
        }
        throw new ParserException.ClassFormatError(extendedMessage);
    }

    @TruffleBoundary
    public ParserException eofError() {
