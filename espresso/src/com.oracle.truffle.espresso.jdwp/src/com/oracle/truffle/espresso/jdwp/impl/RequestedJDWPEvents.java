...
public final class RequestedJDWPEvents {
    ...
    public CommandResult registerEvent(Packet packet, Commands callback) {
        ...
        DebuggerController.fine(() -> "New event request with ID: " + packet.id + " with kind: " + eventKind + " and modifiers: " + modifiers);
        for (int i = 0; i < modifiers; i++) {
            byte modKind = input.readByte();
            DebuggerController.fine(() -> "Handling modKind: " + modKind);
        ...
        switch (eventKind) {
            ...
            case EXCEPTION:
                ...
                DebuggerController.fine(() -> "Submitting new exception breakpoint");
                break;
            case CLASS_PREPARE:
                eventListener.addClassPrepareRequest(new ClassPrepareRequest(filter));
                DebuggerController.fine(() -> "Class prepare request received");
                break;
            case FIELD_ACCESS:
                ...
                DebuggerController.fine(() -> "Submitting field access breakpoint: " + location);
                break;
            case FIELD_MODIFICATION:
                ...
                DebuggerController.fine(() -> "Submitting field modification breakpoint: " + location);
                break;
            case THREAD_START:
            ...
            case MONITOR_WAITED:
                ...
                break;
            default:
                DebuggerController.fine(() -> "unhandled event kind " + eventKind);
                break;
        }
    ...
    private void handleModKind(RequestFilter filter, PacketStream input, byte modKind, JDWPContext context) {
        switch (modKind) {
            case 1:
                ...
                DebuggerController.fine(() -> "adding count limit: " + count + " to filter");
                filter.addEventCount(count);
                break;
            case 2:
                DebuggerController.fine(() -> "unhandled modKind 2");
                break;
            case 3: // limit to specific thread
                long threadId = input.readLong();
                Object thread = ids.fromId((int) threadId);
                filter.addThread(thread);
                DebuggerController.fine(() -> "limiting to thread: " + context.getThreadName(thread));
                break;
            case 4:
                ...
                DebuggerController.fine(() -> "RefType limit: " + finalKlass);
                break;
            case 5: // class positive pattern
                ...
                try {
                    ...
                    DebuggerController.fine(() -> "adding positive refType pattern: " + pattern.pattern());
            ...
            case 6:
                ...
                try {
                    ...
                    DebuggerController.fine(() -> "adding negative refType pattern: " + pattern.pattern());
                } catch (PatternSyntaxException ex) {
                    // wrong input pattern
                    throw new RuntimeException("should not reach here");
                }
            ...
            case 7: // location-specific
                ...
                DebuggerController.fine(() -> "Adding breakpoint info for location: " + finalKlass2.getNameAsString() + "." + method.getNameAsString() + "." + line);
                break;
            case 8:
                ...
                DebuggerController.fine(() -> "adding exception filter: caught=" + caught + ", uncaught=" + unCaught);
                break;
            case 9: // limit to specific field
                ...
                DebuggerController.fine(() -> "limiting to field: " + field.getNameAsString());
                break;
            case 10:
                ...
                DebuggerController.fine(() -> "Step command: size= " + size + ", depth=" + depth);
                break;
            case 11:
                long thisId = input.readLong();
                DebuggerController.fine(() -> "adding instance filter for object ID: " + thisId);
                filter.addThisFilterId(thisId);
                break;
            case 12:
                DebuggerController.fine(() -> "unhandled modKind 12");
                break;
            default:
                break;
        }
    ...
    public CommandResult clearRequest(Packet packet) {
        ...
        if (requestFilter != null) {
            ...
            if (kind == eventKind) {
                switch (eventKind) {
                    case SINGLE_STEP:
                        DebuggerController.fine(() -> "Clearing step command: " + requestId);
                    ...
                    case MONITOR_WAITED:
                        ...
                        break;
                    default:
                        DebuggerController.fine(() -> "unhandled event clear kind " + eventKind);
                        break;
                }
            } else {
