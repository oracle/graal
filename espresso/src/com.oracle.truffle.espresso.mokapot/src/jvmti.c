...
JNIEXPORT void* JNICALL initializeJvmtiContext(TruffleEnv *truffle_env, void* (*fetch_by_name)(const char *), int version) {
...
JNIEXPORT void JNICALL disposeJvmtiContext(TruffleEnv *truffle_env, void* management_ptr, int version) {
    if (version <= JVMTI_VERSION) {
		disposeJvmtiContextImpl(truffle_env, management_ptr);
	}
}
