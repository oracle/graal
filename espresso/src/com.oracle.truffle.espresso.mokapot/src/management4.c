...
#define MANAGEMENT_METHOD_LIST_4(V) \
    V(GetVersion) \
    ...
    V(GetTotalThreadAllocatedMemory) /* Since JDK 21 */ \
...
void* initializeManagementContext4(void* (*fetch_by_name)(const char *)) {
  ...
  struct jmmInterface_4 *management = (JmmInterface*) malloc(sizeof(struct jmmInterface_4));

  #define INIT__(name) \
      management->name = fetch_by_name(#name);

  MANAGEMENT_METHOD_LIST_4(INIT__)
  #undef INIT_
...
void disposeManagementContext4(void *management_ptr, void (*release_closure)(void *)) {
  struct jmmInterface_4 *management = (struct jmmInterface_4*) management_ptr;
  ...
  MANAGEMENT_METHOD_LIST_4(DISPOSE__)
  #undef DISPOSE__
