...
public abstract class LongArrayLoadNode extends QuickNode {
    protected static final int LIMIT = 3;

    protected LongArrayLoadNode(int top, int callerBCI) {
        super(top, callerBCI);
    }
    ...
    abstract long executeLoad(StaticObject array, int index);

    @Specialization(guards = "array.isForeignObject()")
    long doForeign(StaticObject array, int index,
    ...
    long doEspresso(StaticObject array, int index) {
        return getBytecodesNode().getInterpreterToVM().getArrayLong(index, array);
    }
