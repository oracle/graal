...
public final class UncachedRedefineObject extends RedefineObjectImpl {
    public UncachedRedefineObject(StaticObject object) {
        super(object);
    }
        ...
        while (klassRef != null) {
            for (Field declaredField : klassRef.getDeclaredFields()) {
                if (declaredField.getNameAsString().equals(fieldName)) {
                    return InternalRedefinitionPlugin.createUncached(declaredField.get(theInstance));
                }
            }
