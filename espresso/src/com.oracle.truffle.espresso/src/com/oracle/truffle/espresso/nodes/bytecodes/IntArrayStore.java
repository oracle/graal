...
/**
 * IASTORE bytecode with interop extensions.
 *
 * <h3>Extensions</h3>
 * <ul>
 * <li>For {@link InteropLibrary#hasArrayElements(Object) array-like} foreign objects, IASTORE is
 ...
 */
@GenerateUncached
@NodeInfo(shortName = "IASTORE")
public abstract class IntArrayStore extends EspressoNode {

    public abstract void execute(StaticObject receiver, int index, int value);

    @Specialization
    void executeWithNullCheck(StaticObject array, int index, int value,
                    @Cached NullCheck nullCheck,
                    @Cached IntArrayStore.WithoutNullCheck intArrayStore) {
        intArrayStore.execute(nullCheck.execute(array), index, value);
    }

    @GenerateUncached
    @NodeInfo(shortName = "IASTORE !nullcheck")
    public abstract static class WithoutNullCheck extends EspressoNode {
        ...
        void doEspresso(StaticObject array, int index, int value) {
            ...
            getContext().getInterpreterToVM().setArrayInt(getLanguage(), value, index, array);
        }
