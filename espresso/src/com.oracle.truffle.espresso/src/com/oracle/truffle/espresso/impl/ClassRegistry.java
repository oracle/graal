        ...
        if (context.getJavaAgents() != null && !info.isHidden() && !info.isAnonymousClass()) {
            ...
            if (!tls.isInTransformer()) {
                try (EspressoThreadLocalState.TransformerScope transformerScope = tls.transformerScope()) {
                    ...
                    beforeRetransformBytes = context.getJavaAgents().applyTransformers(StaticObject.NULL, module, getClassLoader(), typeOrNull, protectionDomain, initialBytes);
                    bytes = context.getJavaAgents().applyRetransformers(StaticObject.NULL, module, getClassLoader(), typeOrNull, protectionDomain, beforeRetransformBytes);
                }
            }
        }
