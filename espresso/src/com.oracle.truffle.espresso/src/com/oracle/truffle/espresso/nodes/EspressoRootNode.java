...
public abstract class EspressoRootNode extends RootNode implements ContextAccess {
    ...
    static final class Default extends EspressoRootNode {
        ...
        @Override
        public Object execute(VirtualFrame frame) {
            assert getMethod().getDeclaringKlass().isInitializedOrInitializing() || getContext().anyHierarchyChanged() : getMethod().toString() + (getMethod().isStatic() ? "" : " recv: " + frame.getArguments()[0].toString());
            if (usesMonitors()) {
