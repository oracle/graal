...
public final class SaveAllAction extends CallableSystemAction {
    ...
    private void saveAsAllSessions() {
        SaveOperation oper = new SaveOperation(true, 
                (Collection)SessionManagerImpl.getInstance().getSessions(),
    ...
    private void saveSessions() {
        SaveOperation oper = new SaveOperation(false, 
                (Collection)SessionManagerImpl.getInstance().getSessions(),
                SessionManagerImpl.getInstance()
        );
    ...
    public void actionPerformed(ActionEvent ev) {
        shiftPressed = (ev.getModifiers() & ActionEvent.SHIFT_MASK) > 0;
    ...
    @NbBundle.Messages({
        "MSG_SavesAllGroupsToFile=Save all groups to file..."
    })
    public SaveAllAction() {
        putValue(Action.SHORT_DESCRIPTION, Bundle.MSG_SavesAllGroupsToFile());
