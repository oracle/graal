...
abstract class InvalidCustomDispatchAccessor1Test extends CustomReceiverService {

    @CustomDispatchAccessor
    static CustomReceiverService getDispatch1(ServiceAPI receiver) {
        return receiver.dispatch;
    }

    @ExpectError("Only a single method can be annotated by the `CustomDispatchAccessor`.%n" +
                    "Fix the ambiguity by removing the `static CustomReceiverService getDispatch1(ServiceAPI receiver)` method or the `static CustomReceiverService getDispatch2(ServiceAPI receiver)` method.")
    @CustomDispatchAccessor
    static CustomReceiverService getDispatch2(ServiceAPI receiver) {
        return receiver.dispatch;
    }
