...
public class LoopFragmentInside extends LoopFragment {
    ...
    private void placeNewSegmentAndCleanup(LoopEx loop, EconomicMap<Node, Node> new2OldPhis) {
        ...
        if (mainCounted.getBody().next() instanceof LoopEndNode && mainCounted.getLimitTest().predecessor() == loop.loopBegin()) {
            /*
             * We assume here that the body of the loop is completely empty, i.e., we assume that
             * there is no control flow in the counted loop body. This however means that we also
             * did not have any code between the loop header and the counted begin (we allow a few
             * special nodes there). Else we would be killing nodes that are as well between - that
             * potentially could be used by loop phis (which we also disallow). Thus, just be safe
             * here and ensure we really see the pattern we are expect namely a completely empty
             * (fixed nodes) loop body.
             */
