...
public class PluginGenerator {
    ...
    private static void createPluginFactory(AbstractProcessor processor, Element topLevelClass, List<GeneratedPlugin> plugins) {
        ...
        processor.createProviderFile(qualifiedGenClassName, "jdk.compiler.graal.nodes.graphbuilderconf.GeneratedPluginFactory", topLevelClass);
    }
    ...
    protected static void createImports(PrintWriter out, AbstractProcessor processor, List<GeneratedPlugin> plugins, String importingPackage) {
        ...
        extra.add("jdk.compiler.graal.nodes.ValueNode");
        extra.add("jdk.compiler.graal.nodes.graphbuilderconf.GraphBuilderContext");
        extra.add("jdk.compiler.graal.nodes.graphbuilderconf.InvocationPlugin");
        extra.add("jdk.compiler.graal.nodes.graphbuilderconf.InvocationPlugins");
        extra.add("jdk.compiler.graal.nodes.graphbuilderconf.GeneratedPluginFactory");
        extra.add("jdk.compiler.graal.nodes.graphbuilderconf.GeneratedPluginInjectionProvider");

        for (GeneratedPlugin plugin : plugins) {
            plugin.extraImports(processor, extra);
            extra.add("jdk.compiler.graal.nodes.graphbuilderconf." + plugin.pluginSuperclass());
            if (plugin.needsReplacement(processor)) {
                extra.add("jdk.compiler.graal.options.ExcludeFromJacocoGeneratedReport");
                extra.add("jdk.compiler.graal.graph.NodeInputList");
                if (plugin.isWithExceptionReplacement(processor)) {
                    extra.add("jdk.compiler.graal.nodes.PluginReplacementWithExceptionNode");
                } else {
                    extra.add("jdk.compiler.graal.nodes.PluginReplacementNode");
                }
            }
        }
