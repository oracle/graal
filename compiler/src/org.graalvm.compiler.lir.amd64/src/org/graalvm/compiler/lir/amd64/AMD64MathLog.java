...
public class AMD64MathLog extends AMD64HotSpotHelper {
    public static final Value[] temps = new Value[]{
                    r11.asValue(LIRKind.value(AMD64Kind.QWORD)),
                    r8.asValue(LIRKind.value(AMD64Kind.QWORD)),
                    rax.asValue(LIRKind.value(AMD64Kind.QWORD)),
                    rcx.asValue(LIRKind.value(AMD64Kind.QWORD)),
                    rdx.asValue(LIRKind.value(AMD64Kind.QWORD)),
                    xmm0.asValue(LIRKind.value(AMD64Kind.DOUBLE)),
                    xmm1.asValue(LIRKind.value(AMD64Kind.DOUBLE)),
                    xmm2.asValue(LIRKind.value(AMD64Kind.DOUBLE)),
                    xmm3.asValue(LIRKind.value(AMD64Kind.DOUBLE)),
                    xmm4.asValue(LIRKind.value(AMD64Kind.DOUBLE)),
                    xmm5.asValue(LIRKind.value(AMD64Kind.DOUBLE)),
                    xmm6.asValue(LIRKind.value(AMD64Kind.DOUBLE)),
                    xmm7.asValue(LIRKind.value(AMD64Kind.DOUBLE))
    };
...
// rax, rdx, rcx, r8, r11
    ...
    void generate(AMD64MacroAssembler masm, CompilationResultBuilder crb, Value result) {
        ...
        assert_different_registers(tmp1, tmp2, rax, rcx, rdx);
        masm.jmp(start);
        ...
        masm.movl(rcx, 32768);
        masm.movdl(xmm4, rcx);
        masm.movq(tmp1, 0xffffe00000000000L);
        masm.movdq(xmm5, tmp1);
        masm.movdqu(xmm1, xmm0);
        masm.pextrw(rax, xmm0, 3);
        masm.por(xmm0, xmm2);
        masm.movl(rcx, 16352);
        masm.psrlq(xmm0, 27);
        masm.leaq(tmp2, ExternalAddress(crb, lTbl));
        masm.psrld(xmm0, 2);
        ...
        masm.movq(xmm6, ExternalAddress(crb, log2));       // 0xfefa3800, 0x3fa62e42
        masm.movdqu(xmm3, ExternalAddress(crb, coeff));    // 0x92492492, 0x3fc24924, 0x00000000,
                                                           // 0xbfd00000
        masm.subsd(xmm5, xmm2);
        masm.andl(rdx, 16711680);
        masm.shrl(rdx, 12);
        masm.movdqu(xmm0, new AMD64Address(tmp2, rdx, AMD64Address.Scale.Times1));
        masm.movdqu(xmm4, ExternalAddress(crb, coeff16)); // 0x3d6fb175, 0xbfc5555e, 0x55555555,
                                                          // 0x3fd55555
        masm.addsd(xmm1, xmm5);
        masm.movdqu(xmm2, ExternalAddress(crb, coeff32)); // 0x9999999a, 0x3fc99999, 0x00000000,
                                                          // 0xbfe00000
        ...
        masm.mulsd(xmm7, ExternalAddress(crb, log28));    // 0x93c76730, 0x3ceef357
        ...
        masm.bind(b15);
        masm.addq(rsp, 24);
        if (asRegister(result) != xmm0) {
            masm.movdbl(asRegister(result), xmm0);
        }
    }
}
