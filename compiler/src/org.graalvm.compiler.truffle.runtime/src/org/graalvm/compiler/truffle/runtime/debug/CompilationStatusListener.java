...
public final class CompilationStatusListener extends AbstractGraalTruffleRuntimeListener {
    ...
    private final AtomicInteger dequeues = new AtomicInteger();
    private final AtomicInteger deoptimizations = new AtomicInteger();
    ...
    @Override
    public synchronized void onCompilationFailed(OptimizedCallTarget target, String reason, boolean bailout, boolean permanentBailout, int tier) {
    ...
    public synchronized CompilationState sampleCompilationState() {
        /*
         * Thermometer TODO: I'd like to track queue size and running count by
         * using the listener as normal, but I can't find a way to make that
         * work - I think the key problem is that compilations dequeued may
         * either be queued or running and I can't tell which. We'll work to
         * improve that if this is to be merged.
         */
