...
public final class NodeClass<T> extends FieldIntrospection<T> {
    ...
    static EconomicMap<Node, Node> addGraphDuplicate(final Graph graph, final Graph oldGraph, int estimatedNodeCount, Iterable<? extends Node> nodes, final DuplicationReplacement replacements) {
        ...
        InplaceUpdateClosure replacementClosure = new InplaceUpdateClosure() {
            ...
            public Node replacement(Node node, Edges.Type type) {
                return target(replacements, newNodes, node, node.graph() == graph, type == Type.Inputs);
            }
    ...
    private static void transferEdges(final Graph graph, final DuplicationReplacement replacements, final EconomicMap<Node, Node> newNodes, Node oldNode, Node node, Edges.Type type) {
        ...
        for (Position pos : oldEdges.getPositionsIterable(oldNode)) {
            ...
            if (oldEdge != null)
                pos.set(node, target(replacements, newNodes, oldEdge, type == Edges.Type.Inputs, oldEdge.graph() == graph));
        }

    }

    private static Node target(DuplicationReplacement replacements, EconomicMap<Node, Node> nodes, Node oldEdge, boolean b, boolean b2) {
        Node target = nodes.get(oldEdge);
        if (target == null) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (type.isPrimitive()) {
                if (type == Integer.TYPE) {
                    ...
                    if (!data.equalShort(i, a, b)) return false;
                } else if (type == Character.TYPE) {
                    if (!data.equalChar(i, a, b)) return false;
                } else if (type == Byte.TYPE) {
                    if (!data.equalByte(i, a, b)) return false;
                } else {
        ...
        if (nonPrimsRemain > 0) {
            ...
            for (int i = 0; i < n; ++i) {
                if (!data.getType(i).isPrimitive()) {
                    ...
                    if (objectA != objectB) {
                        if (objectA == null || objectB == null || !deepEquals0(objectA, objectB))
                            return false;
                    }
                }
                if (--nonPrimsRemain <= 0)
    ...
    public Node allocateInstance() {
        try {
            Node node = (Node) Node.UNSAFE.allocateInstance(getJavaClass());
    ...
    public NodeIterable<Node> getInputIterable(final Node node) {
        ...
        return new NodeIterable<Node>() {
            ...
            @Override
            public Iterator<Node> iterator() {
                return isModificationCountsEnabled() ?
                        new RawEdgesWithModCountIterator(node, mask) :
                        new RawEdgesIterator(node, mask);
            }
