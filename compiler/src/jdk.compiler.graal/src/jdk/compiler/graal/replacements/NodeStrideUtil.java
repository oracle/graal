 ...
 * {@link ArrayRegionCompareToNode}. These intrinsics may work with fixed strides of 1, 2 or 4 bytes
 * per element, or with a dynamic parameter containing the stride in log2 format, i.e.
 * {@code 0 -> 1 byte, 1 -> 2 byte, 2 -> 4 byte}. If an intrinsic method has two array parameters
 * with potentially different strides, both log2 stride values are encoded into one as follows:
 * {@code (strideA * N_STRIDES) + strideB}. This value is used inside the intrinsic as a jump table
 * index to dispatch to separately compiled versions for each possible combination of strides.
 */
