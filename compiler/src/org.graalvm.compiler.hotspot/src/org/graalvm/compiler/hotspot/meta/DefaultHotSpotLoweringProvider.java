...
public class DefaultHotSpotLoweringProvider extends DefaultJavaLoweringProvider implements HotSpotLoweringProvider {
    ...
    public void initialize(OptionValues options, Iterable<DebugConfigCustomizer> customizers, HotSpotProviders providers, GraalHotSpotVMConfig config) {
        super.initialize(options, customizers, runtime, providers, providers.getSnippetReflection());

        assert target == providers.getCodeCache().getTarget();
        instanceofSnippets = new InstanceOfSnippets.Templates(options, customizers, runtime, providers, target);
        newObjectSnippets = new NewObjectSnippets.Templates(options, customizers, runtime, providers, target, config);
        monitorSnippets = new MonitorSnippets.Templates(options, customizers, runtime, providers, target, config.useFastLocking);
        writeBarrierSnippets = new WriteBarrierSnippets.Templates(options, customizers, runtime, providers, target, config.useCompressedOops ? config.getOopEncoding() : null);
        exceptionObjectSnippets = new LoadExceptionObjectSnippets.Templates(options, customizers, providers, target);
        unsafeLoadSnippets = new UnsafeLoadSnippets.Templates(options, customizers, providers, target);
        assertionSnippets = new AssertionSnippets.Templates(options, customizers, providers, target);
        arraycopySnippets = new ArrayCopySnippets.Templates(options, customizers, runtime, providers, target);
        stringToBytesSnippets = new StringToBytesSnippets.Templates(options, customizers, providers, target);
        hashCodeSnippets = new HashCodeSnippets.Templates(options, customizers, providers, target);
        if (GeneratePIC.getValue(options)) {
            resolveConstantSnippets = new ResolveConstantSnippets.Templates(options, customizers, providers, target);
            profileSnippets = new ProfileSnippets.Templates(options, customizers, providers, target);
        }
        providers.getReplacements().registerSnippetTemplateCache(new UnsafeArrayCopySnippets.Templates(options, customizers, providers, target));
    }
