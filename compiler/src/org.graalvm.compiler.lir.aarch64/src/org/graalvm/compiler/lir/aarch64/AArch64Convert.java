...
public class AArch64Convert {
    ...
    public static class ASIMDSignExtendOp extends AArch64LIRInstruction {
        ...
        public void emitCode(CompilationResultBuilder crb, AArch64MacroAssembler masm) {
            ...
            if (currentESize == finalESize) {
                /* register move */
                masm.neon.moveVector(ASIMDSize.fromVectorKind(resultValue.getPlatformKind()), result, input);
            } else {
                do {
    ...
    public static class ASIMDFloatConvertOp extends AArch64LIRInstruction {
        ...
        public void emitCode(CompilationResultBuilder crb, AArch64MacroAssembler masm) {
            ...
            switch (op) {
                case F2I:
                    masm.neon.fcvtzsVector(size, srcESize, result, input);
                    break;
                case D2L:
                    masm.neon.fcvtzsVector(size, srcESize, result, input);
                    break;
                case I2F:
                    masm.neon.scvtfVector(size, srcESize, result, input);
                    break;
                case L2D:
                    masm.neon.scvtfVector(size, srcESize, result, input);
                    break;
                case D2F:
                    masm.neon.fcvtnVector(srcESize, result, input);
                    break;
                case F2D:
                    masm.neon.fcvtlVector(srcESize, result, input);
                    break;
                case I2D:
                    /* First convert int to long, then to double */
                    masm.neon.sxtlVector(srcESize, result, input);
                    masm.neon.scvtfVector(size, ElementSize.fromKind(resultValue.getPlatformKind()), result, input);
                    break;
                case D2I:
                    /* First convert double to long, then to int */
                    masm.neon.fcvtzsVector(size, srcESize, result, input);
                    masm.neon.xtnVector(srcESize, result, input);
                    break;
                /*
