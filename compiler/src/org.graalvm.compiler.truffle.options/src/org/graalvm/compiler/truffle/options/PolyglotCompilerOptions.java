...
public final class PolyglotCompilerOptions {
    ...
    @Option(help = "Number of invocations or loop iterations needed to compile a guest language root in first tier under normal compilation load. Might be reduced/increased if compilation load is low/high if DynamicCompilationThresholds is enabled. (default: 400).", usageSyntax = "[1, inf)", category = OptionCategory.EXPERT) //
    public static final OptionKey<Integer> FirstTierCompilationThreshold = new OptionKey<>(400);

    @Option(help = "Number of invocations or loop iterations needed to compile a guest language root in first tier under normal compilation load. Might be reduced/increased if compilation load is low/high if DynamicCompilationThresholds is enabled. (default: 10000).", usageSyntax = "[1, inf)", category = OptionCategory.EXPERT) //
