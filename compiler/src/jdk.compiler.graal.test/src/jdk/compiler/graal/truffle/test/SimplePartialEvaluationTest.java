...
import jdk.compiler.graal.truffle.test.nodes.AddTestNode;
import jdk.compiler.graal.truffle.test.nodes.BlockTestNode;
import jdk.compiler.graal.truffle.test.nodes.ConstantTestNode;
import jdk.compiler.graal.truffle.test.nodes.ExplodeLoopUntilReturnNode;
import jdk.compiler.graal.truffle.test.nodes.ExplodeLoopUntilReturnWithThrowNode;
import jdk.compiler.graal.truffle.test.nodes.InliningNullCheckNode1;
import jdk.compiler.graal.truffle.test.nodes.InliningNullCheckNode2;
import jdk.compiler.graal.truffle.test.nodes.LambdaTestNode;
import jdk.compiler.graal.truffle.test.nodes.LoadLocalTestNode;
import jdk.compiler.graal.truffle.test.nodes.LoopTestNode;
import jdk.compiler.graal.truffle.test.nodes.NeverPartOfCompilationTestNode;
import jdk.compiler.graal.truffle.test.nodes.ObjectEqualsNode;
import jdk.compiler.graal.truffle.test.nodes.ObjectHashCodeNode;
import jdk.compiler.graal.truffle.test.nodes.PartialIntrinsicNode;
import jdk.compiler.graal.truffle.test.nodes.RecursionTestNode;
import jdk.compiler.graal.truffle.test.nodes.RootTestNode;
import jdk.compiler.graal.truffle.test.nodes.StoreLocalTestNode;
import jdk.compiler.graal.truffle.test.nodes.StringEqualsNode;
import jdk.compiler.graal.truffle.test.nodes.StringHashCodeFinalNode;
import jdk.compiler.graal.truffle.test.nodes.StringHashCodeNonFinalNode;
import jdk.compiler.graal.truffle.test.nodes.SynchronizedExceptionMergeNode;
import jdk.compiler.graal.truffle.test.nodes.UnrollLoopUntilReturnNode;
import jdk.compiler.graal.truffle.test.nodes.UnrollLoopUntilReturnWithThrowNode;
import jdk.compiler.graal.truffle.test.nodes.explosion.LoopExplosionPhiNode;
import jdk.compiler.graal.truffle.test.nodes.explosion.NestedExplodedLoopTestNode;
import jdk.compiler.graal.truffle.test.nodes.explosion.TwoMergesExplodedLoopTestNode;
import jdk.compiler.graal.truffle.test.nodes.explosion.UnrollingTestNode;
    ...
    public void neverPartOfCompilationTest() {
        ...
        try (PreventDumping noDump = new PreventDumping()) {
            ...
            if (getTruffleCompiler(null).getPartialEvaluator().getGraphBuilderConfigForParsing().trackNodeSourcePosition() || GraalOptions.TrackNodeSourcePosition.getValue(getInitialOptions())) {
                assertStack(trace[0], MethodSource.of(CompilerAsserts.class, "neverPartOfCompilation"));
                assertStack(trace[1], MethodSource.of(NeverPartOfCompilationTestNode.class, "execute"));
                assertStack(trace[2], MethodSource.of(RootTestNode.class, "execute"));
            } else {
                assertStack(trace[0], MethodSource.of(NeverPartOfCompilationTestNode.class, "execute"));
                assertStack(trace[1], MethodSource.of(RootTestNode.class, "execute"));
            }
        }
    }

    static void assertStack(StackTraceElement stack, MethodSource element) {
        assertStack(stack, element.className(), element.methodName(), element.fileName());
    }
