 ...
 * {@link HotSpotGraphBuilderPlugins#registerStringPlugins}.
 */
    ...
    public void testStringUTF16ToBytes() throws ClassNotFoundException {
        ...
        for (int srcOffset = 0; srcOffset < 2; srcOffset++) {
            for (int i = 0; i < N; i++) {
                ...
                byte[] dst2 = (byte[]) GraalCompilerTest.executeVarargsSafe(code, src, srcDelta, copiedLength);
        ...
        for (int dstOffset = 0; dstOffset < 2; dstOffset++) {
            for (int srcOffset = 0; srcOffset < 2; srcOffset++) {
                for (int i = 0; i < N; i++) {
                    ...
                    GraalCompilerTest.executeVarargsSafe(code, src, srcDelta, srcDelta + copiedLength, dst2, dstDelta);
                    assertDeepEquals(dst, dst2);
                }
            }
        }
