...
/**
 * Interface implemented by classes that are interested in being notified of compilation events such
 * when the compiler starts/ends a {@link #enterPhase phase} or {@linkplain #notifyInlining inlines}
 * a method into the current compilation unit.
 */
public interface CompilationListener {
     ...
     * @param nesting the nesting level of the phase, starting at 0
     * @return an object whose {@link CompilerPhaseScope#close()} method will be called when the
     *         phase completes
     */
    CompilerPhaseScope enterPhase(CharSequence phaseName, int nesting);

    /**
     * Scope for a compiler phase event. The {@link #close()} method is called when the compiler
     * phase completes.
     */
    interface CompilerPhaseScope extends AutoCloseable {
        @Override
        void close();
    }
