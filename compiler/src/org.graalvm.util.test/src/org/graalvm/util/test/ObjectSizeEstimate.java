                        ...
                        if (!Modifier.isStatic(f.getModifiers())) {
                            ...
                            if (type == Byte.TYPE) {
                                ...
                                if (maxDepth > 1) {
                                    try {
                                        if (!f.canAccess(o)) {
                                            Module me = ObjectSizeEstimate.class.getModule();
                                            Module fm = f.getDeclaringClass().getModule();
                                            String packageName = f.getDeclaringClass().getPackageName();
                                            if (!fm.isOpen(packageName, me)) {
                                                Modules.addOpens(fm, packageName, me);
                                            }
