...
public class GeneratedFoldPlugin extends GeneratedPlugin {
    ...
    public void extraImports(AbstractProcessor processor, Set<String> imports) {
        if (intrinsicMethod.getReturnType().getKind().isPrimitive()) {
    ...
    protected void createExecute(AbstractProcessor processor, PrintWriter out, InjectedDependencies deps) {
        ...
        out.printf("        return doExecute(b, targetMethod, args%s);\n", extraArguments);
    }
    ...
    private void emitReplace(AbstractProcessor processor, PrintWriter out, InjectedDependencies deps) {
        ...
        if (firstArg == 0) {
            ...
            constantArgument(processor, out, deps, 0, type.asType(), 0, false, false);
        }

        int argCount = firstArg;
        for (VariableElement param : params) {
            if (processor.getAnnotation(param, processor.getType(INJECTED_PARAMETER_CLASS_NAME)) == null) {
                constantArgument(processor, out, deps, argCount, param.asType(), argCount, false, false);
            } else {
        ...
        /* constantArgument assigns in separate branches, so copy its locals before capturing them. */
        for (int i = 0; i < argCount; i++) {
            out.printf("        final var foldArg%d = arg%d;\n", i, i);
        }
        ...
        out.printf("        ValueNode node = b.executeFold(targetMethod, args, () -> {\n");
        /* A cached layered Fold does not need hosted objects for its constant arguments. */
        if (firstArg != 0) {
            emitNonNullCheck(processor, out, 0, ((TypeElement) intrinsicMethod.getEnclosingElement()).asType());
        }
        int parameterIndex = firstArg;
        for (VariableElement param : params) {
            if (processor.getAnnotation(param, processor.getType(INJECTED_PARAMETER_CLASS_NAME)) == null) {
                emitNonNullCheck(processor, out, parameterIndex, param.asType());
            }
            parameterIndex++;
        }
        ...
        if (argCount > firstArg) {
            out.printf("foldArg%d", firstArg);
            for (int i = firstArg + 1; i < argCount; i++) {
                out.printf(", foldArg%d", i);
            }
        }
        out.printf(");\n");
        ...
        switch (returnType.getKind()) {
            case BOOLEAN:
                out.printf("            return JavaConstant.forInt(result ? 1 : 0);\n");
                break;
            case BYTE:
            case SHORT:
            case CHAR:
            case INT:
                out.printf("            return JavaConstant.forInt(result);\n");
                break;
            case LONG:
                out.printf("            return JavaConstant.forLong(result);\n");
                break;
            case FLOAT:
                out.printf("            return JavaConstant.forFloat(result);\n");
                break;
            case DOUBLE:
                out.printf("            return JavaConstant.forDouble(result);\n");
                break;
            case ARRAY:
            case TYPEVAR:
            case DECLARED:
                if (returnType.equals(processor.getType("java.lang.String"))) {
                    out.printf("            return %s.forString(result);\n", deps.use(processor, WellKnownDependency.CONSTANT_REFLECTION));
                } else {
                    out.printf("            return %s.forObject(result);\n", deps.use(processor, WellKnownDependency.SNIPPET_REFLECTION));
                }
                break;
            default:
    ...
    private static void emitNonNullCheck(AbstractProcessor processor, PrintWriter out, int argumentIndex, TypeMirror type) {
        if ((type.getKind() == TypeKind.ARRAY || type.getKind() == TypeKind.DECLARED) && !type.equals(processor.getType("jdk.vm.ci.meta.ResolvedJavaType"))) {
            out.printf("            assert foldArg%d != null;\n", argumentIndex);
        }
    }
    ...
    protected void createPrivateMembersAndConstructor(AbstractProcessor processor, PrintWriter out, InjectedDependencies deps, String constructorName) {
        ...
        out.printf("    static boolean doExecute(GraphBuilderContext b, ResolvedJavaMethod targetMethod, ValueNode[] args%s) {\n", extraArguments);
    ...
    protected void createHelpers(AbstractProcessor processor, PrintWriter out, InjectedDependencies deps) {
        ...
        out.printf("        return %s.doExecute(b, targetMethod, args%s);\n", getPluginName(), extraArguments);
        out.printf("    }\n");
    }
}
