...
public class ArrayUtilsSubstitutions {
        ...
        if (chars.length <= 4) {
            if (JavaVersionUtil.JAVA_SPEC <= 8) {
                ...
                if (JDK9StringSubstitutions.isCompactString(str)) {
                    ...
                    return indexOfCharCompact(sourceArray, maxIndex, fromIndex, chars);
                }
            }
        } else {
    ...
    private static int indexOfChar(char[] array, int arrayLength, int fromIndex, char[] chars) {
        if (chars.length == 1) {
            return ArrayIndexOf.indexOf1Char(array, arrayLength, fromIndex, chars[0]);
        } else if (chars.length == 2) {
            return ArrayIndexOf.indexOf2Chars(array, arrayLength, fromIndex, chars[0], chars[1]);
        } else if (chars.length == 3) {
            return ArrayIndexOf.indexOf3Chars(array, arrayLength, fromIndex, chars[0], chars[1], chars[2]);
        } else {
            return ArrayIndexOf.indexOf4Chars(array, arrayLength, fromIndex, chars[0], chars[1], chars[2], chars[3]);
        }
    }

    private static int indexOfCharCompact(byte[] array, int arrayLength, int fromIndex, char[] chars) {
        if (chars.length == 1) {
            return ArrayIndexOf.indexOf1CharCompact(array, arrayLength, fromIndex, chars[0]);
        } else if (chars.length == 2) {
            return ArrayIndexOf.indexOf2CharsCompact(array, arrayLength, fromIndex, chars[0], chars[1]);
        } else if (chars.length == 3) {
            return ArrayIndexOf.indexOf3CharsCompact(array, arrayLength, fromIndex, chars[0], chars[1], chars[2]);
        } else {
            return ArrayIndexOf.indexOf4CharsCompact(array, arrayLength, fromIndex, chars[0], chars[1], chars[2], chars[3]);
        }
    }

    private static int indexOfByte(byte[] array, int arrayLength, int fromIndex, byte[] bytes, int nValues) {
        if (nValues == 1) {
            return ArrayIndexOf.indexOf1Byte(array, arrayLength, fromIndex, bytes[0]);
        } else if (nValues == 2) {
            return ArrayIndexOf.indexOf2Bytes(array, arrayLength, fromIndex, bytes[0], bytes[1]);
        } else if (nValues == 3) {
            return ArrayIndexOf.indexOf3Bytes(array, arrayLength, fromIndex, bytes[0], bytes[1], bytes[2]);
        } else {
            return ArrayIndexOf.indexOf4Bytes(array, arrayLength, fromIndex, bytes[0], bytes[1], bytes[2], bytes[3]);
        }
    }

    private static int indexOfByte(byte[] array, int arrayLength, int fromIndex, int values, int nValues) {
        if (nValues == 1) {
            return ArrayIndexOf.indexOf1Byte(array, arrayLength, fromIndex, (byte) values);
        } else if (nValues == 2) {
            return ArrayIndexOf.indexOf2Bytes(array, arrayLength, fromIndex, (byte) values, (byte) (values >>> 8));
        } else if (nValues == 3) {
            return ArrayIndexOf.indexOf3Bytes(array, arrayLength, fromIndex, (byte) values, (byte) (values >>> 8), (byte) (values >>> 16));
        } else {
            return ArrayIndexOf.indexOf4Bytes(array, arrayLength, fromIndex, (byte) values, (byte) (values >>> 8), (byte) (values >>> 16), (byte) (values >>> 24));
        }
    }
