...
public class InliningData {
    ...
    private boolean checkTargetConditions(Invoke invoke, ResolvedJavaMethod method) {
        ...
        if (failureMessage == null) {
            ...
            invoke.asNode().graph().notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, failureMessage);
            return false;
        }
    }
    ...
    private InlineInfo getTypeCheckedInlineInfo(Invoke invoke, ResolvedJavaMethod targetMethod) {
        ...
        if (typeProfile == null) {
            ...
            graph.notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, "no type profile exists");
            return null;
        }
        ...
        if (ptypes == null || ptypes.length <= 0) {
            ...
            graph.notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, "no types in profile");
            return null;
        }
        ResolvedJavaType contextType = invoke.getContextType();
        ...
        if (ptypes.length == 1 && notRecordedTypeProbability == 0 && !speculationFailed) {
            if (!optimisticOpts.inlineMonomorphicCalls(options)) {
                ...
                graph.notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, "inlining monomorphic calls is disabled");
                return null;
            }
            ...
            if (!optimisticOpts.inlinePolymorphicCalls(options) && notRecordedTypeProbability == 0) {
                ...
                graph.notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, "inlining polymorphic calls is disabled (%d types)", ptypes.length);
                return null;
            }
            ...
            for (int i = 0; i < ptypes.length; i++) {
                ...
                if (concrete == null) {
                    ...
                    graph.notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, "could not resolve method");
                    return null;
                }
            ...
            if (concreteMethods.size() > maxMethodPerInlining) {
                ...
                graph.notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, "polymorphic call with more than %d target methods", maxMethodPerInlining);
                return null;
            }
            ...
            if (usedTypes.isEmpty()) {
                ...
                graph.notifyInliningDecision(invoke, false, "InliningPhase", null, null, null, "no types remaining after filtering less frequent types (%d types previously)",
                                ptypes.length);
                return null;
            }
