...
public class AMD64HotSpotLoweringProvider extends DefaultHotSpotLoweringProvider {
    ...
    public void initialize(OptionValues options, Iterable<DebugConfigCustomizer> customizers, HotSpotProviders providers, GraalHotSpotVMConfig config) {
        convertSnippets = new AMD64ConvertSnippets.Templates(options, customizers, providers, providers.getSnippetReflection(), providers.getCodeCache().getTarget());
        profileSnippets = ProfileNode.Options.ProbabilisticProfiling.getValue(options)
                        ? new ProbabilisticProfileSnippets.Templates(options, customizers, providers, providers.getCodeCache().getTarget()) : null;
