...
import static jdk.compiler.graal.debug.MemUseTrackerKey.getCurrentThreadAllocatedBytes;

import jdk.compiler.graal.api.test.Graal;
import jdk.compiler.graal.core.test.AllocSpy;
...
public class MemoryUsageBenchmark extends HotSpotGraalCompilerTest {
    ...
    private void doCompilation(String methodName, String label) {
        ...
        try (MemoryUsageCloseable c = label == null ? null : new MemoryUsageCloseable(label)) {
            ...
            task.runCompilation(GraalCompilerTest.getInitialOptions());
        }
    }
    ...
    public void run() {
        ...
        if (CompileTheWorld.Options.Classpath.getValue(harnessOptions) != CompileTheWorld.SUN_BOOT_CLASS_PATH) {
            HotSpotJVMCIRuntime runtime = HotSpotJVMCIRuntime.runtime();
            CompileTheWorld ctw = new CompileTheWorld(runtime, (HotSpotGraalCompiler) runtime.getCompiler(), harnessOptions, GraalCompilerTest.getInitialOptions());
            try {
                ctw.compile();
            } catch (Throwable e) {
                e.printStackTrace();
