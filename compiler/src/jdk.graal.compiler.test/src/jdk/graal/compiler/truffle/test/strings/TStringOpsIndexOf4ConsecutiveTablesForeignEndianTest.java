...
public class TStringOpsIndexOf4ConsecutiveTablesForeignEndianTest extends TStringOpsTest<ArrayIndexOfNode> {

    private Object[] constantArgs = new Object[7];
    private int strideA;

    public static List<Object[]> data() {
        return TStringOpsIndexOfConsecutiveTablesTestData.data(1, 4);
    }

    public TStringOpsIndexOf4ConsecutiveTablesForeignEndianTest() {
    ...
    protected InstalledCode getCode(final ResolvedJavaMethod installedCodeOwner, StructuredGraph graph, boolean ignoreForceCompile, boolean ignoreInstallAsDefault, OptionValues options) {
        return cacheInstalledCodeConstantStride(installedCodeOwner, graph, options, getIndexOf4ConsecutiveTablesForeignEndianIntl(), cache.get(), strideA, 0);
    }

    @Test
    public void testIndexOf4ConsecutiveTablesForeignEndian() {
        testParameterized(data(), args -> {
            this.strideA = (int) args[2];
            testIndexOf4ConsecutiveTablesForeignEndianCase(args);
        });
    }

    private void testIndexOf4ConsecutiveTablesForeignEndianCase(Object[] args) {
        ...
        Assume.assumeTrue(ArrayIndexOfNode.isSupported(getArchitecture(), Stride.fromLog2(strideA), LIRGeneratorTool.ArrayIndexOfVariant.FindFourConsecutiveTablesForeignEndian));
        constantArgs[4] = strideA;
        testWithNative(getIndexOf4ConsecutiveTablesForeignEndianIntl(), null, DUMMY_LOCATION, byteSwapArray(arrayA, strideA), offsetA, lengthA, strideA, fromIndexA, tables);
    }

    @Override
    protected void checkIntrinsicNode(ArrayIndexOfNode node) {
        Assert.assertEquals(LIRGeneratorTool.ArrayIndexOfVariant.FindFourConsecutiveTablesForeignEndian, node.getVariant());
