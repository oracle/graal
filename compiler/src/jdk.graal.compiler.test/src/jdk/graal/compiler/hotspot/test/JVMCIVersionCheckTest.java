...
public class JVMCIVersionCheckTest extends GraalCompilerTest {
    ...
    private static final int[] JVMCI_BUILD_NUMBERS = {
                    1,
                    2,
                    5,
                    11,
    ...
    public static Collection<Object[]> data() {
        ...
        for (String minJdkVersion : JDK_VERSIONS) {
            for (String jdkVersion : JDK_VERSIONS) {
                for (String minReleaseName : RELEASE_NAMES) {
                    for (String releaseName : RELEASE_NAMES) {
                        for (int minBuild : JVMCI_BUILD_NUMBERS) {
                            for (int build : JVMCI_BUILD_NUMBERS) {
                                Version version = getLabsJDKVersion(jdkVersion, releaseName, build);
                                Version minVersion = getLabsJDKVersion(minJdkVersion, minReleaseName, minBuild);
    ...
    private static Version getLabsJDKVersion(String jdkVersion, String releaseName, int build) {
        if (releaseName == null) {
            // legacy version without release name
            return JVMCIVersionCheck.createLabsJDKVersionUntilGraalVmForJDK25(jdkVersion, build);
        }
        return JVMCIVersionCheck.createLabsJDKVersion(jdkVersion, releaseName, build);
    }
    ...
    public void test01() {
        assert !version.toString().startsWith("jvmci");
        String javaVmVersion = version.toString(); // + "Suffix";
        ...
        } catch (JVMCIVersionCheck.IncomparableVersionException e) {
            // pass
        }
    }
}
