...
public class PriorityInliningRecursiveTest extends PriorityInliningTest {
    ...
    public static int recursiveMethod1(int a) {
        ...
        for (int i = 0; GraalDirectives.injectIterationCount(100000000, i < a); i++) {
            res += new A(recursiveMethod2(a - 1)).x;
        }
        return res;
    }

    public static int recursiveMethod2(int a) {
        if (GraalDirectives.injectBranchProbability(0.01D, a <= 0)) {
            return 0;
        }
        int res = 0;
        for (int i = 0; GraalDirectives.injectIterationCount(100000000, i < a); i++) {
            res += new A(recursiveMethod1(a - 1)).x;
        }
        return res;
    }
