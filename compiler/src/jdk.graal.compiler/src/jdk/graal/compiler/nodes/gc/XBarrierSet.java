...
/**
 * Base class for the barrier set for single gen ZGC. This provides the basic implementation that
 * every read of an object from the heap should have an associated read barrier. Read barriers for
 * non-heap locations like handles can be handled by subclasses.
 */
public class XBarrierSet implements BarrierSet {

    private final ResolvedJavaField referentField;

    public XBarrierSet(ResolvedJavaField referentField) {
