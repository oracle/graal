...
/**
 * Provides methods for reporting positive inlining decisions to the IDE.
 *
 * The IDE report is independent of {@code TraceInlining}: the standard inlining log is optional and
 * does not expose a build-scoped report sink. GR-61707 tracks consolidating these paths if the
 * standard log gains the required consumer contract.
 */
    ...
    public static void reportInlining(NodeSourcePosition srcPos, ResolvedJavaMethod callee, String phase) {
        ...
        runIfEnabled(ideReport -> {
            ...
            if (invocationSiteFilePath != null) {
                ...
                ideReport.saveLineReport(IDEReportCategory.INLINING, invocationSiteFilePath, className, line, "Call to " + calleeName + " is inlined by phase " + phase,
                                getInliningTrace(srcPos));
            }
        });
    }
