 ...
 */
public class PriorityInliningPhase extends AbstractInliningPhase {
    ...
    static {
        if (inBuildtimeCode()) {
            // Force loading of this service since services can't be loaded lazily in native images.
    ...
    public PriorityInliningPhase(CanonicalizerPhase canonicalizer, OptionValues options, InliningProvider inliningProvider) {
        ...
        this.policyFactory = inliningProvider.policy(options);
        this.trackInliningMode = Options.TrackInliningStatistics.getValue(options);
        this.priorityForceInlineFilter = Options.PriorityForceInline.getValue(options) == null ? null : MethodFilter.parse(Options.PriorityForceInline.getValue(options));
    ...
    public Instance createInstance(StructuredGraph graph, HighTierContext context) {
        ...
        return new Instance(context, graph, optimizationDuration, totalDuration, expanderExtraAnalysisDuration);
    }
    ...
    public class Instance {
        ...
        protected Instance(HighTierContext context, StructuredGraph graph, TimerKey optimizationDuration, TimerKey totalDuration, TimerKey expanderExtraAnalysisDuration) {
            ...
            final Expander.Policy expanderPolicy = policyFactory.createExpanderPolicy(options, context);
            final Inliner.Policy inlinerPolicy = policyFactory.createInlinerPolicy(options);
            final TuningPolicy analysis = policyFactory.createTuningPolicy(options);
