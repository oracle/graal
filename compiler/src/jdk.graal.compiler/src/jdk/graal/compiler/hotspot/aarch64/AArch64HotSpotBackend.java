            ...
            if (config.icSpeculatedKlassOffset == Integer.MAX_VALUE) {
                ...
                if (config.useCompressedClassPointers) {
                    if (config.useCompactObjectHeaders) {
                        masm.ldr(64, klass, masm.makeAddress(64, receiver, config.markOffset));
                        masm.lsr(64, klass, klass, config.markWordKlassShift);
                    } else {
                ...
                if (config.useCompactObjectHeaders) {
                    masm.ldr(64, tmp1, masm.makeAddress(64, receiver, config.markOffset));
                    masm.lsr(64, tmp1, tmp1, config.markWordKlassShift);
                } else {
