...
public class DynamicDeoptimizationGroupingPhase extends BasePhase<MidTierContext> {
    private final DeoptimizationGroupingPhase deoptimizationGroupingPhase;

    public DynamicDeoptimizationGroupingPhase(DeoptimizationGroupingPhase deoptimizationGroupingPhase) {
        this.deoptimizationGroupingPhase = deoptimizationGroupingPhase;
    }
    ...
    protected void run(StructuredGraph graph, MidTierContext context) {
        ...
        if (deoptCycleDetectionThreshold < 0 || truffleCompilable.getSuccessfulCompilationCount() < deoptCycleDetectionThreshold) {
            deoptimizationGroupingPhase.apply(graph, context);
        }
    }
}
