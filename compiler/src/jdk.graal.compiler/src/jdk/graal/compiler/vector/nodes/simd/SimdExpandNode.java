...
/**
 * This node is similar to {@link jdk.graal.compiler.nodes.calc.ExpandBitsNode}, the difference is
 * that this node expands the elements of the first vector operand based on the bitmask given by the
 * second opmask operand. This node would generate {@code vpexpand dst{mask}{z}, src} in the AMD64
 * backend.
 */
@NodeInfo(cycles = NodeCycles.CYCLES_2, size = NodeSize.SIZE_1)
public class SimdExpandNode extends BinaryNode implements VectorLIRLowerable {
    public static final NodeClass<SimdExpandNode> TYPE = NodeClass.create(SimdExpandNode.class);

    protected SimdExpandNode(SimdStamp stamp, ValueNode src, ValueNode mask) {
    ...
    public static ValueNode create(ValueNode src, ValueNode mask) {
        ...
        return new SimdExpandNode(stamp, src, mask);
    }
    ...
    public void generate(NodeLIRBuilderTool builder, VectorLIRGeneratorTool gen) {
        ...
        builder.setResult(this, gen.emitVectorExpand(kind, builder.operand(getX()), builder.operand(getY())));
    }
