...
public sealed interface ClassFilter permits PrefixFilter, CompositeFilter, AcceptAllFilter {
    ...
    static ClassFilter parseFilterDescr(String filterDescr) {
        if (filterDescr.isBlank()) {
            return AcceptAllFilter.INSTANCE;
        }
        var individualFilterDescr = filterDescr.split(",", -1);
        var filters = new ArrayList<ClassFilter>(individualFilterDescr.length);
        for (var descr : individualFilterDescr) {
            var trimmedDescr = descr.trim();
            if (trimmedDescr.isEmpty()) {
                throw new IllegalArgumentException("IDE report class filter must not contain empty prefixes: '" + filterDescr + "'");
            }
            filters.add(new PrefixFilter(trimmedDescr));
        }
