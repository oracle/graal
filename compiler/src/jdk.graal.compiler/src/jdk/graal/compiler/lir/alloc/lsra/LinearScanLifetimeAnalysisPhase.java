...
public class LinearScanLifetimeAnalysisPhase extends LinearScanAllocationPhase {
            ...
            try (Indent indent = debug.logAndIndent("report failure")) {
                ...
                try (Indent indent2 = debug.logAndIndent("Error: liveIn set of first block must be empty (when this fails, variables are used before they are defined):")) {
                    for (int operandNum = startBlockLiveIn.iterateValues(0); operandNum >= 0; operandNum = startBlockLiveIn.iterateValues(operandNum + 1)) {
                        ...
                        if (interval != null) {
                            ...
                            debug.log("var %d; operand=%s; node=%s", operandNum, operand, LIRGenerationDebugContext.getSourceForOperandFromDebugContext(debug, operand));
                        } else {
                ...
                for (int operandNum = startBlockLiveIn.iterateValues(0); operandNum >= 0; operandNum = startBlockLiveIn.iterateValues(operandNum + 1)) {
                    ...
                    if (interval != null) {
                        ...
                        valueForOperandFromDebugContext = LIRGenerationDebugContext.getSourceForOperandFromDebugContext(debug, operand);
                    }
    ...
    protected void addUse(AllocatableValue operand, int from, int to, RegisterPriority registerPriority, ValueKind<?> kind, boolean detailedAsserts) {
        ...
        if (!kind.equals(LIRKind.Illegal) && !LIRValueUtil.isCast(operand)) {
    ...
    protected void addRegisterHint(final LIRInstruction op, final Value targetValue, LIRInstruction.OperandMode mode, EnumSet<LIRInstruction.OperandFlag> flags, final boolean hintAtDef) {
        if (flags.contains(LIRInstruction.OperandFlag.HINT) && LinearScan.isVariableOrRegister(targetValue)) {
     ...
     * register.
     */
    protected static RegisterPriority registerPriorityOfInputOperand(EnumSet<LIRInstruction.OperandFlag> flags) {
