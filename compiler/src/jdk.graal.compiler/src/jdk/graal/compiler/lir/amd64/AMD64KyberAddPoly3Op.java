...
import static jdk.graal.compiler.lir.amd64.AMD64KyberSupport.XMM16_23;
...
// @formatter:off
@SyncPort(from = "https://github.com/openjdk/jdk25u/blob/d8d51f3327b133af4f21cd6dd248324e4fc2a482/src/hotspot/cpu/x86/stubGenerator_x86_64_kyber.cpp#L755-L801",
          sha1 = "d46ce33af3303fe7c1a888c399076f85a41aabce")
// @formatter:on
public final class AMD64KyberAddPoly3Op extends AMD64LIRInstruction {
    public static final LIRInstructionClass<AMD64KyberAddPoly3Op> TYPE = LIRInstructionClass.create(AMD64KyberAddPoly3Op.class);
    ...
    @Use({REG}) private Value cValue;
    @Temp({REG}) private Value[] temps;

    public AMD64KyberAddPoly3Op(AllocatableValue resultValue, AllocatableValue resultArrayValue, AllocatableValue aValue, AllocatableValue bValue, AllocatableValue cValue) {
        super(TYPE);
        ...
        guaranteeFixedRegister(cValue, rcx, "cValue");
    ...
    public void emitCode(CompilationResultBuilder crb, AMD64MacroAssembler masm) {
        ...
        GraalError.guarantee(cValue.getPlatformKind().equals(AMD64Kind.QWORD), "Invalid %s kind: %s", "cValue", cValue);
        Register c = asRegister(cValue);
        ...
        for (int i = 0; i < 8; i++) {
            ...
            masm.evmovdqu16(XMM16_23[i], new AMD64Address(c, 64 * i));
        }
        EVPADDW.emit(masm, AVXSize.ZMM, xmm31, xmm31, xmm31);
        for (int i = 0; i < 8; i++) {
            EVPADDW.emit(masm, AVXSize.ZMM, XMM0_7[i], XMM0_7[i], XMM8_15[i]);
        }
        for (int i = 0; i < 8; i++) {
            EVPADDW.emit(masm, AVXSize.ZMM, XMM0_7[i], XMM0_7[i], XMM16_23[i]);
        }
        for (int i = 0; i < 8; i++) {
