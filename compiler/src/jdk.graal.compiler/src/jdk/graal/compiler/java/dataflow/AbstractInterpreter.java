...
/**
 * A {@link DataFlowAnalyzer} where the data flow state is represented by an abstract bytecode
 * execution frame. This analyzer assumes that the provided bytecode is valid and verified by a
 * bytecode verifier.
 *
...
public abstract class AbstractInterpreter<T> extends DataFlowAnalyzer<AbstractFrame<T>> {
    ...
    private void handleArrayElementLoad(Context context, AbstractFrame<T> state) {
    ...
    private void handleCast(Context context, AbstractFrame<T> state) {
        T value = state.getOperandStack().pop().value();
