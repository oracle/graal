...
/**
 * A data flow analyzer where the data flow state is represented by an abstract bytecode execution
 * frame.
 *
...
public abstract class AbstractInterpreter<T> extends DataFlowAnalyzer<AbstractFrame<T>> {
    ...
    protected AbstractFrame<T> createExceptionState(AbstractFrame<T> inState, List<JavaType> exceptionTypes) {
        /*
         * The initial frame state when entering an exception handler is created by clearing the
         * operand stack and placing only one object, representing the caught exception, on it.
         */
    ...
    protected AbstractFrame<T> processInstruction(AbstractFrame<T> inState, BytecodeStream stream, Bytecode code) throws DataFlowAnalysisException {
        ...
        switch (opcode) {
            case NOP            : break;
            case ACONST_NULL    : handleConstant(context, outState, JavaConstant.NULL_POINTER); break;
            case ICONST_M1      : handleConstant(context, outState, JavaConstant.forInt(-1)); break;
            ...
            case ICONST_5       : handleConstant(context, outState, JavaConstant.forInt(opcode - ICONST_0)); break;
            case LCONST_0       : // fall through
            case LCONST_1       : handleConstant(context, outState, JavaConstant.forLong((long) opcode - LCONST_0)); break;
            case FCONST_0       : // fall through
            case FCONST_1       : // fall through
            case FCONST_2       : handleConstant(context, outState, JavaConstant.forFloat((float) opcode - FCONST_0)); break;
            case DCONST_0       : // fall through
            case DCONST_1       : handleConstant(context, outState, JavaConstant.forDouble((double) opcode - DCONST_0)); break;
            case BIPUSH         : handleConstant(context, outState, JavaConstant.forByte(stream.readByte())); break;
            case SIPUSH         : handleConstant(context, outState, JavaConstant.forShort(stream.readShort())); break;
            case LDC            : // fall through
            case LDC_W          : // fall through
            case LDC2_W         : handleConstant(context, outState, getConstant(code, stream)); break;
            ...
            case SALOAD         : handleArrayElementLoad(context, outState); break;
            ...
            case I2C            : handleCast(context, outState); break;
            ...
            case ARETURN        : returnValue(context, outState, outState.getOperandStack().pop().value()); break;
            ...
            case ATHROW         : doThrow(context, outState, outState.getOperandStack().pop().value()); break;
            ...
            case MONITOREXIT    : monitorOperation(context, outState, outState.getOperandStack().pop().value()); break;
    ...
    private void handleConstant(Context context, AbstractFrame<T> state, Object value) {
        ...
        if (value instanceof Constant constant) {
            state.getOperandStack().push(wrap(pushConstant(context, state, constant), size));
        } else if (value instanceof JavaType type) {
            state.getOperandStack().push(wrap(pushType(context, state, type), size));
        }
    }

    private void handleVariableLoad(Context context, AbstractFrame<T> state, int variableIndex) throws DataFlowAnalysisException {
        AbstractFrame.SizedValue<T> value = state.getLocalVariableTable().get(variableIndex);
        state.getOperandStack().push(wrap(loadVariable(context, state, variableIndex, value.value()), value.size()));
    }

    private void handleArrayElementLoad(Context context, AbstractFrame<T> state) throws DataFlowAnalysisException {
        T index = state.getOperandStack().pop().value();
        T array = state.getOperandStack().pop().value();
        AbstractFrame.SizedValue.Slots size = context.opcode == LALOAD || context.opcode == DALOAD
                        ? AbstractFrame.SizedValue.Slots.TWO_SLOTS
                        : AbstractFrame.SizedValue.Slots.ONE_SLOT;
        state.getOperandStack().push(wrap(loadArrayElement(context, state, array, index), size));
    }

    private void handleVariableStore(Context context, AbstractFrame<T> state, int variableIndex) throws DataFlowAnalysisException {
        AbstractFrame.SizedValue<T> value = state.getOperandStack().pop();
        state.getLocalVariableTable().put(variableIndex, wrap(storeVariable(context, state, variableIndex, value.value()), value.size()));
    }

    private void handleArrayElementStore(Context context, AbstractFrame<T> state) throws DataFlowAnalysisException {
    ...
    private void handleBinaryOperation(Context context, AbstractFrame<T> state) throws DataFlowAnalysisException {
        AbstractFrame.SizedValue<T> right = state.getOperandStack().pop();
        AbstractFrame.SizedValue<T> left = state.getOperandStack().pop();
        state.getOperandStack().push(wrap(binaryOperation(context, state, left.value(), right.value()), left.size()));
    }

    private void handleUnaryOperation(Context context, AbstractFrame<T> state) throws DataFlowAnalysisException {
        AbstractFrame.SizedValue<T> value = state.getOperandStack().pop();
        state.getOperandStack().push(wrap(unaryOperation(context, state, value.value()), value.size()));
    }

    private void handleIncrement(Context context, AbstractFrame<T> state, int variableIndex, int incrementBy) throws DataFlowAnalysisException {
        AbstractFrame.SizedValue<T> value = state.getLocalVariableTable().get(variableIndex);
        state.getLocalVariableTable().put(variableIndex, wrap(incrementVariable(context, state, variableIndex, incrementBy, value.value()), value.size()));
    }

    private void handleCast(Context context, AbstractFrame<T> state) throws DataFlowAnalysisException {
        T value = state.getOperandStack().pop().value();
        AbstractFrame.SizedValue.Slots size = List.of(I2D, L2D, F2L, F2D, D2L, I2L).contains(context.opcode)
                        ? AbstractFrame.SizedValue.Slots.TWO_SLOTS
                        : AbstractFrame.SizedValue.Slots.ONE_SLOT;
        state.getOperandStack().push(wrap(castOperation(context, state, value), size));
    }

    private void handleCompare(Context context, AbstractFrame<T> state) throws DataFlowAnalysisException {
        T right = state.getOperandStack().pop().value();
        T left = state.getOperandStack().pop().value();
        state.getOperandStack().push(wrap(comparisonOperation(context, state, left, right), AbstractFrame.SizedValue.Slots.ONE_SLOT));
    }

    private void handleUnaryConditionalJump(Context context, AbstractFrame<T> state, int ifDestinationBci, int elseDestinationBci) throws DataFlowAnalysisException {
        T value = state.getOperandStack().pop().value();
        unaryConditionalJump(context, state, ifDestinationBci, elseDestinationBci, value);
    }

    private void handleBinaryConditionalJump(Context context, AbstractFrame<T> state, int ifDestinationBci, int elseDestinationBci) throws DataFlowAnalysisException {
        T right = state.getOperandStack().pop().value();
        T left = state.getOperandStack().pop().value();
        binaryConditionalJump(context, state, ifDestinationBci, elseDestinationBci, left, right);
    }

    private void handleSwitch(Context context, AbstractFrame<T> state, BytecodeSwitch bcSwitch) throws DataFlowAnalysisException {
        T value = state.getOperandStack().pop().value();
    ...
    private void handleStaticFieldLoad(Context context, AbstractFrame<T> state, JavaField field) {
        ...
        state.getOperandStack().push(wrap(loadStaticField(context, state, field), size));
    }

    private void handleStaticFieldStore(Context context, AbstractFrame<T> state, JavaField field) throws DataFlowAnalysisException {
        T value = state.getOperandStack().pop().value();
        storeStaticField(context, state, field, value);
    }

    private void handleFieldLoad(Context context, AbstractFrame<T> state, JavaField field) throws DataFlowAnalysisException {
        JavaKind fieldKind = field.getJavaKind();
        AbstractFrame.SizedValue.Slots size = getSizeForKind(fieldKind);
        T object = state.getOperandStack().pop().value();
        state.getOperandStack().push(wrap(loadField(context, state, field, object), size));
    }

    private void handleFieldStore(Context context, AbstractFrame<T> state, JavaField field) throws DataFlowAnalysisException {
        T value = state.getOperandStack().pop().value();
        T object = state.getOperandStack().pop().value();
        storeField(context, state, field, object, value);
    }

    private void handleInvoke(Context context, AbstractFrame<T> state, JavaMethod method, JavaConstant appendix) throws DataFlowAnalysisException {
        if (appendix != null) {
        ...
        if (returnKind.equals(JavaKind.Void)) {
            ...
            state.getOperandStack().push(wrap(invokeMethod(context, state, method, operands), size));
        }
    }

    private void handleNew(Context context, AbstractFrame<T> state, JavaType type) {
        state.getOperandStack().push(wrap(newObject(context, state, type), AbstractFrame.SizedValue.Slots.ONE_SLOT));
    }

    private void handleNewArray(Context context, AbstractFrame<T> state, JavaType type, int dimensions) throws DataFlowAnalysisException {
        List<T> counts = new ArrayList<>(dimensions);
        for (int i = 0; i < dimensions; i++) {
            counts.add(state.getOperandStack().pop().value());
        }
        counts = counts.reversed();
        state.getOperandStack().push(wrap(newArray(context, state, type, counts), AbstractFrame.SizedValue.Slots.ONE_SLOT));
    }
    ...
    private void handleArrayLength(Context context, AbstractFrame<T> state) throws DataFlowAnalysisException {
        T array = state.getOperandStack().pop().value();
        state.getOperandStack().push(wrap(arrayLength(context, state, array), AbstractFrame.SizedValue.Slots.ONE_SLOT));
    }

    private void handleCastCheck(Context context, AbstractFrame<T> state, JavaType type) throws DataFlowAnalysisException {
        T object = state.getOperandStack().pop().value();
        state.getOperandStack().push(wrap(castCheckOperation(context, state, type, object), AbstractFrame.SizedValue.Slots.ONE_SLOT));
    }
     ...
     * for the LDC, LDC_W and LDC2_W instructions.
     *
     * @param context The execution context for this handler.
     * @param state The abstract frame being modified by this instruction.
