...
public abstract class AbstractInterpreter<T> extends DataFlowAnalyzer<AbstractFrame<T>> {
    ...
    protected AbstractFrame<T> processInstruction(AbstractFrame<T> inState, BytecodeStream stream, Bytecode code) {
        ...
        switch (opcode) {
            case NOP            : break;
            case ACONST_NULL    : handleConstant(opcode, bci, outState, JavaConstant.NULL_POINTER, code.getMethod()); break;
            case ICONST_M1      : handleConstant(opcode, bci, outState, JavaConstant.forInt(-1), code.getMethod()); break;
            ...
            case ICONST_5       : handleConstant(opcode, bci, outState, JavaConstant.forInt(opcode - ICONST_0), code.getMethod()); break;
            case LCONST_0       : // fall through
            case LCONST_1       : handleConstant(opcode, bci, outState, JavaConstant.forLong((long) opcode - LCONST_0), code.getMethod()); break;
            case FCONST_0       : // fall through
            case FCONST_1       : // fall through
            case FCONST_2       : handleConstant(opcode, bci, outState, JavaConstant.forFloat((float) opcode - FCONST_0), code.getMethod()); break;
            case DCONST_0       : // fall through
            case DCONST_1       : handleConstant(opcode, bci, outState, JavaConstant.forDouble((double) opcode - DCONST_0), code.getMethod()); break;
            case BIPUSH         : handleConstant(opcode, bci, outState, JavaConstant.forByte(stream.readByte()), code.getMethod()); break;
            case SIPUSH         : handleConstant(opcode, bci, outState, JavaConstant.forShort(stream.readShort()), code.getMethod()); break;
            case LDC            : // fall through
            case LDC_W          : // fall through
            case LDC2_W         : handleConstant(opcode, bci, outState, code.getConstantPool().lookupConstant(stream.readCPI()), code.getMethod()); break;
            ...
            case RET            : throw GraalError.shouldNotReachHere("Unsupported opcode " + opcode);
            ...
            default             : throw GraalError.shouldNotReachHere("Unsupported opcode " + opcode);
        }
        // @formatter:on
        // Checkstyle: resume
    ...
    private static JavaType tryToResolve(JavaType type, ResolvedJavaMethod accessingMethod) {
        if (type instanceof UnresolvedJavaType unresolvedType) {
            try {
                ResolvedJavaType resolved = unresolvedType.resolve(accessingMethod.getDeclaringClass());
                if (resolved != null) {
                    return resolved;
                }
            } catch (LinkageError e) {
                // Ignore and leave the type as unresolved.
            }
        }
        return type;
    }

    private static JavaType getJavaType(Bytecode code, BytecodeStream stream) {
        JavaType type = code.getConstantPool().lookupType(stream.readCPI(), stream.currentBC());
        return tryToResolve(type, code.getMethod());
    }

    private static JavaField getJavaField(Bytecode code, BytecodeStream stream) {
        return code.getConstantPool().lookupField(stream.readCPI(), code.getMethod(), stream.currentBC());
    }

    private static JavaMethod getJavaMethod(Bytecode code, BytecodeStream stream) {
        int opcode = stream.currentBC();
        int cpi = opcode == INVOKEDYNAMIC ? stream.readCPI4() : stream.readCPI();
        return code.getConstantPool().lookupMethod(cpi, opcode);
    }
    ...
    private void handleConstant(int opcode, int bci, AbstractFrame<T> state, Object value, ResolvedJavaMethod accessingMethod) {
        ...
        if (value instanceof Constant constant) {
            ...
            JavaType type = tryToResolve((JavaType) value, accessingMethod);
            state.getOperandStack().push(wrap(pushType(opcode, bci, state, type), size));
        }
    }

    private void handleVariableLoad(int opcode, int bci, AbstractFrame<T> state, int variableIndex) {
    ...
    private void handleArrayElementLoad(int opcode, int bci, AbstractFrame<T> state) {
    ...
    private void handleVariableStore(int opcode, int bci, AbstractFrame<T> state, int variableIndex) {
    ...
    private void handleArrayElementStore(int opcode, int bci, AbstractFrame<T> state) {
    ...
    private void handleBinaryOperation(int opcode, int bci, AbstractFrame<T> state) {
    ...
    private void handleUnaryOperation(int opcode, int bci, AbstractFrame<T> state) {
        AbstractFrame.SizedValue<T> value = state.getOperandStack().pop();
        state.getOperandStack().push(wrap(unaryOperation(opcode, bci, state, value.value()), value.size()));
    }

    private void handleIncrement(int opcode, int bci, AbstractFrame<T> state, int variableIndex, int incrementBy) {
        AbstractFrame.SizedValue<T> value = state.getLocalVariableTable().get(variableIndex);
    ...
    private void handleCast(int opcode, int bci, AbstractFrame<T> state) {
        T value = state.getOperandStack().pop().value();
    ...
    private void handleCompare(int opcode, int bci, AbstractFrame<T> state) {
    ...
    private void handleUnaryConditionalJump(int opcode, int bci, AbstractFrame<T> state, int ifDestinationBci, int elseDestinationBci) {
        T value = state.getOperandStack().pop().value();
        unaryConditionalJump(opcode, bci, state, ifDestinationBci, elseDestinationBci, value);
    }

    private void handleBinaryConditionalJump(int opcode, int bci, AbstractFrame<T> state, int ifDestinationBci, int elseDestinationBci) {
        T right = state.getOperandStack().pop().value();
        T left = state.getOperandStack().pop().value();
        binaryConditionalJump(opcode, bci, state, ifDestinationBci, elseDestinationBci, left, right);
    }

    private void handleSwitch(int opcode, int bci, AbstractFrame<T> state, BytecodeSwitch bcSwitch) {
        T value = state.getOperandStack().pop().value();
    ...
    private void handleStaticFieldStore(int opcode, int bci, AbstractFrame<T> state, JavaField field) {
        T value = state.getOperandStack().pop().value();
        storeStaticField(opcode, bci, state, field, value);
    }

    private void handleFieldLoad(int opcode, int bci, AbstractFrame<T> state, JavaField field) {
        JavaKind fieldKind = field.getJavaKind();
        AbstractFrame.SizedValue.Slots size = getSizeForKind(fieldKind);
    ...
    private void handleFieldStore(int opcode, int bci, AbstractFrame<T> state, JavaField field) {
        T value = state.getOperandStack().pop().value();
        T object = state.getOperandStack().pop().value();
        storeField(opcode, bci, state, field, object, value);
    }

    private void handleInvoke(int opcode, int bci, AbstractFrame<T> state, JavaMethod method, JavaConstant appendix) {
        if (appendix != null) {
    ...
    private void handleNewArray(int opcode, int bci, AbstractFrame<T> state, JavaType type, int dimensions) {
    ...
    private void handleArrayLength(int opcode, int bci, AbstractFrame<T> state) {
        T array = state.getOperandStack().pop().value();
        state.getOperandStack().push(wrap(arrayLength(opcode, bci, state, array), AbstractFrame.SizedValue.Slots.ONE_SLOT));
    }

    private void handleCastCheck(int opcode, int bci, AbstractFrame<T> state, JavaType type) {
        T object = state.getOperandStack().pop().value();
    ...
    /**
     * Handler for loading a method's appendix. This is used in INVOKEDYNAMIC instructions.
     *
