...
public class AgnosticInliningPhaseTest extends PartialEvaluationTest {
    ...
    @Before
    public void before() {
        context = Context.newBuilder().allowAllAccess(true).allowExperimentalOptions(true).option("engine.LanguageAgnosticInlining", Boolean.TRUE.toString()).option("engine.InliningInliningBudget",
                        "1").build();
        context.enter();
    }
    ...
    protected StructuredGraph runLanguageAgnosticInliningPhase(OptimizedCallTarget callTarget) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        ...
        final StructuredGraph graph = (StructuredGraph) createGraphForPE.invoke(partialEvaluator,
                        ...
                        getSpeculationLog(),
                        null);
        final AgnosticInliningPhase agnosticInliningPhase = new AgnosticInliningPhase(partialEvaluator, callNodeProvider, callTarget, callTarget.getOptionValues());
