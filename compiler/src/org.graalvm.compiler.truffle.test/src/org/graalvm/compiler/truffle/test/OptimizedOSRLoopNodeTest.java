...
public class OptimizedOSRLoopNodeTest extends TestWithSynchronousCompiling {
    ...
    private static final int OSR_THRESHOLD = TruffleRuntimeOptions.getValue(SharedTruffleRuntimeOptions.TruffleOSRCompilationThreshold);
    private static final int OSR_INVALIDATION_REPROFILE = TruffleRuntimeOptions.getValue(SharedTruffleRuntimeOptions.TruffleInvalidationReprofileCount);
    ...
    public void testOSRAndRewriteDoesNotSuppressTargetCompilation(OSRLoopFactory factory) {
        try (TruffleRuntimeOptions.TruffleRuntimeOptionsOverrideScope s = TruffleRuntimeOptions.overrideOptions(SharedTruffleRuntimeOptions.TruffleCompilationThreshold, 3)) {
            ...
            for (int i = 0; i < TruffleRuntimeOptions.getValue(SharedTruffleRuntimeOptions.TruffleCompilationThreshold) + TruffleRuntimeOptions.getValue(SharedTruffleRuntimeOptions.TruffleReplaceReprofileCount) - 1; i++) {
                target.call(2);
            }
            assertCompiled(rootNode.getOSRTarget());
    ...
    public void testOuterInvalidationTriggersOSR(OSRLoopFactory factory) {
        ...
        int i;
        for (i = 0; i < TruffleRuntimeOptions.getValue(SharedTruffleRuntimeOptions.TruffleMinInvokeThreshold) - 2; i++) {
            target.call(0);
            assertNotCompiled(target);
            assertCompiled(rootNode.getOSRTarget());
        }
    ...
    public void testNoOSRAfterMinInvocationThreshold(OSRLoopFactory factory) {
        ...
        RootCallTarget target = runtime.createCallTarget(rootNode);
        int i;
        for (i = 0; i < TruffleRuntimeOptions.getValue(SharedTruffleRuntimeOptions.TruffleMinInvokeThreshold); i++) {
            target.call(0);
            assertNotCompiled(rootNode.getOSRTarget());
        }
    ...
    public void testOSRMinInvocationThresholdPropagateLoopCounts(OSRLoopFactory factory) {
        ...
        int truffleMinInvokes = TruffleRuntimeOptions.getValue(SharedTruffleRuntimeOptions.TruffleMinInvokeThreshold);
