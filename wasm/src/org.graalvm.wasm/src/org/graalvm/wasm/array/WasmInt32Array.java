...
public final class WasmInt32Array extends WasmArray {

    private final int[] array;

    public WasmInt32Array(DefinedType type, int[] array) {
        super(type, array.length);
        assert type.asArrayType().fieldType().storageType() == NumberType.I32;
        this.array = array;
    }

    public WasmInt32Array(DefinedType type, int length, int initialValue) {
        this(type, new int[length]);
        fill(0, length, initialValue);
    }

    public WasmInt32Array(DefinedType type, int length) {
        this(type, new int[length]);
    }

    public WasmInt32Array(DefinedType type, int length, byte[] source, int srcOffset) {
        this(type, new int[length]);
    ...
    public void copyFrom(WasmInt32Array src, int srcOffset, int dstOffset, int length) {
    ...
    public void initialize(byte[] source, int srcOffset, int dstOffset, int length) {
        for (int i = 0; i < length; i++) {
            array[dstOffset + i] = byteArraySupport.getInt(source, srcOffset + (i << 2));
        }
    }
