...
public final class WasmInt8Array extends WasmArray {

    private final byte[] array;

    public WasmInt8Array(DefinedType type, byte[] array) {
        super(type, array.length);
        assert type.asArrayType().fieldType().storageType() == PackedType.I8;
        this.array = array;
    }

    public WasmInt8Array(DefinedType type, int length, byte initialValue) {
        this(type, new byte[length]);
        fill(0, length, initialValue);
    }

    public WasmInt8Array(DefinedType type, int length) {
        this(type, new byte[length]);
    }

    public WasmInt8Array(DefinedType type, int length, byte[] source, int srcOffset) {
        this(type, new byte[length]);
    ...
    public void copyFrom(WasmInt8Array src, int srcOffset, int dstOffset, int length) {
