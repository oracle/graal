...
public final class WasmFloat64Array extends WasmArray {

    private final double[] array;

    public WasmFloat64Array(DefinedType type, double[] array) {
        super(type, array.length);
        assert type.asArrayType().fieldType().storageType() == NumberType.F64;
        this.array = array;
    }

    public WasmFloat64Array(DefinedType type, int length, double initialValue) {
        this(type, new double[length]);
        fill(0, length, initialValue);
    }

    public WasmFloat64Array(DefinedType type, int length) {
        this(type, new double[length]);
    }

    public WasmFloat64Array(DefinedType type, int length, byte[] source, int srcOffset) {
        this(type, new double[length]);
    ...
    public void copyFrom(WasmFloat64Array src, int srcOffset, int dstOffset, int length) {
    ...
    public void initialize(byte[] source, int srcOffset, int dstOffset, int length) {
        for (int i = 0; i < length; i++) {
            array[dstOffset + i] = byteArraySupport.getDouble(source, srcOffset + (i << 3));
        }
    }
